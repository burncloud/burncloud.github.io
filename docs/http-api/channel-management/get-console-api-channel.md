...
| 8 | `crates/server/src/api/response.rs` | `err(), ok()` | 由 check_admin() 直接调用；由 list_channels() 直接调用 | CALL / runtime-specific |
| 9 | `crates/database/src/placeholder.rs` | `ph()` | 由 ChannelProviderModel::list() 直接调用 | CALL / runtime-specific |
