...
| 8 | `crates/server/src/api/response.rs` | `err(), ok()` | 由 check_admin() 直接调用；由 delete_channel() 直接调用 | CALL / runtime-specific |
| 9 | `crates/database/src/placeholder.rs` | `adapt_sql()` | 由 ChannelProviderModel::delete() 直接调用 | CALL / runtime-specific |
