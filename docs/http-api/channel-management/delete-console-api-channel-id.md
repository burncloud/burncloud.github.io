---
title: "DELETE /console/api/channel/{id}"
slug: /http-api/channel-management/delete-console-api-channel-id
hide_table_of_contents: true
---

# DELETE /console/api/channel/&#123;id&#125;

**树路径：** `BurnCloud → HTTP / API → Channel Management → DELETE /console/api/channel/{id}`

> **中文解释：** 管理员删除指定 Channel。 核心调用：ChannelService::delete。
>
...
```http
DELETE /console/api/channel/12 HTTP/1.1
...
| 4 | `crates/server/src/api/channel.rs` | `delete_channel(), check_admin()` | 参数、管理员授权、Handler 响应映射 | READ/WRITE request domain |
...
| 8 | `crates/server/src/api/response.rs` | `err(), ok()` | 由 auth_middleware() 直接调用；由 check_admin() 直接调用；由 delete_channel() 直接调用 | CALL / runtime-specific |
| 9 | `crates/database/src/database.rs` | `Database::fetch_all(), Database::kind(), Database::query(), DatabaseConnection::pool()` | 由 ChannelProviderModel::delete() 直接调用；由 Database::fetch_all() 直接调用；由 Database::query() 直接调用 | CALL / runtime-specific |
| 10 | `crates/database/src/placeholder.rs` | `adapt_sql()` | 由 ChannelProviderModel::delete() 直接调用 | CALL / runtime-specific |
