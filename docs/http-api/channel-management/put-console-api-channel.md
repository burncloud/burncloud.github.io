...
| 8 | `crates/server/src/api/response.rs` | `err(), ok()` | 由 check_admin() 直接调用；由 update_channel() 直接调用 | CALL / runtime-specific |
| 9 | `crates/database/src/placeholder.rs` | `adapt_sql()` | 由 ChannelProviderModel::sync_abilities() 直接调用；由 ChannelProviderModel::update() 直接调用 | CALL / runtime-specific |
