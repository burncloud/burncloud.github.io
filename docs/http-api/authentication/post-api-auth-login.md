...
| 3 | `crates/server/src/api/auth.rs` | `login()` | Auth DTO / Handler / response policy | REQUEST/RESPONSE |
| 4 | `crates/service/crates/user/src/lib.rs` | `UserService::login_user → get_user_roles` | password/JWT/OAuth/user business logic | SERVICE |
