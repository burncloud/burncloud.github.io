---
title: "GET /console/api/tokens"
slug: /http-api/token/get-console-api-tokens
hide_table_of_contents: true
---

# GET /console/api/tokens

**树路径：** `BurnCloud → HTTP / API → Token → GET /console/api/tokens`

> **中文解释：** 列出 Router Token。 核心调用：TokenService::list。
>
...
```http
GET /console/api/tokens HTTP/1.1
...
| 4 | `crates/server/src/api/token.rs` | `list_tokens()` | Token Handler / request validation / response mapping | READ/WRITE token request |
...
| 7 | `crates/server/src/api/response.rs` | `err(), ok()` | 由 auth_middleware() 直接调用；由 list_tokens() 直接调用 | CALL / runtime-specific |
| 8 | `crates/database/src/database.rs` | `Database::fetch_all(), DatabaseConnection::pool()` | 由 Database::fetch_all() 直接调用；由 RouterTokenModel::list() 直接调用 | CALL / runtime-specific |
