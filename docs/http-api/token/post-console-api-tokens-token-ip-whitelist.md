---
title: "POST /console/api/tokens/{token}/ip-whitelist"
slug: /http-api/token/post-console-api-tokens-token-ip-whitelist
hide_table_of_contents: true
---

# POST /console/api/tokens/&#123;token&#125;/ip-whitelist

**树路径：** `BurnCloud → HTTP / API → Token → POST /console/api/tokens/{token}/ip-whitelist`

> **中文解释：** 更新 token 的 IP whitelist。 核心调用：TokenService::set_ip_whitelist。
>
...
```http
POST /console/api/tokens/bc_live_7d4e...example/ip-whitelist HTTP/1.1
...
| 4 | `crates/server/src/api/token.rs` | `set_ip_whitelist()` | Token Handler / request validation / response mapping | READ/WRITE token request |
...
| 7 | `crates/server/src/api/response.rs` | `err(), ok()` | 由 auth_middleware() 直接调用；由 set_ip_whitelist() 直接调用 | CALL / runtime-specific |
| 8 | `crates/database/src/placeholder.rs` | `adapt_sql()` | 由 RouterTokenModel::set_ip_whitelist() 直接调用 | CALL / runtime-specific |
| 9 | `crates/database/src/database.rs` | `Database::fetch_all(), Database::kind(), Database::query(), DatabaseConnection::pool()` | 由 Database::query() 直接调用；由 RouterTokenModel::set_ip_whitelist() 直接调用 | CALL / runtime-specific |
