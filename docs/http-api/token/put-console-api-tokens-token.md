---
title: "PUT /console/api/tokens/{token}"
slug: /http-api/token/put-console-api-tokens-token
hide_table_of_contents: true
---

# PUT /console/api/tokens/&#123;token&#125;

**树路径：** `BurnCloud → HTTP / API → Token → PUT /console/api/tokens/{token}`

> **中文解释：** 更新 token status。 核心调用：TokenService::update_status。
>
...
```http
PUT /console/api/tokens/bc_live_7d4e...example HTTP/1.1
...
| 4 | `crates/server/src/api/token.rs` | `update_token()` | Token Handler / request validation / response mapping | READ/WRITE token request |
...
| 7 | `crates/server/src/api/response.rs` | `err(), ok()` | 由 auth_middleware() 直接调用；由 update_token() 直接调用 | CALL / runtime-specific |
| 8 | `crates/database/src/placeholder.rs` | `adapt_sql()` | 由 RouterTokenModel::update_status() 直接调用 | CALL / runtime-specific |
| 9 | `crates/database/src/database.rs` | `Database::fetch_all(), Database::kind(), Database::query(), DatabaseConnection::pool()` | 由 Database::query() 直接调用；由 RouterTokenModel::update_status() 直接调用 | CALL / runtime-specific |
