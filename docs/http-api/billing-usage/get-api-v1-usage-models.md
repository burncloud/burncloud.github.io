...
│    │    │    └─ CALL → get_usage_stats_by_model() @ crates/database/crates/router/src/log.rs
...
| 6 | `crates/database/src/database.rs` | `Database::fetch_all(), Database::fetch_optional(), Database::kind(), DatabaseConnection::pool()` | 由 RouterDatabase::validate_token_and_get_info() 直接调用；由 get_usage_stats_by_model() 直接调用 | CALL / runtime-specific |
| 7 | `crates/database/src/placeholder.rs` | `ph()` | 由 get_usage_stats_by_model() 直接调用 | CALL / runtime-specific |
