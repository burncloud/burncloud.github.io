---
title: "GET /console/api/monitor/security"
slug: /http-api/monitoring-security/get-console-api-monitor-security
hide_table_of_contents: true
---

# GET /console/api/monitor/security

**树路径：** `BurnCloud → HTTP / API → Monitoring / Security → GET /console/api/monitor/security`

> **中文解释：** 读取最近 router_logs，按 4xx/5xx 比例计算 score、blocked_count、threat sources 和 7 日 sparkline。 核心调用：RouterLogService::get。
>
...
```http
GET /console/api/monitor/security HTTP/1.1
...
| 4 | `crates/server/src/api/security.rs` | `security_summary(), compute_security_score(), compute_sparkline()` | RouterLog → SecuritySummary 派生 | READ logs |
...
| 7 | `crates/server/src/api/response.rs` | `err()` | 由 security_summary() 直接调用 | CALL / runtime-specific |
