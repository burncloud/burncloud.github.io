---
title: "burncloud update --check-only"
slug: /cli/burncloud/update/check-only
hide_table_of_contents: true
---

# burncloud update --check-only

**树路径：** `BurnCloud → CLI / Executables → burncloud → burncloud update --check-only`

> **中文解释：** Clap 解析到 update 分支，再进入 src/main.rs 中对应命令实现；参数校验失败时由 Clap/命令逻辑提前结束。
>
...
│
├─ Shell input
│    ├─ command: burncloud update --check-only
