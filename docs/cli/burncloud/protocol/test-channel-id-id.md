---
title: "burncloud protocol test --channel-id <id>"
slug: /cli/burncloud/protocol/test-channel-id-id
hide_table_of_contents: true
---

# burncloud protocol test --channel-id &lt;id&gt;

**树路径：** `BurnCloud → CLI / Executables → burncloud → burncloud protocol test --channel-id <id>`
...
│
├─ Shell input
│    ├─ command: burncloud protocol test --channel-id <id>
...
| 3 | `src/cli/protocol.rs` | `见上方 E2E 对应函数/入口` | 该 CLI/UI/Background/Startup 页面真实执行文件 | runtime-specific |
| 4 | `crates/database/crates/channel/src/channel_protocol_config.rs` | `ChannelProtocolConfigModel::*` | protocol config persistence | READ/WRITE channel protocol configs |
