---
title: "burncloud user check-username"
slug: /cli/burncloud/user/check-username
hide_table_of_contents: true
---

# burncloud user check-username

**树路径：** `BurnCloud → CLI / Executables → burncloud → burncloud user check-username`
...
| 3 | `src/cli/user.rs` | `见上方 E2E 对应函数/入口` | 该 CLI/UI/Background/Startup 页面真实执行文件 | runtime-specific |
| 4 | `crates/service/crates/user/src/lib.rs` | `UserService::*` | CLI user business logic | SERVICE |
| 5 | `crates/database/crates/user/src/lib.rs` | `UserDatabase::*` | user/role/recharge persistence | READ/WRITE users |
