...
| 2 | `crates/server/src/logging.rs` | `file_appender(), init_logging(), module_filter()` | 由 init_logging() 直接调用；由 main() 直接调用 | CALL / runtime-specific |
| 3 | `src/cli/commands.rs` | `handle_command(), show_help()` | 由 main() 直接调用；由 run_async_cli() 直接调用 | CALL / runtime-specific |
