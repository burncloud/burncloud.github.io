---
title: "PR #473 — cleanup(cache): remove unused token cache API"
slug: /pr/pr-473
---

# PR #473 — cleanup(cache): remove unused token cache API

> **状态：** MERGED  
> **作者：** `rustburn`  
> **创建：** `2026-09-14T02:42:15Z`  
> **Base：** `main@407f67d27191`  
> **Head：** `cleanup/remove-unused-token-cache-472@a31c650bc8c8`  
...
- **Changed Files：** `2`
- **Additions / Deletions：** `+10 / -52`
- **受影响的完整 E2E Flow：** `8`
- **未映射 changed files：** `1`

**PR 说明摘要：** ## Summary Closes #472. Remove the unused Token business cache surface from `burncloud-service-cache`: - `CachedToken` and its public re-export; - Token get/set/invalidate methods; - `bc:token:` key constant; - `CacheConfig.token_ttl` and its default; - obsolete Token cache documentation; - the disabled-cache test's dependency on `get_token`. The disabled-cache test now verifies the retained Platform behavior through availability and cache statistics. ## Boundary This PR deletes an invalid cross-domain projection. It does not create a replacement Identity cache and does not move quota balance or traffic class into Platform. Changed only: - `crates/service/crates/cache/src/service.rs` - `crates/service/crates/cache/src/lib.rs` Unchanged: - Price and Quota cache APIs; - Cache stats and clear behavior; - Redis connection, error, logging, and remaining keys; - authentication, database Token logic, Server, Client, and HTTP contracts; - dependencies and directory layout. ## Verification Completed: - reconfirmed no production consumer uses this Cache Token API; - `git diff --check main...HEAD`; - scoped diff review; - verified the Cache crate contains no `CachedToken`, `token_ttl`, `bc:token:`, or CacheService Token cache method; - confirmed the unrelated password-reset `get_token` remains unchanged. Required before merge in a Rust-enabled environment: The current execution environmen…

## Changed Files

| 文件 | 状态 | + | - | Atlas 命中 |
|---|---:|---:|---:|---:|
| `crates/service/crates/cache/src/lib.rs` | modified | 3 | 7 | 0 |
| `crates/service/crates/cache/src/service.rs` | modified | 7 | 45 | 8 |
