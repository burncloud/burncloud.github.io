---
title: "PR #477 — cleanup(cache): remove unused price cache API"
slug: /pr/pr-477
---

# PR #477 — cleanup(cache): remove unused price cache API

> **状态：** MERGED  
> **作者：** `rustburn`  
> **创建：** `2026-09-14T05:16:04Z`  
> **Base：** `main@87efe4999817`  
> **Head：** `cleanup/remove-unused-price-cache-476@fdc0732f80f3`  
...
- **Changed Files：** `2`
- **Additions / Deletions：** `+1 / -87`
- **受影响的完整 E2E Flow：** `8`
- **未映射 changed files：** `1`

**PR 说明摘要：** ## Summary Closes #476. Remove the unused Commerce / Pricing cache surface and the private implementation that only supported it: - Price get/set/invalidate API; - `bc:price:` key and the now-empty key module; - `CacheConfig.price_ttl`; - private generic `get/set/delete`; - `DeserializeOwned` import; - the no-longer-read `CacheService.config` field; - obsolete Price cache documentation. ## Boundary This is deletion only. It does not create a Pricing cache and does not move price truth or JSON formats into Platform. Changed only: - `crates/service/crates/cache/src/service.rs` - `crates/service/crates/cache/src/lib.rs` Unchanged: - `CacheService::new/is_available/stats/clear_all`; - Redis connection and safe error behavior; - Cache HTTP paths, authentication, response envelope, and stats fields; - Pricing, Billing, database, Server, Client, dependencies, and directories. ## Verification Completed: - reconfirmed no production consumer uses the Price cache API; - `git diff --check main...HEAD`; - scoped diff review: 87 deletions, one import replacement; - verified the Cache crate contains no Price API, `price_ttl`, `bc:price:`, generic cache helpers, `DeserializeOwned`, or stored `CacheService.config`. Required before merge in a Rust-enabled environment: The current execution environment does not contain a Rust toolchain. Cargo dependency and dead error-variant cleanup remain a sep…

## Changed Files

| 文件 | 状态 | + | - | Atlas 命中 |
|---|---:|---:|---:|---:|
| `crates/service/crates/cache/src/lib.rs` | modified | 0 | 3 | 0 |
| `crates/service/crates/cache/src/service.rs` | modified | 1 | 84 | 8 |
...
@@ -24,23 +22,15 @@ impl Default for CacheConfig {
@@ -57,7 +47,6 @@ impl CacheService {
@@ -88,7 +77,6 @@ impl CacheService {
@@ -116,77 +104,6 @@ impl CacheService {
```
