---
title: "PR #471 — cleanup(cache): remove unused channel cache API"
slug: /pr/pr-471
---

# PR #471 — cleanup(cache): remove unused channel cache API

> **状态：** MERGED  
> **作者：** `rustburn`  
> **创建：** `2026-09-14T02:30:35Z`  
> **Base：** `main@a80321d771cd`  
> **Head：** `cleanup/remove-unused-channel-cache-470@9f720fe6bbe3`  
...
- **Changed Files：** `2`
- **Additions / Deletions：** `+1 / -63`
- **受影响的完整 E2E Flow：** `8`
- **未映射 changed files：** `1`

**PR 说明摘要：** ## Summary Closes #470. Remove the unused Channel business cache surface from `burncloud-service-cache`: - `CachedChannel` and its public re-export; - Channel get/set/list/invalidate methods; - `bc:channel:` and `bc:channels:all` key constants; - `CacheConfig.channel_ttl` and its default; - obsolete Channel cache documentation. ## Boundary This is deletion only. It does not create a replacement cache and does not move Supply data, Traffic weight, or Provider API keys into Platform. Changed only: - `crates/service/crates/cache/src/service.rs` - `crates/service/crates/cache/src/lib.rs` Unchanged: - Token, Price, and Quota cache APIs; - Cache stats and clear behavior; - Redis connection, error, logging, key/value behavior; - Server and Client; - HTTP paths, authentication, and response envelopes; - dependencies and directory layout. ## Verification Completed: - reconfirmed there are no production consumers of the removed Cache Channel API; - `git diff --check main...HEAD`; - scoped diff review: 63 deletions, one re-export replacement; - verified the Cache crate no longer contains `CachedChannel`, Channel cache methods, Channel key constants, or `channel_ttl`. Required before merge in a Rust-enabled environment: The current execution environment does not contain a Rust toolchain.
...
| `crates/service/crates/cache/src/service.rs` | modified | 0 | 61 | 8 |
