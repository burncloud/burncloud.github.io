---
title: "PR #493 — security(jwt): require one injected secret source"
slug: /pr/pr-493
---

# PR #493 — security(jwt): require one injected secret source

> **状态：** MERGED  
> **作者：** `rustburn`  
> **创建：** `2026-09-14T12:59:22Z`  
> **Base：** `main@d8fe1723c443`  
> **Head：** `security/jwt-single-source-492@ac537fc61088`  
> **完整 E2E 拓扑基线：** `burncloud/burncloud@aa54e21393c6d46a6b09555ffd3661c1f22484f3`  
> **证据规则：** PR changed files / patch 为 `STATIC CONFIRMED`；下方完整 E2E 使用当前已审计 Atlas 拓扑，并在命中 PR changed file 的节点标记 `[PR CHANGED]`。旧 PR 的历史 head 可能与当前拓扑存在后续演化，因此未把当前拓扑伪装成历史源码逐行复现。

## PR 影响总览

- **Change Class：** `RUNTIME`
- **Changed Files：** `14`
- **Additions / Deletions：** `+564 / -314`
- **受影响的完整 E2E Flow：** `108`
- **未映射 changed files：** `8`

**PR 说明摘要：** Closes #492 ## 结果 - 新增由 Identity 拥有的 JwtSecret，构造时拒绝缺失、空值和纯空白值。 - JwtSecret 的 Debug 固定脱敏，不实现 Display 或 Serialize。 - JWT_SECRET 只在 start_server 的 composition root 读取一次。 - 同一个 JwtSecret 显式注入 UserService、Server middleware 和 Router fallback。 - 删除 common 中的 DEFAULT_JWT_SECRET 与 jwt_secret()。 - 删除 Router handler 中两处环境读取和固定弱默认值。 - 测试改为显式注入测试密钥，避免依赖 JWT_SECRET 全局环境。 - 更新 .env.example，明确 JWT_SECRET 启动必填。 ## 保持不变 - JWT claims、算法、过期规则； - Authorization: Bearer 格式； - 数据库 Token 优先、JWT fallback 顺序； - HTTP 路径和响应格式； - BURNCLOUD_INTERNAL_SECRET、MASTER_KEY、OAuth 与数据库 Schema。 ## 验证 已完成： - git diff --check - 静态搜索确认生产源码仅 start_server 一处读取 JWT_SECRET - 静态搜索确认固定弱默认值、DEFAULT_JWT_SECRET 和 common::jwt_secret 已清除 - 树对象与本地提交树一致：94af04b274d151f0f12586cf42bbdadb198db794 本地环境未安装 cargo/rustfmt，因此编译、格式化、Clippy 和测试交由 GitHub Actions 验证。若 CI 失败，仅在 #492 范围内修正。

## Changed Files

| 文件 | 状态 | + | - | Atlas 命中 |
|---|---:|---:|---:|---:|
| `.env.example` | modified | 3 | 3 | 0 |
| `crates/common/src/constants.rs` | modified | 0 | 8 | 19 |
| `crates/router/src/lib.rs` | modified | 349 | 189 | 44 |
| `crates/router/src/state.rs` | modified | 4 | 2 | 0 |
| `crates/router/tests/common.rs` | modified | 6 | 3 | 0 |
| `crates/server/src/api/auth.rs` | modified | 21 | 24 | 44 |
| `crates/server/src/api/mod.rs` | modified | 5 | 8 | 49 |
| `crates/server/src/lib.rs` | modified | 20 | 6 | 92 |
| `crates/server/tests/api_tests.rs` | modified | 19 | 7 | 0 |
| `crates/server/tests/log_api_tests.rs` | modified | 6 | 2 | 0 |
| `crates/server/tests/security_invariants.rs` | modified | 37 | 28 | 0 |
| `crates/server/tests/test_utils.rs` | modified | 6 | 0 | 0 |
| `crates/service/crates/user/examples/usage.rs` | modified | 4 | 2 | 0 |
| `crates/service/crates/user/src/lib.rs` | modified | 84 | 32 | 39 |

## Affected E2E Impact Matrix

| # | 完整 E2E / Entry Point | 类型 | PR 命中的源码文件 |
|---:|---|---|---|
| 1 | `GET /v1/models` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 2 | `GET /api/v1/usage` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 3 | `GET /api/v1/usage/models` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 4 | `POST /v1/chat/completions` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 5 | `POST /chat/completions` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 6 | `POST /v1/completions` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 7 | `POST /v1/embeddings` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 8 | `POST /v1/messages` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 9 | `POST /v1/video/generations` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 10 | `POST /v1beta/models/{model}:generateContent` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 11 | `POST /v1beta/models/{model}:streamGenerateContent` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 12 | `POST /v1beta/models/{model}:countTokens` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 13 | `POST /v1beta/models/{model}:embedContent` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 14 | `POST /v1/models/{model}:generateContent` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 15 | `POST /v1/models/{model}:streamGenerateContent` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 16 | `POST /v1/models/{model}:countTokens` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 17 | `POST /v1/models/{model}:embedContent` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 18 | `GET /v1/videos/{task_id}` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 19 | `Router fallback → proxy_handler` | HTTP / API / AI API / Data Plane | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 20 | `POST /api/auth/register` | HTTP / API / Authentication | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 21 | `POST /api/auth/login` | HTTP / API / Authentication | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 22 | `POST /api/auth/forgot-password` | HTTP / API / Authentication | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 23 | `POST /api/auth/reset-password` | HTTP / API / Authentication | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 24 | `GET /api/auth/google` | HTTP / API / Authentication | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 25 | `GET /api/auth/github` | HTTP / API / Authentication | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs` |
| 26 | `GET /console/api/channel` | HTTP / API / Channel Management | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 27 | `POST /console/api/channel` | HTTP / API / Channel Management | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 28 | `PUT /console/api/channel` | HTTP / API / Channel Management | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 29 | `GET /console/api/channel/{id}` | HTTP / API / Channel Management | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 30 | `DELETE /console/api/channel/{id}` | HTTP / API / Channel Management | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 31 | `GET /console/api/tokens` | HTTP / API / Token | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 32 | `POST /console/api/tokens` | HTTP / API / Token | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 33 | `GET /console/api/tokens/{token}` | HTTP / API / Token | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 34 | `PUT /console/api/tokens/{token}` | HTTP / API / Token | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 35 | `DELETE /console/api/tokens/{token}` | HTTP / API / Token | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 36 | `POST /console/api/tokens/{token}/rotate` | HTTP / API / Token | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 37 | `POST /console/api/tokens/{token}/revoke-old` | HTTP / API / Token | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 38 | `POST /console/api/tokens/{token}/ip-whitelist` | HTTP / API / Token | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 39 | `POST /console/api/user/register` | HTTP / API / User | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 40 | `POST /console/api/user/login` | HTTP / API / User | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 41 | `POST /console/api/user/topup` | HTTP / API / User | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 42 | `GET /console/api/user/check_username` | HTTP / API / User | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 43 | `GET /console/api/user/recharges` | HTTP / API / User | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 44 | `GET /console/api/list_users` | HTTP / API / User | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/common/src/constants.rs` |
| 45 | `GET /api/billing/summary` | HTTP / API / Billing / Usage | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 46 | `GET /console/api/logs` | HTTP / API / Logs | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 47 | `GET /console/api/usage/{user_id}` | HTTP / API / Logs | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 48 | `GET /console/internal/billing/summary` | HTTP / API / Billing / Usage | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 49 | `GET /console/api/monitor` | HTTP / API / Monitoring / Security | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 50 | `GET /console/api/monitor/security` | HTTP / API / Monitoring / Security | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 51 | `GET /console/api/monitor/security/events` | HTTP / API / Monitoring / Security | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 52 | `GET /console/api/monitor/security/filters` | HTTP / API / Monitoring / Security | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 53 | `PUT /console/api/monitor/security/filters` | HTTP / API / Monitoring / Security | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 54 | `POST /console/api/monitor/security/emergency-circuit-break` | HTTP / API / Monitoring / Security | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 55 | `GET /console/api/monitor/security/circuit-breaker-status` | HTTP / API / Monitoring / Security | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 56 | `GET /console/api/cache/stats` | HTTP / API / Cache | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 57 | `POST /console/api/cache/clear` | HTTP / API / Cache | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 58 | `GET /api/v1/usage` | HTTP / API / Billing / Usage | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 59 | `GET /api/v1/usage/models` | HTTP / API / Billing / Usage | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 60 | `GET /console/api/usage/{user_id}` | HTTP / API / Billing / Usage | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs` |
| 61 | `GET /health` | HTTP / API / Admin / Internal | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 62 | `GET /console/internal/health` | HTTP / API / Admin / Internal | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 63 | `POST /console/internal/prices/sync` | HTTP / API / Admin / Internal | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 64 | `POST /console/internal/circuit-breaker/trip-all` | HTTP / API / Admin / Internal | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 65 | `GET /console/internal/metrics` | HTTP / API / Admin / Internal | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs` |
| 66 | `GET /console/api/{*path} → protected 404 catch-all` | HTTP / API / Admin / Internal | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs` |
| 67 | `GET /api-docs/openapi.json` | HTTP / API / OpenAPI / Swagger | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 68 | `GET /swagger-ui` | HTTP / API / OpenAPI / Swagger | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 69 | `GET /swagger-ui/` | HTTP / API / OpenAPI / Swagger | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 70 | `GET /` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 71 | `GET /home` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 72 | `GET /login` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 73 | `GET /register` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 74 | `GET /forgot-password` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 75 | `GET /reset-password` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 76 | `GET /console` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 77 | `GET /console/` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 78 | `GET /console/{*path}` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 79 | `GET /favicon.ico` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 80 | `GET /preview/home` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 81 | `GET /preview/login` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 82 | `GET /preview/console` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 83 | `GET /preview/console/` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 84 | `GET /preview/console/{*path}` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 85 | `GET /ws` | HTTP / API / Web UI / LiveView / WebSocket | `crates/server/src/lib.rs` |
| 86 | `burncloud` | CLI / Executables / burncloud | `crates/server/src/lib.rs` |
| 87 | `server` | CLI / Executables / burncloud | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 88 | `router` | CLI / Executables / burncloud | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 89 | `burncloud user register` | CLI / Executables / burncloud | `crates/service/crates/user/src/lib.rs` |
| 90 | `burncloud user login` | CLI / Executables / burncloud | `crates/service/crates/user/src/lib.rs` |
| 91 | `burncloud user list` | CLI / Executables / burncloud | `crates/service/crates/user/src/lib.rs` |
| 92 | `burncloud user topup` | CLI / Executables / burncloud | `crates/service/crates/user/src/lib.rs` |
| 93 | `burncloud user recharges` | CLI / Executables / burncloud | `crates/service/crates/user/src/lib.rs` |
| 94 | `burncloud user check-username` | CLI / Executables / burncloud | `crates/service/crates/user/src/lib.rs` |
| 95 | `System Monitor Auto Update` | Background Jobs / Async Side Effects / Long-running Jobs | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs` |
| 96 | `Price Sync` | Background Jobs / Async Side Effects / Long-running Jobs | `crates/router/src/lib.rs` |
| 97 | `Exchange Rate Sync` | Background Jobs / Async Side Effects / Long-running Jobs | `crates/router/src/lib.rs` |
| 98 | `AIMD Budget Feedback` | Background Jobs / Async Side Effects / Long-running Jobs | `crates/router/src/lib.rs` |
| 99 | `Async Router Log Writer` | Background Jobs / Async Side Effects / Long-running Jobs | `crates/router/src/lib.rs` |
| 100 | `Async Request Log Writer` | Background Jobs / Async Side Effects / Long-running Jobs | `crates/router/src/lib.rs` |
| 101 | `Token accessed_time update` | Background Jobs / Async Side Effects / Request-time Async Side Effects | `crates/router/src/lib.rs` |
| 102 | `Quota deduction` | Background Jobs / Async Side Effects / Request-time Async Side Effects | `crates/router/src/lib.rs` |
| 103 | `Video task mapping save` | Background Jobs / Async Side Effects / Request-time Async Side Effects | `crates/router/src/lib.rs` |
| 104 | `API version detect / update` | Background Jobs / Async Side Effects / Request-time Async Side Effects | `crates/router/src/lib.rs` |
| 105 | `start_server` | Startup / Startup Chain | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs` |
| 106 | `create_app` | Startup / Startup Chain | `crates/server/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/server/src/api/auth.rs` |
| 107 | `create_router_app` | Startup / Startup Chain | `crates/router/src/lib.rs` |
| 108 | `Windows Background Server Thread` | Background Jobs / Async Side Effects / Desktop Background Work | `crates/server/src/lib.rs`&lt;br/&gt;`crates/router/src/lib.rs`&lt;br/&gt;`crates/service/crates/user/src/lib.rs`&lt;br/&gt;`crates/server/src/api/mod.rs` |

## 完整受影响 E2E Request Flow

### E2E FLOW #1 — GET /v1/models

**树路径：** `HTTP / API → AI API / Data Plane → GET /v1/models`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /v1/models
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（其它顶层 route）→ 进入对应 handler；本页路径结束
│         └─ NO（/v1/models）→ fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route match
│    ├─ create_router_app() 已注册显式 GET /v1/models
│    └─ DECISION: Method == GET AND Path == /v1/models ?
│         ├─ NO  → 其它显式 usage route 或 proxy_handler fallback
│         └─ YES → models_handler(State<AppState>)
│
├─ Authentication / authorization boundary
│    ├─ 当前 handler 不读取 Authorization
│    ├─ 不调用 Token/JWT validation
│    ├─ 不解析 user_id / group
│    └─ 因此没有当前用户维度的模型可见性过滤
│
├─ Handler local state
│    ├─ model_entries = []
│    ├─ SystemTime::now()
│    ├─ duration_since(UNIX_EPOCH)
│    └─ DECISION: system time conversion OK?
│         ├─ YES → current_time = duration.as_secs()
│         └─ NO  → unwrap_or_default() → current_time = 0
│
├─ CALL → ChannelAbilityModel::list_distinct_models(&state.db)
│
▼
FILE: crates/database/crates/channel/src/channel_ability.rs
│
├─ Database connection
│    ├─ db.get_connection()
│    └─ DECISION: connection acquired?
│         ├─ NO  → return Err → 回到 handler
│         └─ YES → conn.pool()
│
├─ SQL / state read
│    ├─ SELECT DISTINCT model
│    ├─ FROM channel_abilities
│    ├─ WHERE enabled = 1
│    └─ ORDER BY model
│
├─ sqlx::query_as(sql).fetch_all(pool).await
│    └─ DECISION: SQL success?
│         ├─ NO  → return Err
│         └─ YES → Vec<(String,)> → map → Ok(Vec<String>)
│
├─ Visibility semantics
│    ├─ INCLUDE only ability.enabled = 1
│    ├─ DISTINCT by model
│    ├─ NO user/group filter
│    ├─ NO channel_providers.status join
│    ├─ NO health/circuit/capacity check
│    └─ NO quota/price/billing check
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Handler branch merge
│    └─ DECISION: list_distinct_models returned Ok?
│         ├─ NO / Err
│         │    ├─ if let Ok(...) body skipped
│         │    └─ model_entries stays []
│         └─ YES
│              └─ FOR EACH model
│                   ├─ id = model
│                   ├─ object = "model"
│                   ├─ created = current_time
│                   ├─ owned_by = "burncloud"
│                   ├─ permission = []
│                   ├─ root = model
│                   ├─ parent = null
│                   └─ push → model_entries
│
├─ Serialization
│    ├─ response_json = {object:"list", data:model_entries}
│    ├─ serde_json::to_string(...)
│    └─ DECISION: serialization success?
│         ├─ YES → normal JSON body
│         └─ NO  → literal fallback {"object":"list","data":[]}
│
├─ HTTP response construction
│    ├─ build_response_with_header(StatusCode::OK, content-type, application/json, body)
│    ├─ Response::builder().status(200).header(...).body(...)
│    └─ DECISION: response builder success?
│         ├─ YES → HTTP 200 + JSON body
│         └─ NO  → retry status 200 + empty body
│              └─ DECISION: retry success?
│                   ├─ YES → HTTP 200 + empty body
│                   └─ NO  → Response::new(Body::empty())
│
├─ Explicitly NOT executed
│    ├─ proxy_handler
│    ├─ Token/JWT auth
│    ├─ Quota / rate limiter
│    ├─ ModelRouter / Scheduler
│    ├─ Circuit Breaker
│    ├─ Billing
│    └─ Provider / upstream
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ models_handler()
│    │    │    └─ CALL → ChannelAbilityModel::list_distinct_models() @ crates/database/crates/channel/src/channel_ability.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ configure_rate_budget_from_db()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    │    ├─ PriceSyncService::with_config()
│    │    ├─ PriceSyncService::new()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    ├─ FILE: crates/database/crates/channel/src/channel_ability.rs
│    │    ├─ ChannelAbilityModel::list_distinct_models()
│    └─ FILE: crates/database/crates/router/src/log.rs
│    │    ├─ RouterLogModel::insert()
│    │    ├─ RouterRequestLogModel::insert()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives model-list response
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "object": "list",
  "data": [
    {
      "id": "gpt-5.4",
      "object": "model",
      "created": 1786380000,
      "owned_by": "burncloud",
      "permission": [],
      "root": "gpt-5.4",
      "parent": null
    },
    {
      "id": "claude-sonnet-4-5",
      "object": "model",
      "created": 1786380000,
      "owned_by": "burncloud",
      "permission": [],
      "root": "claude-sonnet-4-5",
      "parent": null
    }
  ]
}
```

### E2E FLOW #2 — GET /api/v1/usage

**树路径：** `HTTP / API → AI API / Data Plane → GET /api/v1/usage`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /api/v1/usage
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（其它顶层 route）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Explicit Data Plane route
│    ├─ Route match: GET /api/v1/usage
│    └─ handler = usage_handler()
│
├─ Credential extraction
│    ├─ read Authorization header
│    ├─ require Bearer token
│    └─ DECISION: Bearer credential present?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → extract_token_user(...)
│
├─ Multi-generation identity resolution
│    ├─ Try new Router token table
│    │    └─ validate_token_and_get_info(...)
│    ├─ DECISION: new token valid?
│    │    ├─ YES → resolve user_id
│    │    └─ NO  → legacy validation
│    ├─ validate_token_detailed(...)
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → resolve user_id
│    │    └─ NO  → JWT fallback
│    ├─ JWT decode / Claims.sub
│    └─ DECISION: any identity path resolved?
│         ├─ NO  → 401 / service-unavailable error branch → END
│         └─ YES → user_id
│
▼
FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│
├─ Usage aggregation query
│    ├─ CALL get_usage_stats(user_id, "month")
│    ├─ period = month
│    ├─ scope = resolved user_id
│    └─ DECISION: DB aggregation success?
│         ├─ NO  → HTTP 500 → END
│         └─ YES → usage rows / aggregate
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mapping
│    ├─ map DB aggregate → API response object
│    ├─ serialize JSON
│    └─ DECISION: serialization/build success?
│         ├─ NO  → internal response error branch
│         └─ YES → HTTP 200 application/json
│
├─ Side effects
│    ├─ no Provider call
│    ├─ no Scheduler
│    ├─ no inference billing deduction
│    └─ read-only usage query
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ usage_handler()
│    │    │    └─ CALL → extract_token_user() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → json_error_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ extract_token_user()
│    │    │    └─ CALL → build_response() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│    │    ├─ build_response()
│    │    ├─ json_error_body()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    └─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::validate_detailed()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives 当前用户月度总 usage
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "period": "month",
  "requests": 12842,
  "prompt_tokens": 18420560,
  "completion_tokens": 6912840,
  "total_tokens": 25333400,
  "cost": 126.67
}
```

### E2E FLOW #3 — GET /api/v1/usage/models

**树路径：** `HTTP / API → AI API / Data Plane → GET /api/v1/usage/models`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /api/v1/usage/models
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（其它顶层 route）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Explicit Data Plane route
│    ├─ Route match: GET /api/v1/usage/models
│    └─ handler = usage_models_handler()
│
├─ Credential extraction
│    ├─ read Authorization header
│    ├─ require Bearer token
│    └─ DECISION: Bearer credential present?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → extract_token_user(...)
│
├─ Multi-generation identity resolution
│    ├─ Try new Router token table
│    │    └─ validate_token_and_get_info(...)
│    ├─ DECISION: new token valid?
│    │    ├─ YES → resolve user_id
│    │    └─ NO  → legacy validation
│    ├─ validate_token_detailed(...)
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → resolve user_id
│    │    └─ NO  → JWT fallback
│    ├─ JWT decode / Claims.sub
│    └─ DECISION: any identity path resolved?
│         ├─ NO  → 401 / service-unavailable error branch → END
│         └─ YES → user_id
│
▼
FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│
├─ Usage aggregation query
│    ├─ CALL get_usage_stats_by_model(user_id, "month")
│    ├─ period = month
│    ├─ scope = resolved user_id
│    └─ DECISION: DB aggregation success?
│         ├─ NO  → HTTP 500 → END
│         └─ YES → usage rows / aggregate
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mapping
│    ├─ map DB aggregate → API response object
│    ├─ serialize JSON
│    └─ DECISION: serialization/build success?
│         ├─ NO  → internal response error branch
│         └─ YES → HTTP 200 application/json
│
├─ Side effects
│    ├─ no Provider call
│    ├─ no Scheduler
│    ├─ no inference billing deduction
│    └─ read-only usage query
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ usage_models_handler()
│    │    │    └─ CALL → extract_token_user() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → json_error_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ extract_token_user()
│    │    │    └─ CALL → build_response() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│    │    ├─ build_response()
│    │    ├─ json_error_body()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    └─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::validate_detailed()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives 按模型聚合的月度 usage
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "period": "month",
  "models": [
    {
      "model": "gpt-5.4",
      "requests": 8021,
      "total_tokens": 16600420,
      "cost": 81.24
    },
    {
      "model": "claude-sonnet-4-5",
      "requests": 4821,
      "total_tokens": 8732980,
      "cost": 45.43
    }
  ]
}
```

### E2E FLOW #4 — POST /v1/chat/completions

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/chat/completions`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/chat/completions
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ 无额外 endpoint-specific branch
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "id": "chatcmpl_bc_01JXYZ",
  "object": "chat.completion",
  "created": 1786380000,
  "model": "gpt-5.4",
  "choices": [
    {
      "index": 0,
      "message": {
        "role": "assistant",
        "content": "你好，我已经收到你的请求。"
      },
      "finish_reason": "stop"
    }
  ],
  "usage": {
    "prompt_tokens": 18,
    "completion_tokens": 14,
    "total_tokens": 32
  }
}
```

### E2E FLOW #5 — POST /chat/completions

**树路径：** `HTTP / API → AI API / Data Plane → POST /chat/completions`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /chat/completions
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ 无额外 endpoint-specific branch
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "id": "chatcmpl_bc_01JXYZ",
  "object": "chat.completion",
  "created": 1786380000,
  "model": "gpt-5.4",
  "choices": [
    {
      "index": 0,
      "message": {
        "role": "assistant",
        "content": "你好，我已经收到你的请求。"
      },
      "finish_reason": "stop"
    }
  ],
  "usage": {
    "prompt_tokens": 18,
    "completion_tokens": 14,
    "total_tokens": 32
  }
}
```

### E2E FLOW #6 — POST /v1/completions

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/completions`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/completions
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ 无额外 endpoint-specific branch
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "id": "cmpl_bc_01JXYZ",
  "object": "text_completion",
  "created": 1786380000,
  "model": "gpt-5.4",
  "choices": [
    {
      "text": "BurnCloud 已完成本次补全。",
      "index": 0,
      "finish_reason": "stop"
    }
  ],
  "usage": {
    "prompt_tokens": 12,
    "completion_tokens": 9,
    "total_tokens": 21
  }
}
```

### E2E FLOW #7 — POST /v1/embeddings

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/embeddings`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/embeddings
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ 无额外 endpoint-specific branch
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "object": "list",
  "data": [
    {
      "object": "embedding",
      "index": 0,
      "embedding": [
        0.0124,
        -0.0431,
        0.0088,
        0.0912
      ]
    }
  ],
  "model": "text-embedding-3-large",
  "usage": {
    "prompt_tokens": 8,
    "total_tokens": 8
  }
}
```

### E2E FLOW #8 — POST /v1/messages

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/messages`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/messages
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ 无额外 endpoint-specific branch
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "id": "msg_bc_01JXYZ",
  "type": "message",
  "role": "assistant",
  "model": "claude-sonnet-4-5",
  "content": [
    {
      "type": "text",
      "text": "这是通过 BurnCloud 转发后的 Anthropic 原生响应示例。"
    }
  ],
  "stop_reason": "end_turn",
  "usage": {
    "input_tokens": 22,
    "output_tokens": 18
  }
}
```

### E2E FLOW #9 — POST /v1/video/generations

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/video/generations`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/video/generations
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ 解析 duration/resolution；成功后异步保存 task_id → channel_id 映射
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ save video task mapping asynchronously
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 202 Accepted
Content-Type: application/json

{
  "id": "video_task_bc_01JXYZ",
  "object": "video.generation",
  "status": "queued",
  "model": "video-model-pro",
  "created": 1786380000
}
```

### E2E FLOW #10 — POST /v1beta/models/&#123;model&#125;:generateContent

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1beta/models/{model}:generateContent`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1beta/models/{model}:generateContent
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ model 可能从 Gemini URL path 提取，而不是 JSON body
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "candidates": [
    {
      "content": {
        "role": "model",
        "parts": [
          {
            "text": "这是 Gemini 原生 generateContent 返回示例。"
          }
        ]
      },
      "finishReason": "STOP",
      "index": 0
    }
  ],
  "usageMetadata": {
    "promptTokenCount": 16,
    "candidatesTokenCount": 13,
    "totalTokenCount": 29
  },
  "modelVersion": "gemini-example"
}
```

### E2E FLOW #11 — POST /v1beta/models/&#123;model&#125;:streamGenerateContent

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1beta/models/{model}:streamGenerateContent`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1beta/models/{model}:streamGenerateContent
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ model 可能从 Gemini URL path 提取，而不是 JSON body
│    ├─ 上游返回流式内容，进入 streaming usage/response path
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/event-stream

data: {"candidates":[{"content":{"role":"model","parts":[{"text":"这是"}]},"index":0}]}

data: {"candidates":[{"content":{"role":"model","parts":[{"text":"流式返回示例。"}]},"finishReason":"STOP","index":0}],"usageMetadata":{"totalTokenCount":27}}
```

### E2E FLOW #12 — POST /v1beta/models/&#123;model&#125;:countTokens

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1beta/models/{model}:countTokens`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1beta/models/{model}:countTokens
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ model 可能从 Gemini URL path 提取，而不是 JSON body
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "totalTokens": 42
}
```

### E2E FLOW #13 — POST /v1beta/models/&#123;model&#125;:embedContent

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1beta/models/{model}:embedContent`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1beta/models/{model}:embedContent
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ model 可能从 Gemini URL path 提取，而不是 JSON body
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "embedding": {
    "values": [
      0.0182,
      -0.0711,
      0.0043,
      0.1128
    ]
  }
}
```

### E2E FLOW #14 — POST /v1/models/&#123;model&#125;:generateContent

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/models/{model}:generateContent`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/models/{model}:generateContent
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ model 可能从 Gemini URL path 提取，而不是 JSON body
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "candidates": [
    {
      "content": {
        "role": "model",
        "parts": [
          {
            "text": "这是 Gemini 原生 generateContent 返回示例。"
          }
        ]
      },
      "finishReason": "STOP",
      "index": 0
    }
  ],
  "usageMetadata": {
    "promptTokenCount": 16,
    "candidatesTokenCount": 13,
    "totalTokenCount": 29
  },
  "modelVersion": "gemini-example"
}
```

### E2E FLOW #15 — POST /v1/models/&#123;model&#125;:streamGenerateContent

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/models/{model}:streamGenerateContent`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/models/{model}:streamGenerateContent
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ model 可能从 Gemini URL path 提取，而不是 JSON body
│    ├─ 上游返回流式内容，进入 streaming usage/response path
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/event-stream

data: {"candidates":[{"content":{"role":"model","parts":[{"text":"这是"}]},"index":0}]}

data: {"candidates":[{"content":{"role":"model","parts":[{"text":"流式返回示例。"}]},"finishReason":"STOP","index":0}],"usageMetadata":{"totalTokenCount":27}}
```

### E2E FLOW #16 — POST /v1/models/&#123;model&#125;:countTokens

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/models/{model}:countTokens`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/models/{model}:countTokens
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ model 可能从 Gemini URL path 提取，而不是 JSON body
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "totalTokens": 42
}
```

### E2E FLOW #17 — POST /v1/models/&#123;model&#125;:embedContent

**树路径：** `HTTP / API → AI API / Data Plane → POST /v1/models/{model}:embedContent`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /v1/models/{model}:embedContent
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ model 可能从 Gemini URL path 提取，而不是 JSON body
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "embedding": {
    "values": [
      0.0182,
      -0.0711,
      0.0043,
      0.1128
    ]
  }
}
```

### E2E FLOW #18 — GET /v1/videos/&#123;task_id&#125;

**树路径：** `HTTP / API → AI API / Data Plane → GET /v1/videos/{task_id}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /v1/videos/{task_id}
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（其它 route）→ corresponding handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ proxy_handler entry
│    ├─ normalize path
│    ├─ extract supported credential
│    ├─ validate token/user
│    ├─ quota guard
│    └─ local rate limiter
│
├─ Video polling special-case detection
│    └─ DECISION: Method == GET AND Path starts with /v1/videos/ ?
│         ├─ NO  → normal inference proxy_logic
│         └─ YES → polling branch
│
├─ Task identity
│    ├─ task_id = URL suffix
│    └─ DECISION: task_id non-empty/valid enough for lookup?
│         ├─ NO  → client/not-found error → END
│         └─ YES → DB lookup
│
▼
FILE: crates/database/crates/router/src/video_task.rs
│
├─ Read persisted task mapping
│    ├─ RouterVideoTaskModel::get_by_task_id(task_id)
│    └─ DECISION: mapping exists?
│         ├─ NO  → HTTP 404 task_not_found → END
│         └─ YES → channel_id + stored task metadata
│
▼
FILE: crates/database/crates/channel/src/lib.rs
│
├─ Load original Channel
│    ├─ ChannelProviderModel::get_by_id(channel_id)
│    └─ DECISION: Channel record available/configured?
│         ├─ NO  → HTTP 502 → END
│         └─ YES → base_url + credential
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Direct upstream polling request
│    ├─ construct {base_url}/v1/videos/{task_id}
│    ├─ apply original Channel authentication
│    ├─ IMPORTANT: no new scheduler/model selection
│    └─ send GET through shared HTTP client
│
├─ Upstream polling result
│    └─ DECISION: network/upstream request succeeds?
│         ├─ NO  → HTTP 502 → END
│         └─ YES
│              ├─ preserve upstream status
│              ├─ collect upstream body
│              └─ return polling response
│
├─ Side effects
│    ├─ task mapping is read-only in this request
│    ├─ no fresh task mapping created
│    └─ no model scheduler selection
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives current video task state
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "id": "video_task_bc_01JXYZ",
  "object": "video.generation",
  "status": "completed",
  "model": "video-model-pro",
  "output": [
    {
      "url": "https://example.invalid/videos/video_task_bc_01JXYZ.mp4"
    }
  ],
  "duration": 5,
  "resolution": "1080p"
}
```

### E2E FLOW #19 — Router fallback → proxy_handler

**树路径：** `HTTP / API → AI API / Data Plane → Router fallback → proxy_handler`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: 任意未被显式 Router 捕获的数据面路径
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（Management/Internal/LiveView 等）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Data Plane route selection
│    ├─ explicit /v1/models or usage routes checked first
│    └─ DECISION: explicit route matched?
│         ├─ YES → corresponding explicit handler
│         └─ NO  → proxy_handler()
│
├─ Path normalization + request identity
│    ├─ normalize_doubled_path()
│    ├─ preserve Method / URI / headers
│    ├─ request-id already attached by server middleware
│    └─ prepare AppState/runtime services
│
├─ Credential source selection
│    ├─ Candidate 1: Authorization: Bearer ...
│    ├─ Candidate 2: x-api-key
│    ├─ Candidate 3: x-goog-api-key
│    └─ DECISION: any supported credential exists?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → token validation chain
│
├─ Token / user / group resolution
│    ├─ RouterDatabase new-token validation
│    ├─ DECISION: new token valid?
│    │    ├─ YES → token metadata / user_id / group / quota / policy
│    │    └─ NO  → legacy token validation
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → legacy metadata / user_id
│    │    └─ NO  → JWT fallback where supported
│    └─ DECISION: identity ultimately valid?
│         ├─ NO  → authorization error → END
│         └─ YES → continue
│

▼
FILE: crates/database/crates/router/src/token.rs
│
├─ Router token persistence backing validation/quota metadata
├─ validate current/legacy token state as invoked from RouterDatabase
└─ return token/user/group/quota/order metadata
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Account / quota guard
│    ├─ inspect quota/order metadata
│    └─ DECISION: quota exhausted / account cannot spend?
│         ├─ YES → HTTP 402 Payment Required → END
│         └─ NO  → continue
│
├─ Local request rate limiting
│    ├─ limiter key derived from authenticated request context
│    └─ DECISION: limiter allows request?
│         ├─ NO  → HTTP 429 Too Many Requests → END
│         └─ YES → continue
│
├─ Request-body acquisition
│    ├─ collect Axum body bytes
│    └─ DECISION: body read/size/parse boundary succeeds?
│         ├─ NO  → client/request error → END
│         └─ YES → immutable request payload for routing/adaptor
│
├─ Model + protocol context extraction
│    ├─ OpenAI/Anthropic: model usually from JSON body
│    ├─ Gemini: model may come from URL path
│    ├─ detect streaming / batch / priority hints
│    ├─ detect API protocol family
│    ├─ 无额外 endpoint-specific branch
│    └─ DECISION: required model/context resolved?
│         ├─ NO  → invalid request / model resolution error → END
│         └─ YES → proxy_logic(...)
│
├─ Enter proxy_logic(...)
│    ├─ load scheduling policy for resolved user group
│    ├─ resolve requested model / model mapping
│    ├─ load candidate channel abilities
│    ├─ apply user/order/price constraints
│    └─ produce ordered/weighted candidate set
│

▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::resolve_traffic_class()
├─ TTL cache lookup
└─ cache miss → UserDatabase::get_user_roles()
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ read user roles used for TrafficColor classification
└─ return roles → UserService → proxy_logic
│
▼
FILE: crates/router/src/model_router.rs
│
├─ ModelRouter::route_with_scheduler()
├─ evaluate group/model/scheduler/request classification
├─ consult affinity / health / pricing inputs
└─ return ordered channel candidates（DYNAMIC result）
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate eligibility filtering
│    ├─ ability enabled?
│    ├─ channel state allows traffic?
│    ├─ circuit breaker allows attempt?
│    ├─ rate budget / shaper allows attempt?
│    ├─ price/order constraints satisfied?
│    └─ DECISION: any candidate remains?
│         ├─ NO  → no-upstream / routing error → END
│         └─ YES → candidate attempt loop
│

▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache supplies model/region pricing used by routing/billing
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::preflight()
└─ DECISION: strict pricing requirement satisfied?
     ├─ NO → reject before upstream
     └─ YES → return to candidate execution
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Candidate attempt loop
│    ├─ select next candidate
│    ├─ acquire rate budget / shaping permission
│    ├─ consult circuit breaker
│    ├─ resolve protocol / API version
│    ├─ construct upstream URL + headers + credentials
│    └─ hand request to passthrough/adaptor boundary
│
▼
FILE: crates/router/src/passthrough.rs
│
├─ Protocol transformation boundary
│    ├─ inspect source protocol + target Channel protocol
│    └─ DECISION: native passthrough possible?
│         ├─ YES
│         │    ├─ preserve compatible body/headers semantics
│         │    └─ avoid unnecessary transform
│         └─ NO
│              └─ DynamicAdaptorFactory / adaptor conversion path
│                   └─ DYNAMIC: exact transformation depends on Provider adaptor
│
├─ Upstream network I/O
│    ├─ send HTTP request through shared client
│    ├─ wait for headers/body or stream
│    └─ DECISION: upstream attempt succeeds?
│         ├─ NO
│         │    ├─ classify failure
│         │    ├─ update channel/circuit feedback
│         │    ├─ optional API-version detect/update
│         │    └─ DECISION: another eligible candidate?
│         │         ├─ YES → 返回 Candidate attempt loop
│         │         └─ NO  → final upstream/routing error → END
│         └─ YES → response handling
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Response mode split
│    └─ DECISION: streaming response?
│         ├─ YES
│         │    ├─ stream chunks to client
│         │    ├─ collect/derive usage while stream progresses
│         │    └─ handle stream completion/disconnect
│         └─ NO
│              ├─ collect upstream body
│              └─ parse/derive usage metadata
│

▼
FILE: crates/service/crates/billing/src/usage/*
│
├─ provider-specific UsageParser extracts/normalizes usage
└─ return UnifiedUsage
│
▼
FILE: crates/service/crates/billing/src/calculator.rs
│
├─ CostCalculator::calculate()
├─ PriceCache + RequestOptions + UnifiedUsage
└─ return cost breakdown
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ persistence backing RouterLog / RequestLog aggregation/writes
└─ async writers persist request accounting state
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Unified usage + cost
│    ├─ normalize prompt/input tokens
│    ├─ normalize completion/output tokens
│    ├─ endpoint-specific usage augmentation when needed
│    ├─ PriceCache / CostCalculator lookup
│    └─ CostCalculator::calculate()
│
├─ Accounting / persistence side effects
│    ├─ enqueue RouterLog
│    ├─ enqueue RequestLog according to storage policy
│    ├─ send AIMD/rate-budget feedback
│    ├─ update circuit/channel state feedback
│    ├─ async token accessed_time update where applicable
│    └─ DECISION: calculated cost > 0?
│         ├─ YES → async quota deduction
│         └─ NO  → no quota deduction task
│
├─ Endpoint-specific async side effects
│    ├─ none beyond common request side effects
│
├─ Final response construction
│    ├─ preserve/normalize upstream-compatible status + body
│    ├─ attach resolved channel/model diagnostic headers where configured
│    └─ return Axum Response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ normalize_doubled_path()
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ build_response_with_header()
│    │    ├─ inject_video_tokens_if_empty()
│    │    ├─ sanitize_request_body()
│    │    │    └─ CALL → redact_sensitive_fields() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → safe_cut() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ sanitize_request_headers()
│    │    ├─ record_failover_attempt()
│    │    ├─ apply_header_override()
│    │    ├─ record_upstream_failure()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::resolve_traffic_class()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::route_with_scheduler()
│    │    │    └─ CALL → pick_hrw() @ crates/router/src/affinity.rs
│    │    │    └─ CALL → CombinedScheduler::new() @ crates/router/src/scheduler/combined.rs
│    │    │    └─ CALL → build_context() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_candidates() @ crates/router/src/scheduler/mod.rs
│    │    │    └─ CALL → rank_passthrough() @ crates/router/src/scheduler/mod.rs
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::preflight()
│    │    ├─ CostCalculator::calculate()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::deduct_quota()
│    ├─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│    │    ├─ should_passthrough()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → is_gemini_native_content() @ crates/router/src/passthrough.rs
│    │    ├─ build_gemini_passthrough_url()
│    │    │    └─ CALL → is_gemini_native_path() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/crates/router/src/router_video_task.rs
│    │    ├─ RouterVideoTaskModel::get_by_task_id()
│    │    ├─ RouterVideoTaskModel::save()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/service/crates/billing/src/counter.rs
│    │    ├─ UnifiedTokenCounter::new()
│    ├─ FILE: crates/router/src/order_type.rs
│    │    ├─ OrderType::from_db_row()
│    └─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ BudgetGuard::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives successful upstream-compatible response OR a terminal error from an earlier branch
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "id": "chatcmpl_bc_fallback",
  "object": "chat.completion",
  "model": "resolved-upstream-model",
  "choices": [
    {
      "index": 0,
      "message": {
        "role": "assistant",
        "content": "该未显式注册路径被数据面 fallback 接管并成功转发。"
      },
      "finish_reason": "stop"
    }
  ]
}
```

### E2E FLOW #20 — POST /api/auth/register

**树路径：** `HTTP / API → Authentication → POST /api/auth/register`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /api/auth/register
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: public Authentication route 命中?
│    ├─ NO → protected/other route
│    └─ YES → public_routes（不经过 JWT middleware）
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ public_routes = auth::public_routes()
└─ merge public + protected routers
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ public_routes() matches POST /api/auth/register
├─ create_user()
├─ Axum Json/State extraction
└─ CALL UserService::register_user → get_user_roles → generate_token
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService operation: register_user → get_user_roles → generate_token
├─ password hash/verify, JWT, OAuth/config logic as applicable
└─ persistence calls: UserDatabase::get_user_by_username / count_users / create_user / assign_role / get_user_roles
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ user/password-reset state read/write when this path needs persistence
└─ return database result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ combine DB result with password/JWT/business rules
└─ DECISION: UserService operation successful?
     ├─ NO → typed UserServiceError
     └─ YES → user/token/reset/OAuth result
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ map success/error via ok(...) / err(...)
└─ return HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ create_user()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "token": "eyJhbGciOi...example",
    "user": {
      "id": 10001,
      "username": "demo_user",
      "roles": [
        "user"
      ]
    }
  }
}
```

### E2E FLOW #21 — POST /api/auth/login

**树路径：** `HTTP / API → Authentication → POST /api/auth/login`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /api/auth/login
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: public Authentication route 命中?
│    ├─ NO → protected/other route
│    └─ YES → public_routes（不经过 JWT middleware）
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ public_routes = auth::public_routes()
└─ merge public + protected routers
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ public_routes() matches POST /api/auth/login
├─ login()
├─ Axum Json/State extraction
└─ CALL UserService::login_user → get_user_roles
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService operation: login_user → get_user_roles
├─ password hash/verify, JWT, OAuth/config logic as applicable
└─ persistence calls: UserDatabase::get_user_by_username / get_user_roles
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ user/password-reset state read/write when this path needs persistence
└─ return database result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ combine DB result with password/JWT/business rules
└─ DECISION: UserService operation successful?
     ├─ NO → typed UserServiceError
     └─ YES → user/token/reset/OAuth result
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ map success/error via ok(...) / err(...)
└─ return HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ login()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "token": "eyJhbGciOi...example",
    "user": {
      "id": 10001,
      "username": "demo_user",
      "roles": [
        "user"
      ]
    }
  }
}
```

### E2E FLOW #22 — POST /api/auth/forgot-password

**树路径：** `HTTP / API → Authentication → POST /api/auth/forgot-password`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /api/auth/forgot-password
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: public Authentication route 命中?
│    ├─ NO → protected/other route
│    └─ YES → public_routes（不经过 JWT middleware）
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ public_routes = auth::public_routes()
└─ merge public + protected routers
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ public_routes() matches POST /api/auth/forgot-password
├─ forgot_password()
├─ Axum Json/State extraction
└─ CALL UserService::request_password_reset
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService operation: request_password_reset
├─ password hash/verify, JWT, OAuth/config logic as applicable
└─ persistence calls: PasswordResetDatabase + UserDatabase
│
▼
FILE: crates/database/crates/user/src/password_reset.rs
│
├─ user/password-reset state read/write when this path needs persistence
└─ return database result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ combine DB result with password/JWT/business rules
└─ DECISION: UserService operation successful?
     ├─ NO → typed UserServiceError
     └─ YES → user/token/reset/OAuth result
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ map success/error via ok(...) / err(...)
└─ return HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ forgot_password()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "message": "如果该邮箱存在，密码重置说明将被发送。"
}
```

### E2E FLOW #23 — POST /api/auth/reset-password

**树路径：** `HTTP / API → Authentication → POST /api/auth/reset-password`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /api/auth/reset-password
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: public Authentication route 命中?
│    ├─ NO → protected/other route
│    └─ YES → public_routes（不经过 JWT middleware）
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ public_routes = auth::public_routes()
└─ merge public + protected routers
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ public_routes() matches POST /api/auth/reset-password
├─ reset_password()
├─ Axum Json/State extraction
└─ CALL UserService::reset_password
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService operation: reset_password
├─ password hash/verify, JWT, OAuth/config logic as applicable
└─ persistence calls: PasswordResetDatabase + UserDatabase
│
▼
FILE: crates/database/crates/user/src/password_reset.rs
│
├─ user/password-reset state read/write when this path needs persistence
└─ return database result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ combine DB result with password/JWT/business rules
└─ DECISION: UserService operation successful?
     ├─ NO → typed UserServiceError
     └─ YES → user/token/reset/OAuth result
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ map success/error via ok(...) / err(...)
└─ return HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ reset_password()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "message": "Password reset successfully"
}
```

### E2E FLOW #24 — GET /api/auth/google

**树路径：** `HTTP / API → Authentication → GET /api/auth/google`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /api/auth/google
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: public Authentication route 命中?
│    ├─ NO → protected/other route
│    └─ YES → public_routes（不经过 JWT middleware）
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ public_routes = auth::public_routes()
└─ merge public + protected routers
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ public_routes() matches GET /api/auth/google
├─ oauth_google()
├─ Axum Json/State extraction
└─ CALL UserService::oauth_url("google")
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService operation: oauth_url("google")
├─ password hash/verify, JWT, OAuth/config logic as applicable
└─ persistence calls: 环境变量 / URL 构造；无 DB 写入
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ user/password-reset state read/write when this path needs persistence
└─ return database result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ combine DB result with password/JWT/business rules
└─ DECISION: UserService operation successful?
     ├─ NO → typed UserServiceError
     └─ YES → user/token/reset/OAuth result
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ map success/error via ok(...) / err(...)
└─ return HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ oauth_google()
│    │    │    └─ CALL → UserService::oauth_url() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::oauth_url()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "url": "https://accounts.google.com/o/oauth2/v2/auth?..."
  }
}
```

### E2E FLOW #25 — GET /api/auth/github

**树路径：** `HTTP / API → Authentication → GET /api/auth/github`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /api/auth/github
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: public Authentication route 命中?
│    ├─ NO → protected/other route
│    └─ YES → public_routes（不经过 JWT middleware）
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ public_routes = auth::public_routes()
└─ merge public + protected routers
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ public_routes() matches GET /api/auth/github
├─ oauth_github()
├─ Axum Json/State extraction
└─ CALL UserService::oauth_url("github")
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService operation: oauth_url("github")
├─ password hash/verify, JWT, OAuth/config logic as applicable
└─ persistence calls: 环境变量 / URL 构造；无 DB 写入
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ user/password-reset state read/write when this path needs persistence
└─ return database result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ combine DB result with password/JWT/business rules
└─ DECISION: UserService operation successful?
     ├─ NO → typed UserServiceError
     └─ YES → user/token/reset/OAuth result
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ map success/error via ok(...) / err(...)
└─ return HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ oauth_github()
│    │    │    └─ CALL → UserService::oauth_url() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::oauth_url()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "url": "https://github.com/login/oauth/authorize?..."
  }
}
```

### E2E FLOW #26 — GET /console/api/channel

**树路径：** `HTTP / API → Channel Management → GET /console/api/channel`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /console/api/channel
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO  → 其它顶层路由 / fallback
│    └─ YES → api::routes()
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ routes()
│    ├─ merge(channel::routes()) into protected_routes
│    └─ layer(middleware::from_fn(crate::auth_middleware))
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware()
│    ├─ read Authorization
│    ├─ require Bearer prefix
│    ├─ verify_jwt()
│    └─ DECISION: JWT valid?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → Claims inserted into request extensions
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ routes() matches GET /console/api/channel
├─ list_channels()
│    ├─ request extraction: Query<PaginationParams> → limit.clamp(1,100) / offset.max(0)
│    ├─ check_admin(&state, &claims)
│    └─ DECISION: admin role present?
│         ├─ NO  → err("Admin access required") → END
│         └─ YES → continue
│
├─ check_admin()
│    └─ CALL UserDatabase::get_user_roles(...)
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase::get_user_roles()
├─ DB connection / SQL role lookup
└─ return roles → channel.rs::check_admin()
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ business invariant: 继续
└─ CALL ChannelService::list(...)
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
├─ ChannelService::list()
└─ CALL ChannelProviderModel::list(...)
│
▼
FILE: crates/database/crates/channel/src/channel_provider.rs
│
├─ ChannelProviderModel::list()
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: database operation successful?
     ├─ NO  → DatabaseError → service → handler → err(...)
     └─ YES → Vec<Channel>
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
└─ return Result to channel handler
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ DECISION: ChannelService result Ok?
│    ├─ NO  → err(e)
│    └─ YES → ok(domain/DTO)
└─ IntoResponse → HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/channel.rs
│    │    ├─ list_channels()
│    │    │    └─ CALL → check_admin() @ crates/server/src/api/channel.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ check_admin()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::get_user_roles()
│    ├─ FILE: crates/service/crates/channel/src/lib.rs
│    │    ├─ ChannelService::list()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::list()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ ph()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": [
    {
      "id": 12,
      "name": "openai-primary",
      "channel_type": "openai",
      "base_url": "https://api.openai.com",
      "status": 1,
      "priority": 100,
      "weight": 100
    }
  ],
  "total": 1
}
```

### E2E FLOW #27 — POST /console/api/channel

**树路径：** `HTTP / API → Channel Management → POST /console/api/channel`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /console/api/channel
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO  → 其它顶层路由 / fallback
│    └─ YES → api::routes()
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ routes()
│    ├─ merge(channel::routes()) into protected_routes
│    └─ layer(middleware::from_fn(crate::auth_middleware))
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware()
│    ├─ read Authorization
│    ├─ require Bearer prefix
│    ├─ verify_jwt()
│    └─ DECISION: JWT valid?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → Claims inserted into request extensions
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ routes() matches POST /console/api/channel
├─ create_channel()
│    ├─ request extraction: Json<ChannelDto> → into_channel()
│    ├─ check_admin(&state, &claims)
│    └─ DECISION: admin role present?
│         ├─ NO  → err("Admin access required") → END
│         └─ YES → continue
│
├─ check_admin()
│    └─ CALL UserDatabase::get_user_roles(...)
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase::get_user_roles()
├─ DB connection / SQL role lookup
└─ return roles → channel.rs::check_admin()
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ business invariant: 继续
└─ CALL ChannelService::create(...)
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
├─ ChannelService::create()
└─ CALL ChannelProviderModel::create(...)
│
▼
FILE: crates/database/crates/channel/src/channel_provider.rs
│
├─ ChannelProviderModel::create()
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: database operation successful?
     ├─ NO  → DatabaseError → service → handler → err(...)
     └─ YES → domain result
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
└─ return Result to channel handler
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ DECISION: ChannelService result Ok?
│    ├─ NO  → err(e)
│    └─ YES → ok(domain/DTO)
└─ IntoResponse → HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/channel.rs
│    │    ├─ create_channel()
│    │    │    └─ CALL → check_admin() @ crates/server/src/api/channel.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ ChannelDto::into_channel()
│    │    ├─ check_admin()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::get_user_roles()
│    ├─ FILE: crates/service/crates/channel/src/lib.rs
│    │    ├─ ChannelService::create()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::create()
│    │    │    └─ CALL → phs() @ crates/database/src/placeholder.rs
│    │    │    └─ CALL → ChannelProviderModel::sync_abilities() @ crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::sync_abilities()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ phs()
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 201 Created
Content-Type: application/json

{
  "success": true,
  "data": {
    "id": 12,
    "name": "openai-primary",
    "channel_type": "openai",
    "base_url": "https://api.openai.com",
    "status": 1,
    "priority": 100,
    "weight": 100
  }
}
```

### E2E FLOW #28 — PUT /console/api/channel

**树路径：** `HTTP / API → Channel Management → PUT /console/api/channel`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: PUT /console/api/channel
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO  → 其它顶层路由 / fallback
│    └─ YES → api::routes()
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ routes()
│    ├─ merge(channel::routes()) into protected_routes
│    └─ layer(middleware::from_fn(crate::auth_middleware))
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware()
│    ├─ read Authorization
│    ├─ require Bearer prefix
│    ├─ verify_jwt()
│    └─ DECISION: JWT valid?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → Claims inserted into request extensions
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ routes() matches PUT /console/api/channel
├─ update_channel()
│    ├─ request extraction: Json<ChannelDto> → into_channel()
│    ├─ check_admin(&state, &claims)
│    └─ DECISION: admin role present?
│         ├─ NO  → err("Admin access required") → END
│         └─ YES → continue
│
├─ check_admin()
│    └─ CALL UserDatabase::get_user_roles(...)
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase::get_user_roles()
├─ DB connection / SQL role lookup
└─ return roles → channel.rs::check_admin()
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ business invariant: DECISION: channel.id == 0? → YES: err("id is required")
└─ CALL ChannelService::update(...)
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
├─ ChannelService::update()
└─ CALL ChannelProviderModel::update(...)
│
▼
FILE: crates/database/crates/channel/src/channel_provider.rs
│
├─ ChannelProviderModel::update()
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: database operation successful?
     ├─ NO  → DatabaseError → service → handler → err(...)
     └─ YES → domain result
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
└─ return Result to channel handler
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ DECISION: ChannelService result Ok?
│    ├─ NO  → err(e)
│    └─ YES → ok(domain/DTO)
└─ IntoResponse → HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/channel.rs
│    │    ├─ update_channel()
│    │    │    └─ CALL → check_admin() @ crates/server/src/api/channel.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    ├─ ChannelDto::into_channel()
│    │    ├─ check_admin()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::get_user_roles()
│    ├─ FILE: crates/service/crates/channel/src/lib.rs
│    │    ├─ ChannelService::update()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::update()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    │    │    └─ CALL → ChannelProviderModel::sync_abilities() @ crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::sync_abilities()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ err()
│    │    ├─ ok()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "id": 12,
    "name": "openai-primary",
    "channel_type": "openai",
    "base_url": "https://api.openai.com",
    "status": 1,
    "priority": 100,
    "weight": 120
  }
}
```

### E2E FLOW #29 — GET /console/api/channel/&#123;id&#125;

**树路径：** `HTTP / API → Channel Management → GET /console/api/channel/{id}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /console/api/channel/{id}
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO  → 其它顶层路由 / fallback
│    └─ YES → api::routes()
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ routes()
│    ├─ merge(channel::routes()) into protected_routes
│    └─ layer(middleware::from_fn(crate::auth_middleware))
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware()
│    ├─ read Authorization
│    ├─ require Bearer prefix
│    ├─ verify_jwt()
│    └─ DECISION: JWT valid?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → Claims inserted into request extensions
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ routes() matches GET /console/api/channel/{id}
├─ get_channel()
│    ├─ request extraction: Path<i32> → channel id
│    ├─ check_admin(&state, &claims)
│    └─ DECISION: admin role present?
│         ├─ NO  → err("Admin access required") → END
│         └─ YES → continue
│
├─ check_admin()
│    └─ CALL UserDatabase::get_user_roles(...)
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase::get_user_roles()
├─ DB connection / SQL role lookup
└─ return roles → channel.rs::check_admin()
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ business invariant: 继续
└─ CALL ChannelService::get_by_id(...)
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
├─ ChannelService::get_by_id()
└─ CALL ChannelProviderModel::get_by_id(...)
│
▼
FILE: crates/database/crates/channel/src/channel_provider.rs
│
├─ ChannelProviderModel::get_by_id()
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: database operation successful?
     ├─ NO  → DatabaseError → service → handler → err(...)
     └─ YES → Vec<Channel>
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
└─ return Result to channel handler
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ DECISION: ChannelService result Ok?
│    ├─ NO  → err(e)
│    └─ YES → ok(domain/DTO)
└─ IntoResponse → HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/channel.rs
│    │    ├─ get_channel()
│    │    │    └─ CALL → check_admin() @ crates/server/src/api/channel.rs
│    │    │    └─ CALL → ChannelService::get_by_id() @ crates/service/crates/channel/src/lib.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ check_admin()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::get_user_roles()
│    ├─ FILE: crates/service/crates/channel/src/lib.rs
│    │    ├─ ChannelService::get_by_id()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::get_by_id()
│    │    │    └─ CALL → ph() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ ph()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "id": 12,
    "name": "openai-primary",
    "channel_type": "openai",
    "base_url": "https://api.openai.com",
    "status": 1,
    "priority": 100,
    "weight": 100
  }
}
```

### E2E FLOW #30 — DELETE /console/api/channel/&#123;id&#125;

**树路径：** `HTTP / API → Channel Management → DELETE /console/api/channel/{id}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: DELETE /console/api/channel/{id}
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO  → 其它顶层路由 / fallback
│    └─ YES → api::routes()
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ routes()
│    ├─ merge(channel::routes()) into protected_routes
│    └─ layer(middleware::from_fn(crate::auth_middleware))
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware()
│    ├─ read Authorization
│    ├─ require Bearer prefix
│    ├─ verify_jwt()
│    └─ DECISION: JWT valid?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → Claims inserted into request extensions
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ routes() matches DELETE /console/api/channel/{id}
├─ delete_channel()
│    ├─ request extraction: Path<i32> → channel id
│    ├─ check_admin(&state, &claims)
│    └─ DECISION: admin role present?
│         ├─ NO  → err("Admin access required") → END
│         └─ YES → continue
│
├─ check_admin()
│    └─ CALL UserDatabase::get_user_roles(...)
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase::get_user_roles()
├─ DB connection / SQL role lookup
└─ return roles → channel.rs::check_admin()
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ business invariant: 继续
└─ CALL ChannelService::delete(...)
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
├─ ChannelService::delete()
└─ CALL ChannelProviderModel::delete(...)
│
▼
FILE: crates/database/crates/channel/src/channel_provider.rs
│
├─ ChannelProviderModel::delete()
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: database operation successful?
     ├─ NO  → DatabaseError → service → handler → err(...)
     └─ YES → domain result
│
▼
FILE: crates/service/crates/channel/src/lib.rs
│
└─ return Result to channel handler
│
▼
FILE: crates/server/src/api/channel.rs
│
├─ DECISION: ChannelService result Ok?
│    ├─ NO  → err(e)
│    └─ YES → ok(domain/DTO)
└─ IntoResponse → HTTP response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/channel.rs
│    │    ├─ delete_channel()
│    │    │    └─ CALL → check_admin() @ crates/server/src/api/channel.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ check_admin()
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::get_user_roles()
│    ├─ FILE: crates/service/crates/channel/src/lib.rs
│    │    ├─ ChannelService::delete()
│    ├─ FILE: crates/database/crates/channel/src/channel_provider.rs
│    │    ├─ ChannelProviderModel::delete()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "message": "Channel deleted"
}
```

### E2E FLOW #31 — GET /console/api/tokens

**树路径：** `HTTP / API → Token → GET /console/api/tokens`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /console/api/tokens
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(token::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims → next
│
▼
FILE: crates/server/src/api/token.rs
│
├─ routes() matches GET /console/api/tokens
├─ list_tokens()
├─ parse Path / Query / Json according to method
├─ validate token-specific fields / rotation / whitelist parameters
└─ CALL TokenService::list(...)
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
├─ TokenService::list()
└─ CALL RouterTokenModel::list(...)
│
▼
FILE: crates/database/crates/router/src/token.rs
│
├─ RouterTokenModel::list()
├─ DB read/write + token state/rotation/quota fields
└─ DECISION: DB/token operation successful?
     ├─ NO → DatabaseError / not found / validation result
     └─ YES → RouterToken / bool / TokenRotationResult / ()
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
└─ return Result to API handler
│
▼
FILE: crates/server/src/api/token.rs
│
├─ map result to ok(...) / err(...)
└─ return HTTP JSON response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/token.rs
│    │    ├─ list_tokens()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/token/src/lib.rs
│    │    ├─ TokenService::list()
│    ├─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::list()
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": [
    {
      "token": "bc_live_7d4e...example",
      "user_id": 10001,
      "name": "production",
      "status": 1,
      "quota": 100000000,
      "ip_whitelist": [
        "203.0.113.10"
      ]
    }
  ]
}
```

### E2E FLOW #32 — POST /console/api/tokens

**树路径：** `HTTP / API → Token → POST /console/api/tokens`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /console/api/tokens
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(token::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims → next
│
▼
FILE: crates/server/src/api/token.rs
│
├─ routes() matches POST /console/api/tokens
├─ create_token()
├─ parse Path / Query / Json according to method
├─ validate token-specific fields / rotation / whitelist parameters
└─ CALL TokenService::create(...)
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
├─ TokenService::create()
└─ CALL RouterTokenModel::create(...)
│
▼
FILE: crates/database/crates/router/src/token.rs
│
├─ RouterTokenModel::create()
├─ DB read/write + token state/rotation/quota fields
└─ DECISION: DB/token operation successful?
     ├─ NO → DatabaseError / not found / validation result
     └─ YES → RouterToken / bool / TokenRotationResult / ()
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
└─ return Result to API handler
│
▼
FILE: crates/server/src/api/token.rs
│
├─ map result to ok(...) / err(...)
└─ return HTTP JSON response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/token.rs
│    │    ├─ create_token()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/token/src/lib.rs
│    │    ├─ TokenService::create()
│    ├─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::create()
│    │    │    └─ CALL → phs() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ phs()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 201 Created
Content-Type: application/json

{
  "success": true,
  "data": {
    "token": "bc_live_7d4e...example",
    "user_id": 10001,
    "name": "production",
    "status": 1,
    "quota": 100000000,
    "ip_whitelist": [
      "203.0.113.10"
    ]
  }
}
```

### E2E FLOW #33 — GET /console/api/tokens/&#123;token&#125;

**树路径：** `HTTP / API → Token → GET /console/api/tokens/{token}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /console/api/tokens/{token}
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(token::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims → next
│
▼
FILE: crates/server/src/api/token.rs
│
├─ routes() matches GET /console/api/tokens/{token}
├─ get_token()
├─ parse Path / Query / Json according to method
├─ validate token-specific fields / rotation / whitelist parameters
└─ CALL TokenService::validate(...)
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
├─ TokenService::validate()
└─ CALL RouterTokenModel::validate(...)
│
▼
FILE: crates/database/crates/router/src/token.rs
│
├─ RouterTokenModel::validate()
├─ DB read/write + token state/rotation/quota fields
└─ DECISION: DB/token operation successful?
     ├─ NO → DatabaseError / not found / validation result
     └─ YES → RouterToken / bool / TokenRotationResult / ()
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
└─ return Result to API handler
│
▼
FILE: crates/server/src/api/token.rs
│
├─ map result to ok(...) / err(...)
└─ return HTTP JSON response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/token.rs
│    │    ├─ get_token()
│    │    │    └─ CALL → TokenService::validate() @ crates/service/crates/token/src/lib.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/token/src/lib.rs
│    │    ├─ TokenService::validate()
│    ├─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::validate()
│    │    │    └─ CALL → RouterTokenModel::current_timestamp() @ crates/database/crates/router/src/token.rs
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    │    ├─ RouterTokenModel::current_timestamp()
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "token": "bc_live_7d4e...example",
    "user_id": 10001,
    "name": "production",
    "status": 1,
    "quota": 100000000,
    "ip_whitelist": [
      "203.0.113.10"
    ]
  }
}
```

### E2E FLOW #34 — PUT /console/api/tokens/&#123;token&#125;

**树路径：** `HTTP / API → Token → PUT /console/api/tokens/{token}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: PUT /console/api/tokens/{token}
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(token::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims → next
│
▼
FILE: crates/server/src/api/token.rs
│
├─ routes() matches PUT /console/api/tokens/{token}
├─ update_token()
├─ parse Path / Query / Json according to method
├─ validate token-specific fields / rotation / whitelist parameters
└─ CALL TokenService::update_status(...)
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
├─ TokenService::update_status()
└─ CALL RouterTokenModel::update_status(...)
│
▼
FILE: crates/database/crates/router/src/token.rs
│
├─ RouterTokenModel::update_status()
├─ DB read/write + token state/rotation/quota fields
└─ DECISION: DB/token operation successful?
     ├─ NO → DatabaseError / not found / validation result
     └─ YES → RouterToken / bool / TokenRotationResult / ()
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
└─ return Result to API handler
│
▼
FILE: crates/server/src/api/token.rs
│
├─ map result to ok(...) / err(...)
└─ return HTTP JSON response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/token.rs
│    │    ├─ update_token()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/token/src/lib.rs
│    │    ├─ TokenService::update_status()
│    ├─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::update_status()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "token": "bc_live_7d4e...example",
    "user_id": 10001,
    "name": "production",
    "status": 0,
    "quota": 100000000,
    "ip_whitelist": [
      "203.0.113.10"
    ]
  }
}
```

### E2E FLOW #35 — DELETE /console/api/tokens/&#123;token&#125;

**树路径：** `HTTP / API → Token → DELETE /console/api/tokens/{token}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: DELETE /console/api/tokens/{token}
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(token::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims → next
│
▼
FILE: crates/server/src/api/token.rs
│
├─ routes() matches DELETE /console/api/tokens/{token}
├─ delete_token()
├─ parse Path / Query / Json according to method
├─ validate token-specific fields / rotation / whitelist parameters
└─ CALL TokenService::delete(...)
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
├─ TokenService::delete()
└─ CALL RouterTokenModel::delete(...)
│
▼
FILE: crates/database/crates/router/src/token.rs
│
├─ RouterTokenModel::delete()
├─ DB read/write + token state/rotation/quota fields
└─ DECISION: DB/token operation successful?
     ├─ NO → DatabaseError / not found / validation result
     └─ YES → RouterToken / bool / TokenRotationResult / ()
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
└─ return Result to API handler
│
▼
FILE: crates/server/src/api/token.rs
│
├─ map result to ok(...) / err(...)
└─ return HTTP JSON response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/token.rs
│    │    ├─ delete_token()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/token/src/lib.rs
│    │    ├─ TokenService::delete()
│    ├─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::delete()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "message": "Token deleted"
}
```

### E2E FLOW #36 — POST /console/api/tokens/&#123;token&#125;/rotate

**树路径：** `HTTP / API → Token → POST /console/api/tokens/{token}/rotate`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /console/api/tokens/{token}/rotate
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(token::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims → next
│
▼
FILE: crates/server/src/api/token.rs
│
├─ routes() matches POST /console/api/tokens/{token}/rotate
├─ rotate_token()
├─ parse Path / Query / Json according to method
├─ validate token-specific fields / rotation / whitelist parameters
└─ CALL TokenService::rotate(...)
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
├─ TokenService::rotate()
└─ CALL RouterTokenModel::rotate(...)
│
▼
FILE: crates/database/crates/router/src/token.rs
│
├─ RouterTokenModel::rotate()
├─ DB read/write + token state/rotation/quota fields
└─ DECISION: DB/token operation successful?
     ├─ NO → DatabaseError / not found / validation result
     └─ YES → RouterToken / bool / TokenRotationResult / ()
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
└─ return Result to API handler
│
▼
FILE: crates/server/src/api/token.rs
│
├─ map result to ok(...) / err(...)
└─ return HTTP JSON response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/token.rs
│    │    ├─ rotate_token()
│    │    │    └─ CALL → TokenService::rotate() @ crates/service/crates/token/src/lib.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/token/src/lib.rs
│    │    ├─ TokenService::rotate()
│    ├─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::rotate()
│    │    │    └─ CALL → RouterTokenModel::find_by_token() @ crates/database/crates/router/src/token.rs
│    │    │    └─ CALL → RouterTokenModel::current_timestamp() @ crates/database/crates/router/src/token.rs
│    │    │    └─ CALL → RouterTokenModel::generate_token() @ crates/database/crates/router/src/token.rs
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    │    ├─ RouterTokenModel::find_by_token()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    │    ├─ RouterTokenModel::current_timestamp()
│    │    ├─ RouterTokenModel::generate_token()
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "new_token": "bc_live_9af2...example",
    "old_token_valid_until": "2026-08-11T15:30:00+08:00"
  }
}
```

### E2E FLOW #37 — POST /console/api/tokens/&#123;token&#125;/revoke-old

**树路径：** `HTTP / API → Token → POST /console/api/tokens/{token}/revoke-old`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /console/api/tokens/{token}/revoke-old
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(token::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims → next
│
▼
FILE: crates/server/src/api/token.rs
│
├─ routes() matches POST /console/api/tokens/{token}/revoke-old
├─ revoke_old_key()
├─ parse Path / Query / Json according to method
├─ validate token-specific fields / rotation / whitelist parameters
└─ CALL TokenService::revoke_old_key(...)
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
├─ TokenService::revoke_old_key()
└─ CALL RouterTokenModel::revoke_old_key(...)
│
▼
FILE: crates/database/crates/router/src/token.rs
│
├─ RouterTokenModel::revoke_old_key()
├─ DB read/write + token state/rotation/quota fields
└─ DECISION: DB/token operation successful?
     ├─ NO → DatabaseError / not found / validation result
     └─ YES → RouterToken / bool / TokenRotationResult / ()
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
└─ return Result to API handler
│
▼
FILE: crates/server/src/api/token.rs
│
├─ map result to ok(...) / err(...)
└─ return HTTP JSON response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/token.rs
│    │    ├─ revoke_old_key()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/token/src/lib.rs
│    │    ├─ TokenService::revoke_old_key()
│    ├─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::revoke_old_key()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "message": "Old key revoked"
}
```

### E2E FLOW #38 — POST /console/api/tokens/&#123;token&#125;/ip-whitelist

**树路径：** `HTTP / API → Token → POST /console/api/tokens/{token}/ip-whitelist`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /console/api/tokens/{token}/ip-whitelist
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: Management API route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(token::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims → next
│
▼
FILE: crates/server/src/api/token.rs
│
├─ routes() matches POST /console/api/tokens/{token}/ip-whitelist
├─ set_ip_whitelist()
├─ parse Path / Query / Json according to method
├─ validate token-specific fields / rotation / whitelist parameters
└─ CALL TokenService::set_ip_whitelist(...)
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
├─ TokenService::set_ip_whitelist()
└─ CALL RouterTokenModel::set_ip_whitelist(...)
│
▼
FILE: crates/database/crates/router/src/token.rs
│
├─ RouterTokenModel::set_ip_whitelist()
├─ DB read/write + token state/rotation/quota fields
└─ DECISION: DB/token operation successful?
     ├─ NO → DatabaseError / not found / validation result
     └─ YES → RouterToken / bool / TokenRotationResult / ()
│
▼
FILE: crates/service/crates/token/src/lib.rs
│
└─ return Result to API handler
│
▼
FILE: crates/server/src/api/token.rs
│
├─ map result to ok(...) / err(...)
└─ return HTTP JSON response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/token.rs
│    │    ├─ set_ip_whitelist()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/token/src/lib.rs
│    │    ├─ TokenService::set_ip_whitelist()
│    ├─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::set_ip_whitelist()
│    │    │    └─ CALL → adapt_sql() @ crates/database/src/placeholder.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/src/placeholder.rs
│    │    ├─ adapt_sql()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "ip_whitelist": [
      "203.0.113.10",
      "203.0.113.11"
    ]
  }
}
```

### E2E FLOW #39 — POST /console/api/user/register

**树路径：** `HTTP / API → User → POST /console/api/user/register`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /console/api/user/register
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: protected User route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(user::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims inserted
│
▼
FILE: crates/server/src/api/user.rs
│
├─ routes() matches POST /console/api/user/register
├─ register()
├─ Path/Query/Json/Claims extraction
└─ CALL UserService::register_user(...)
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::register_user()
├─ password/JWT/balance/recharge logic as applicable
└─ DB calls: UserDatabase::get_user_by_username / count_users / create_user / assign_role
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase read/write
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: persistence operation successful?
     ├─ NO → DatabaseError → UserServiceError
     └─ YES → user/balance/roles/recharge result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ return domain result
│
▼
FILE: crates/server/src/api/user.rs
│
├─ map domain result to API response
└─ return HTTP JSON
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/user.rs
│    │    ├─ register()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::register_user()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::count_users() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_user() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::get_user_by_username()
│    │    ├─ UserDatabase::count_users()
│    │    ├─ UserDatabase::create_user()
│    │    ├─ UserDatabase::assign_role()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "id": 10001,
    "username": "demo_user",
    "roles": [
      "user"
    ]
  }
}
```

### E2E FLOW #40 — POST /console/api/user/login

**树路径：** `HTTP / API → User → POST /console/api/user/login`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /console/api/user/login
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: protected User route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(user::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims inserted
│
▼
FILE: crates/server/src/api/user.rs
│
├─ routes() matches POST /console/api/user/login
├─ login()
├─ Path/Query/Json/Claims extraction
└─ CALL UserService::login_user(...)
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::login_user()
├─ password/JWT/balance/recharge logic as applicable
└─ DB calls: UserDatabase::get_user_by_username
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase read/write
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: persistence operation successful?
     ├─ NO → DatabaseError → UserServiceError
     └─ YES → user/balance/roles/recharge result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ return domain result
│
▼
FILE: crates/server/src/api/user.rs
│
├─ map domain result to API response
└─ return HTTP JSON
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/user.rs
│    │    ├─ login()
│    │    │    └─ CALL → persist_client_state() @ crates/server/src/api/user.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ persist_client_state()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::login_user()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → BundleVerifier::verify() @ crates/installer/src/bundle.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::get_user_by_username()
│    ├─ FILE: crates/installer/src/bundle.rs
│    │    ├─ BundleVerifier::verify()
│    │    │    └─ CALL → BundleManifest::load() @ crates/installer/src/bundle.rs
│    │    │    └─ CALL → Platform::current() @ crates/installer/src/platform.rs
│    │    │    └─ CALL → BundleVerifier::verify_checksums() @ crates/installer/src/bundle.rs
│    │    ├─ BundleManifest::load()
│    │    ├─ BundleVerifier::verify_checksums()
│    ├─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│    └─ FILE: crates/installer/src/platform.rs
│    │    ├─ Platform::current()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "token": "eyJhbGciOi...example",
    "username": "demo_user"
  }
}
```

### E2E FLOW #41 — POST /console/api/user/topup

**树路径：** `HTTP / API → User → POST /console/api/user/topup`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: POST /console/api/user/topup
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: protected User route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(user::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims inserted
│
▼
FILE: crates/server/src/api/user.rs
│
├─ routes() matches POST /console/api/user/topup
├─ topup()
├─ Path/Query/Json/Claims extraction
└─ CALL UserService::topup(...)
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::topup()
├─ password/JWT/balance/recharge logic as applicable
└─ DB calls: UserDatabase::create_recharge / update_balance
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase read/write
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: persistence operation successful?
     ├─ NO → DatabaseError → UserServiceError
     └─ YES → user/balance/roles/recharge result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ return domain result
│
▼
FILE: crates/server/src/api/user.rs
│
├─ map domain result to API response
└─ return HTTP JSON
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/user.rs
│    │    ├─ topup()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::topup()
│    │    │    └─ CALL → UserDatabase::create_recharge() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::create_recharge()
│    │    │    └─ CALL → UserDatabase::update_balance_cny() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance_usd() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::update_balance()
│    │    │    └─ CALL → UserDatabase::update_balance_cny() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance_usd() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::update_balance_cny()
│    │    ├─ UserDatabase::update_balance_usd()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "user_id": 10001,
    "amount": 100.0,
    "currency": "USD",
    "balance": 286.42
  }
}
```

### E2E FLOW #42 — GET /console/api/user/check_username

**树路径：** `HTTP / API → User → GET /console/api/user/check_username`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /console/api/user/check_username
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: protected User route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(user::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims inserted
│
▼
FILE: crates/server/src/api/user.rs
│
├─ routes() matches GET /console/api/user/check_username
├─ check_username()
├─ Path/Query/Json/Claims extraction
└─ CALL UserService::is_username_available(...)
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::is_username_available()
├─ password/JWT/balance/recharge logic as applicable
└─ DB calls: UserDatabase::get_user_by_username
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase read/write
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: persistence operation successful?
     ├─ NO → DatabaseError → UserServiceError
     └─ YES → user/balance/roles/recharge result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ return domain result
│
▼
FILE: crates/server/src/api/user.rs
│
├─ map domain result to API response
└─ return HTTP JSON
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/user.rs
│    │    ├─ check_username()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::is_username_available()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::get_user_by_username()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "username": "demo_user",
    "available": false
  }
}
```

### E2E FLOW #43 — GET /console/api/user/recharges

**树路径：** `HTTP / API → User → GET /console/api/user/recharges`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /console/api/user/recharges
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: protected User route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(user::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims inserted
│
▼
FILE: crates/server/src/api/user.rs
│
├─ routes() matches GET /console/api/user/recharges
├─ list_recharges()
├─ Path/Query/Json/Claims extraction
└─ CALL UserService::list_recharges(...)
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::list_recharges()
├─ password/JWT/balance/recharge logic as applicable
└─ DB calls: UserDatabase::list_recharges
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase read/write
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: persistence operation successful?
     ├─ NO → DatabaseError → UserServiceError
     └─ YES → user/balance/roles/recharge result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ return domain result
│
▼
FILE: crates/server/src/api/user.rs
│
├─ map domain result to API response
└─ return HTTP JSON
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/user.rs
│    │    ├─ list_recharges()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::list_recharges()
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": [
    {
      "id": 9001,
      "amount": 100.0,
      "currency": "USD",
      "created_at": "2026-08-11T14:30:00+08:00"
    }
  ]
}
```

### E2E FLOW #44 — GET /console/api/list_users

**树路径：** `HTTP / API → User → GET /console/api/list_users`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/service/crates/user/src/lib.rs`, `crates/common/src/constants.rs`

```text
START
│
├─ 调用方输入
│    ├─ Entry: GET /console/api/list_users
│    ├─ Method / Path / Query / Headers / Body
│    └─ DECISION: 请求到达 BurnCloud listener?
│         ├─ NO  → 网络层结束，应用代码不执行 → END
│         └─ YES → Axum Unified App
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server()（启动时）
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app()
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ create_app()
│    ├─ merge(api::routes(...))
│    ├─ merge(internal_app)
│    ├─ optional merge(liveview_router)
│    ├─ fallback_service(router_app)
│    └─ middleware: CORS / Trace / request-id
│
├─ DECISION: protected User route 命中?
│    ├─ NO → other route
│    └─ YES → protected_routes
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ merge(user::routes())
└─ auth_middleware wraps protected router
│
▼
FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│
├─ auth_middleware() → verify_jwt()
└─ DECISION: JWT valid?
     ├─ NO → HTTP 401 → END
     └─ YES → Claims inserted
│
▼
FILE: crates/server/src/api/user.rs
│
├─ routes() matches GET /console/api/list_users
├─ list_users()
├─ Path/Query/Json/Claims extraction
└─ CALL UserService::list_users / get_user_roles(...)
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
├─ UserService::list_users / get_user_roles()
├─ password/JWT/balance/recharge logic as applicable
└─ DB calls: UserDatabase::list_users / get_user_roles
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase read/write
├─ db.get_connection() / SQL execution / row mapping
└─ DECISION: persistence operation successful?
     ├─ NO → DatabaseError → UserServiceError
     └─ YES → user/balance/roles/recharge result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ return domain result
│
▼
FILE: crates/server/src/api/user.rs
│
├─ map domain result to API response
└─ return HTTP JSON
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ auth_middleware()
│    │    │    └─ CALL → verify_jwt() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ verify_jwt()
│    │    │    └─ CALL → get_jwt_secret() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ get_jwt_secret()
│    │    │    └─ CALL → jwt_secret() @ crates/common/src/constants.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/user.rs
│    │    ├─ list_users()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::get_user_roles()
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    └─ FILE: crates/common/src/constants.rs  [PR CHANGED]
│    │    ├─ jwt_secret()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/common/src/constants.rs`**

HUNKS:
```text
@@ -3,14 +3,6 @@ pub const API_PREFIX: &str = "/console/api";
```

BEFORE / REMOVED:
```diff
-/// Dev-only fallback when `JWT_SECRET` is unset. Must match token signing (`UserService`).
-pub const DEFAULT_JWT_SECRET: &str = "burncloud-default-secret-change-in-production";
-
-/// Resolve JWT signing/verification secret from the environment.
-pub fn jwt_secret() -> String {
-    std::env::var("JWT_SECRET").unwrap_or_else(|_| DEFAULT_JWT_SECRET.to_string())
-}
-
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": [
    {
      "id": 10001,
      "username": "demo_user",
      "roles": [
        "user"
      ],
      "balance": 286.42
    }
  ]
}
```

### E2E FLOW #45 — GET /api/billing/summary

**树路径：** `HTTP / API → Billing / Usage → GET /api/billing/summary`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /api/billing/summary
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/billing.rs
│
├─ Handler
│    └─ billing_summary_handler()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/router-log/src/lib.rs
│
├─ BillingService::get_billing_summary_for_user(db, claims.sub, start, end)
└─ CALL burncloud_database_router::get_billing_summary_for_user(...)
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ build user/time filtered billing aggregation
├─ aggregate per-model request/token/cost state
└─ DECISION: billing SQL succeeds?
     ├─ NO → DatabaseError → BillingService → err(...)
     └─ YES → BillingSummary → API handler
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/billing.rs
│    │    ├─ billing_summary_handler()
│    │    │    └─ CALL → BillingService::get_billing_summary_for_user() @ crates/service/crates/router-log/src/lib.rs
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/router-log/src/lib.rs
│    │    ├─ BillingService::get_billing_summary_for_user()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "user_id": 10001,
  "currency": "USD",
  "request_count": 12842,
  "prompt_tokens": 18420560,
  "completion_tokens": 6912840,
  "total_tokens": 25333400,
  "total_cost": 126.67
}
```

### E2E FLOW #46 — GET /console/api/logs

**树路径：** `HTTP / API → Logs → GET /console/api/logs`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/logs
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/log.rs
│
├─ Handler
│    └─ list_logs()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/router-log/src/lib.rs
│
├─ RouterLogService::get(db, page_size, offset)
└─ RouterLogModel::get(...)
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ query router_logs with pagination
└─ DECISION: DB query succeeds?
     ├─ NO → ApiError
     └─ YES → Vec<RouterLog> → LogPage
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/log.rs
│    │    ├─ list_logs()
│    │    │    └─ CALL → RouterLogService::get() @ crates/service/crates/router-log/src/lib.rs
│    └─ FILE: crates/service/crates/router-log/src/lib.rs
│    │    ├─ RouterLogService::get()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": [
    {
      "id": 50123,
      "user_id": 10001,
      "model": "gpt-5.4",
      "channel_id": 12,
      "status": 200,
      "prompt_tokens": 28,
      "completion_tokens": 16,
      "cost": 0.00042,
      "created_at": "2026-08-11T14:40:15+08:00"
    }
  ],
  "page": 1,
  "page_size": 20
}
```

### E2E FLOW #47 — GET /console/api/usage/&#123;user_id&#125;

**树路径：** `HTTP / API → Logs → GET /console/api/usage/{user_id}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/usage/{user_id}
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/log.rs
│
├─ Handler
│    └─ get_user_usage()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/router-log/src/lib.rs
│
└─ RouterLogService::get_usage_by_user(db, user_id)
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ SUM prompt_tokens / completion_tokens for user_id
└─ return tuple → handler calculates total_tokens
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/log.rs
│    │    ├─ get_user_usage()
│    │    │    └─ CALL → RouterLogService::get_usage_by_user() @ crates/service/crates/router-log/src/lib.rs
│    └─ FILE: crates/service/crates/router-log/src/lib.rs
│    │    ├─ RouterLogService::get_usage_by_user()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "user_id": 10001,
  "prompt_tokens": 18420560,
  "completion_tokens": 6912840,
  "total_tokens": 25333400
}
```

### E2E FLOW #48 — GET /console/internal/billing/summary

**树路径：** `HTTP / API → Billing / Usage → GET /console/internal/billing/summary`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/internal/billing/summary
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/log.rs
│
├─ Handler
│    └─ billing_summary_handler()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/server/src/api/log.rs
│
├─ billing_summary_inner()
├─ DECISION: BURNCLOUD_INTERNAL_SECRET configured?
│    ├─ YES → x-internal-secret must match or HTTP 401
│    └─ NO  → no extra internal-secret rejection
└─ BillingService::get_billing_summary(...)
│
▼
FILE: crates/service/crates/router-log/src/lib.rs
│
└─ BillingService::get_billing_summary() → database router
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ aggregate billing state from router_logs
└─ return BillingSummary
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/log.rs
│    │    ├─ billing_summary_handler()
│    │    │    └─ CALL → billing_summary_inner() @ crates/server/src/api/log.rs
│    │    ├─ billing_summary_inner()
│    └─ FILE: crates/service/crates/router-log/src/lib.rs
│    │    ├─ BillingService::get_billing_summary()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "user_id": 10001,
  "currency": "USD",
  "request_count": 12842,
  "prompt_tokens": 18420560,
  "completion_tokens": 6912840,
  "total_tokens": 25333400,
  "total_cost": 126.67
}
```

### E2E FLOW #49 — GET /console/api/monitor

**树路径：** `HTTP / API → Monitoring / Security → GET /console/api/monitor`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/monitor
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/monitor.rs
│
├─ Handler
│    └─ get_system_metrics()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/monitor/src/service.rs
│
├─ SystemMonitorService::get_metrics()
├─ DECISION: cached_metrics exists and still fresh?
│    ├─ YES → return cached clone
│    └─ NO  → collect_fresh_metrics()
│         └─ collect_metrics_internal()
│              └─ tokio::join!(CPU, Memory, Disk)
│
▼
FILE: crates/service/crates/monitor/src/collectors/cpu.rs
│
└─ CpuCollector::collect()
│
▼
FILE: crates/service/crates/monitor/src/collectors/memory.rs
│
└─ MemoryCollector::collect()
│
▼
FILE: crates/service/crates/monitor/src/collectors/disk.rs
│
└─ DiskCollector::collect_all()
│
▼
FILE: crates/service/crates/monitor/src/service.rs
│
└─ update cached_metrics → return SystemMetrics
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/monitor.rs
│    │    ├─ get_system_metrics()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/monitor/src/service.rs
│    │    ├─ SystemMonitorService::get_metrics()
│    │    ├─ SystemMonitorService::collect_fresh_metrics()
│    │    │    └─ CALL → SystemMonitorService::collect_metrics_internal() @ crates/service/crates/monitor/src/service.rs
│    │    ├─ SystemMonitorService::collect_metrics_internal()
│    │    │    └─ CALL → SystemMetrics::new() @ crates/service/crates/monitor/src/types.rs
│    ├─ FILE: crates/service/crates/monitor/src/collectors/cpu.rs
│    │    ├─ CpuCollector::collect()
│    ├─ FILE: crates/service/crates/monitor/src/collectors/memory.rs
│    │    ├─ MemoryCollector::collect()
│    ├─ FILE: crates/service/crates/monitor/src/collectors/disk.rs
│    │    ├─ DiskCollector::collect_all()
│    ├─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│    └─ FILE: crates/service/crates/monitor/src/types.rs
│    │    ├─ SystemMetrics::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "cpu_usage": 31.7,
  "memory_usage": 62.4,
  "disk_usage": 48.9,
  "uptime_seconds": 483920
}
```

### E2E FLOW #50 — GET /console/api/monitor/security

**树路径：** `HTTP / API → Monitoring / Security → GET /console/api/monitor/security`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/monitor/security
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/security.rs
│
├─ Handler
│    └─ security_summary()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/router-log/src/lib.rs
│
└─ RouterLogService::get(...) → RouterLogModel::get(...)
│
▼
FILE: crates/database/crates/router/src/log.rs
│
└─ read router_logs
│
▼
FILE: crates/server/src/api/security.rs
│
├─ summary: compute_security_score() + compute_sparkline()
└─ events: filter status >= 400 → log_to_risk_event() → paginate
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/security.rs
│    │    ├─ security_summary()
│    │    │    └─ CALL → RouterLogService::get() @ crates/service/crates/router-log/src/lib.rs
│    │    │    └─ CALL → compute_security_score() @ crates/server/src/api/security.rs
│    │    │    └─ CALL → compute_sparkline() @ crates/server/src/api/security.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ compute_security_score()
│    │    ├─ compute_sparkline()
│    │    ├─ log_to_risk_event()
│    ├─ FILE: crates/service/crates/router-log/src/lib.rs
│    │    ├─ RouterLogService::get()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "score": 96,
  "blocked_count": 14,
  "threat_sources": 3,
  "error_rate": 0.012,
  "sparkline_7d": [
    98,
    97,
    97,
    96,
    96,
    95,
    96
  ]
}
```

### E2E FLOW #51 — GET /console/api/monitor/security/events

**树路径：** `HTTP / API → Monitoring / Security → GET /console/api/monitor/security/events`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/monitor/security/events
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/security.rs
│
├─ Handler
│    └─ security_events()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/router-log/src/lib.rs
│
└─ RouterLogService::get(...) → RouterLogModel::get(...)
│
▼
FILE: crates/database/crates/router/src/log.rs
│
└─ read router_logs
│
▼
FILE: crates/server/src/api/security.rs
│
├─ summary: compute_security_score() + compute_sparkline()
└─ events: filter status >= 400 → log_to_risk_event() → paginate
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/security.rs
│    │    ├─ security_events()
│    │    │    └─ CALL → RouterLogService::get() @ crates/service/crates/router-log/src/lib.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ compute_security_score()
│    │    ├─ compute_sparkline()
│    │    ├─ log_to_risk_event()
│    ├─ FILE: crates/service/crates/router-log/src/lib.rs
│    │    ├─ RouterLogService::get()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "data": [
    {
      "id": "risk_50123",
      "type": "upstream_error",
      "severity": "medium",
      "status": 502,
      "source": "203.0.113.20",
      "created_at": "2026-08-11T14:42:31+08:00"
    }
  ],
  "total": 1
}
```

### E2E FLOW #52 — GET /console/api/monitor/security/filters

**树路径：** `HTTP / API → Monitoring / Security → GET /console/api/monitor/security/filters`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/monitor/security/filters
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/security.rs
│
├─ Handler
│    └─ security_filters_get()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/server/src/api/security.rs
│
├─ DECISION: GET or PUT filters?
│    ├─ GET → Database::query_with_params(SELECT sys_settings)
│    │    └─ empty / invalid JSON → FilterConfig::default()
│    └─ PUT → serde_json::to_string(config)
│         └─ Database::execute_query_with_params(INSERT OR REPLACE)
│
▼
FILE: crates/database/src/lib.rs
│
└─ execute parameterized SQL against sys_settings
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/security.rs
│    │    ├─ security_filters_get()
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ Default::default()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "enabled": true,
  "block_4xx_burst": true,
  "block_5xx_burst": true,
  "threshold": 20
}
```

### E2E FLOW #53 — PUT /console/api/monitor/security/filters

**树路径：** `HTTP / API → Monitoring / Security → PUT /console/api/monitor/security/filters`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: PUT /console/api/monitor/security/filters
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/security.rs
│
├─ Handler
│    └─ security_filters_put()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: write/mutate state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/server/src/api/security.rs
│
├─ DECISION: GET or PUT filters?
│    ├─ GET → Database::query_with_params(SELECT sys_settings)
│    │    └─ empty / invalid JSON → FilterConfig::default()
│    └─ PUT → serde_json::to_string(config)
│         └─ Database::execute_query_with_params(INSERT OR REPLACE)
│
▼
FILE: crates/database/src/lib.rs
│
└─ execute parameterized SQL against sys_settings
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/security.rs
│    │    ├─ security_filters_put()
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ Default::default()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "data": {
    "enabled": true,
    "block_4xx_burst": true,
    "block_5xx_burst": true,
    "threshold": 25
  }
}
```

### E2E FLOW #54 — POST /console/api/monitor/security/emergency-circuit-break

**树路径：** `HTTP / API → Monitoring / Security → POST /console/api/monitor/security/emergency-circuit-break`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /console/api/monitor/security/emergency-circuit-break
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/security.rs
│
├─ Handler
│    └─ security_emergency_circuit_break()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: write/mutate state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/server/src/api/security.rs
│
├─ validate reason non-empty
├─ post_router_internal("/console/internal/circuit-breaker/trip-all", body)
├─ build http://127.0.0.1:{PORT} URL
└─ optional X-Internal-Secret header
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
└─ circuit_breaker_trip_all_handler()
│
▼
FILE: crates/router/src/circuit_breaker.rs
│
└─ trip_all() mutates known breaker states
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/security.rs
│    │    ├─ security_emergency_circuit_break()
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → post_router_internal() @ crates/server/src/api/security.rs
│    │    ├─ post_router_internal()
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ circuit_breaker_trip_all_handler()
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::trip_all()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "message": "All known upstream circuits tripped",
  "reason": "manual emergency isolation"
}
```

### E2E FLOW #55 — GET /console/api/monitor/security/circuit-breaker-status

**树路径：** `HTTP / API → Monitoring / Security → GET /console/api/monitor/security/circuit-breaker-status`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/monitor/security/circuit-breaker-status
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/security.rs
│
├─ Handler
│    └─ security_circuit_breaker_status()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/server/src/api/security.rs
│
├─ call_router_internal("/console/internal/health")
├─ localhost reqwest GET
└─ optional X-Internal-Secret
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
└─ health_status_handler() builds runtime health JSON
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/security.rs
│    │    ├─ security_circuit_breaker_status()
│    │    │    └─ CALL → call_router_internal() @ crates/server/src/api/security.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    │    ├─ call_router_internal()
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ health_status_handler()
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "status": "ok",
  "circuit_breaker": {
    "open": 2,
    "half_open": 0,
    "closed": 17
  }
}
```

### E2E FLOW #56 — GET /console/api/cache/stats

**树路径：** `HTTP / API → Cache → GET /console/api/cache/stats`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/cache/stats
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/cache.rs
│
├─ Handler
│    └─ stats()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/cache/src/service.rs
│
├─ DECISION: CacheService available / Redis connected?
│    ├─ NO → disabled/empty semantics or CacheError
│    └─ YES → ConnectionManager
├─ stats(): read Redis INFO/dbsize-like statistics
└─ clear_all(): KEYS "bc:*" → DECISION keys empty? → DEL matching keys
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/cache.rs
│    │    ├─ stats()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::stats()
│    │    ├─ CacheService::clear_all()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "enabled": true,
  "backend": "redis",
  "hits": 182304,
  "misses": 21044,
  "hit_rate": 0.8965,
  "keys": 3812
}
```

### E2E FLOW #57 — POST /console/api/cache/clear

**树路径：** `HTTP / API → Cache → POST /console/api/cache/clear`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /console/api/cache/clear
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/cache.rs
│
├─ Handler
│    └─ clear()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: write/mutate state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/cache/src/service.rs
│
├─ DECISION: CacheService available / Redis connected?
│    ├─ NO → disabled/empty semantics or CacheError
│    └─ YES → ConnectionManager
├─ stats(): read Redis INFO/dbsize-like statistics
└─ clear_all(): KEYS "bc:*" → DECISION keys empty? → DEL matching keys
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/cache.rs
│    │    ├─ clear()
│    │    │    └─ CALL → ok() @ crates/server/src/api/response.rs
│    │    │    └─ CALL → err() @ crates/server/src/api/response.rs
│    ├─ FILE: crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::stats()
│    │    ├─ CacheService::clear_all()
│    └─ FILE: crates/server/src/api/response.rs
│    │    ├─ ok()
│    │    ├─ err()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "message": "Cache cleared"
}
```

### E2E FLOW #58 — GET /api/v1/usage

**树路径：** `HTTP / API → Billing / Usage → GET /api/v1/usage`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /api/v1/usage
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（其它顶层 route）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Explicit Data Plane route
│    ├─ Route match: GET /api/v1/usage
│    └─ handler = usage_handler()
│
├─ Credential extraction
│    ├─ read Authorization header
│    ├─ require Bearer token
│    └─ DECISION: Bearer credential present?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → extract_token_user(...)
│
├─ Multi-generation identity resolution
│    ├─ Try new Router token table
│    │    └─ validate_token_and_get_info(...)
│    ├─ DECISION: new token valid?
│    │    ├─ YES → resolve user_id
│    │    └─ NO  → legacy validation
│    ├─ validate_token_detailed(...)
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → resolve user_id
│    │    └─ NO  → JWT fallback
│    ├─ JWT decode / Claims.sub
│    └─ DECISION: any identity path resolved?
│         ├─ NO  → 401 / service-unavailable error branch → END
│         └─ YES → user_id
│
▼
FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│
├─ Usage aggregation query
│    ├─ CALL get_usage_stats(user_id, "month")
│    ├─ period = month
│    ├─ scope = resolved user_id
│    └─ DECISION: DB aggregation success?
│         ├─ NO  → HTTP 500 → END
│         └─ YES → usage rows / aggregate
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
▼
FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│
├─ get_usage_stats(...) / get_usage_stats_by_model(...)
└─ delegate to router log aggregation
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ build period boundary / aggregation SQL
├─ query router_logs for current user
└─ DECISION: SQL succeeds?
     ├─ NO → DatabaseError → handler error response
     └─ YES → UsageStats / Vec<ModelUsageStats>
│
├─ Response mapping
│    ├─ map DB aggregate → API response object
│    ├─ serialize JSON
│    └─ DECISION: serialization/build success?
│         ├─ NO  → internal response error branch
│         └─ YES → HTTP 200 application/json
│
├─ Side effects
│    ├─ no Provider call
│    ├─ no Scheduler
│    ├─ no inference billing deduction
│    └─ read-only usage query
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ usage_handler()
│    │    │    └─ CALL → extract_token_user() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → json_error_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ extract_token_user()
│    │    │    └─ CALL → build_response() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│    │    ├─ build_response()
│    │    ├─ json_error_body()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    └─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::validate_detailed()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives 当前用户月度总 usage
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "period": "month",
  "requests": 12842,
  "prompt_tokens": 18420560,
  "completion_tokens": 6912840,
  "total_tokens": 25333400,
  "cost": 126.67
}
```

### E2E FLOW #59 — GET /api/v1/usage/models

**树路径：** `HTTP / API → Billing / Usage → GET /api/v1/usage/models`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /api/v1/usage/models
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES（其它顶层 route）→ 对应 handler
│         └─ NO → fallback_service(router_app)
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Explicit Data Plane route
│    ├─ Route match: GET /api/v1/usage/models
│    └─ handler = usage_models_handler()
│
├─ Credential extraction
│    ├─ read Authorization header
│    ├─ require Bearer token
│    └─ DECISION: Bearer credential present?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → extract_token_user(...)
│
├─ Multi-generation identity resolution
│    ├─ Try new Router token table
│    │    └─ validate_token_and_get_info(...)
│    ├─ DECISION: new token valid?
│    │    ├─ YES → resolve user_id
│    │    └─ NO  → legacy validation
│    ├─ validate_token_detailed(...)
│    ├─ DECISION: legacy token valid?
│    │    ├─ YES → resolve user_id
│    │    └─ NO  → JWT fallback
│    ├─ JWT decode / Claims.sub
│    └─ DECISION: any identity path resolved?
│         ├─ NO  → 401 / service-unavailable error branch → END
│         └─ YES → user_id
│
▼
FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│
├─ Usage aggregation query
│    ├─ CALL get_usage_stats_by_model(user_id, "month")
│    ├─ period = month
│    ├─ scope = resolved user_id
│    └─ DECISION: DB aggregation success?
│         ├─ NO  → HTTP 500 → END
│         └─ YES → usage rows / aggregate
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
▼
FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│
├─ get_usage_stats(...) / get_usage_stats_by_model(...)
└─ delegate to router log aggregation
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ build period boundary / aggregation SQL
├─ query router_logs for current user
└─ DECISION: SQL succeeds?
     ├─ NO → DatabaseError → handler error response
     └─ YES → UsageStats / Vec<ModelUsageStats>
│
├─ Response mapping
│    ├─ map DB aggregate → API response object
│    ├─ serialize JSON
│    └─ DECISION: serialization/build success?
│         ├─ NO  → internal response error branch
│         └─ YES → HTTP 200 application/json
│
├─ Side effects
│    ├─ no Provider call
│    ├─ no Scheduler
│    ├─ no inference billing deduction
│    └─ read-only usage query
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ usage_models_handler()
│    │    │    └─ CALL → extract_token_user() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → json_error_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ extract_token_user()
│    │    │    └─ CALL → build_response() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│    │    ├─ build_response()
│    │    ├─ json_error_body()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    └─ FILE: crates/database/crates/router/src/token.rs
│    │    ├─ RouterTokenModel::validate_detailed()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Client receives 按模型聚合的月度 usage
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "period": "month",
  "models": [
    {
      "model": "gpt-5.4",
      "requests": 8021,
      "total_tokens": 16600420,
      "cost": 81.24
    },
    {
      "model": "claude-sonnet-4-5",
      "requests": 4821,
      "total_tokens": 8732980,
      "cost": 45.43
    }
  ]
}
```

### E2E FLOW #60 — GET /console/api/usage/&#123;user_id&#125;

**树路径：** `HTTP / API → Billing / Usage → GET /console/api/usage/{user_id}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/usage/{user_id}
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API / protected route candidate
│         └─ NO  → other top-level/fallback route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ protected_routes composition
│    ├─ route registered under Management API
│    └─ auth_middleware() wraps protected router
│
├─ JWT authentication
│    ├─ read Authorization header
│    └─ DECISION: Authorization starts with Bearer?
│         ├─ NO  → HTTP 401 → END
│         └─ YES → verify_jwt(...)
│
├─ DECISION: JWT signature/claims valid?
│    ├─ NO  → HTTP 401 → END
│    └─ YES
│         ├─ Claims inserted into request extensions
│         └─ continue to route handler
│
▼
FILE: crates/server/src/api/log.rs
│
├─ Handler
│    └─ get_user_usage()
│
├─ Request extraction
│    ├─ Path params / Query params / JSON body as required by Method
│    ├─ authenticated Claims available from extensions
│    └─ DECISION: extraction/required fields valid?
│         ├─ NO  → client/error response → END
│         └─ YES → authorization/business checks
│
├─ Authorization + invariants
│    ├─ Route uses authenticated Claims/user context as implemented
│    ├─ validate ID/status/range/reason/etc. according to handler
│    └─ DECISION: authorization/invariants pass?
│         ├─ NO  → 4xx/error payload → END
│         └─ YES → service/database call
│
├─ Service / Database boundary
│    ├─ operation type: read/query state
│    ├─ invoke route-specific Service / Database method
│    └─ DECISION: operation succeeds?
│         ├─ NO  → map error → HTTP error response
│         └─ YES → domain result
│
├─ State effects
│    ├─ READ routes: no intended mutation beyond incidental telemetry
│    ├─ WRITE routes: persist create/update/delete/config action
│    └─ route-specific async/internal calls execute before/around result when implemented
│
▼
FILE: crates/service/crates/router-log/src/lib.rs
│
└─ RouterLogService::get_usage_by_user(db, user_id)
│
▼
FILE: crates/database/crates/router/src/log.rs
│
├─ SUM prompt_tokens / completion_tokens for user_id
└─ return tuple → handler calculates total_tokens
│
├─ Response mapping
│    ├─ domain model → DTO/JSON
│    ├─ pagination/summary fields where applicable
│    └─ serialize success payload
│
├─ HTTP exit
│    └─ return success or mapped error status/body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/api/log.rs
│    │    ├─ get_user_usage()
│    │    │    └─ CALL → RouterLogService::get_usage_by_user() @ crates/service/crates/router-log/src/lib.rs
│    └─ FILE: crates/service/crates/router-log/src/lib.rs
│    │    ├─ RouterLogService::get_usage_by_user()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "user_id": 10001,
  "prompt_tokens": 18420560,
  "completion_tokens": 6912840,
  "total_tokens": 25333400
}
```

### E2E FLOW #61 — GET /health

**树路径：** `HTTP / API → Admin / Internal → GET /health`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /health
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → explicit GET /health handler
│         └─ NO  → continue route matching
│
├─ Liveness handler
│    ├─ no JWT middleware
│    ├─ no DB query in handler
│    ├─ no Router/Provider call
│    └─ return literal "ok"
│
├─ Response
│    └─ HTTP 200 text/plain-ish body
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/plain; charset=utf-8

ok
```

### E2E FLOW #62 — GET /console/internal/health

**树路径：** `HTTP / API → Admin / Internal → GET /console/internal/health`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/internal/health
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged Router Internal route
│         └─ NO  → continue to other routes/fallback
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Internal route match
│    └─ health_status_handler()
│
├─ Authentication boundary
│    ├─ Router internal_app itself is not wrapped by Management JWT middleware
│    └─ network exposure therefore depends on server deployment/binding/firewall
│
├─ Runtime-state operation
│    ├─ read/mutate in-memory Router runtime services
│    └─ DECISION: required runtime service/channel available?
│         ├─ NO  → route-specific 5xx/timeout/error response
│         └─ YES → perform operation
│
├─ Route-specific state
│    ├─ health: scheduler/circuit/channel/rate-budget snapshot
│    ├─ price sync: force_sync_tx + oneshot result
│    ├─ trip-all: circuit_breaker.trip_all()
│    └─ metrics: Router runtime counters
│
├─ Serialize result
│    └─ DECISION: operation/result successful?
│         ├─ NO  → error HTTP response
│         └─ YES → JSON success response
│
│
├─ 源码函数展开（静态扫描确认）
│    └─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ health_status_handler()
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "status": "ok",
  "scheduler": {
    "policy": "weighted"
  },
  "circuit_breaker": {
    "open": 0,
    "closed": 19
  },
  "channels": {
    "healthy": 19,
    "degraded": 1
  },
  "rate_budget": {
    "enabled": true
  }
}
```

### E2E FLOW #63 — POST /console/internal/prices/sync

**树路径：** `HTTP / API → Admin / Internal → POST /console/internal/prices/sync`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /console/internal/prices/sync
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged Router Internal route
│         └─ NO  → continue to other routes/fallback
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Internal route match
│    └─ price_sync_handler()
│
├─ Authentication boundary
│    ├─ Router internal_app itself is not wrapped by Management JWT middleware
│    └─ network exposure therefore depends on server deployment/binding/firewall
│
├─ Runtime-state operation
│    ├─ read/mutate in-memory Router runtime services
│    ├─ force-sync route sends channel message and awaits oneshot/timeout
│    └─ DECISION: required runtime service/channel available?
│         ├─ NO  → route-specific 5xx/timeout/error response
│         └─ YES → perform operation
│
├─ Route-specific state
│    ├─ health: scheduler/circuit/channel/rate-budget snapshot
│    ├─ price sync: force_sync_tx + oneshot result
│    ├─ trip-all: circuit_breaker.trip_all()
│    └─ metrics: Router runtime counters
│
├─ Serialize result
│    └─ DECISION: operation/result successful?
│         ├─ NO  → error HTTP response
│         └─ YES → JSON success response
│
│
├─ 源码函数展开（静态扫描确认）
│    └─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ price_sync_handler()
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "updated_models": 46,
  "duration_ms": 842
}
```

### E2E FLOW #64 — POST /console/internal/circuit-breaker/trip-all

**树路径：** `HTTP / API → Admin / Internal → POST /console/internal/circuit-breaker/trip-all`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: POST /console/internal/circuit-breaker/trip-all
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged Router Internal route
│         └─ NO  → continue to other routes/fallback
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Internal route match
│    └─ circuit_breaker_trip_all_handler()
│
├─ Authentication boundary
│    ├─ Router internal_app itself is not wrapped by Management JWT middleware
│    └─ network exposure therefore depends on server deployment/binding/firewall
│
├─ Runtime-state operation
│    ├─ read/mutate in-memory Router runtime services
│    └─ DECISION: required runtime service/channel available?
│         ├─ NO  → route-specific 5xx/timeout/error response
│         └─ YES → perform operation
│
├─ Route-specific state
│    ├─ health: scheduler/circuit/channel/rate-budget snapshot
│    ├─ price sync: force_sync_tx + oneshot result
│    ├─ trip-all: circuit_breaker.trip_all()
│    └─ metrics: Router runtime counters
│
├─ Serialize result
│    └─ DECISION: operation/result successful?
│         ├─ NO  → error HTTP response
│         └─ YES → JSON success response
│
│
├─ 源码函数展开（静态扫描确认）
│    └─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ circuit_breaker_trip_all_handler()
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ build_response_with_header()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "success": true,
  "tripped": 19
}
```

### E2E FLOW #65 — GET /console/internal/metrics

**树路径：** `HTTP / API → Admin / Internal → GET /console/internal/metrics`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/internal/metrics
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged Router Internal route
│         └─ NO  → continue to other routes/fallback
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ Internal route match
│    └─ metrics_handler()
│
├─ Authentication boundary
│    ├─ Router internal_app itself is not wrapped by Management JWT middleware
│    └─ network exposure therefore depends on server deployment/binding/firewall
│
├─ Runtime-state operation
│    ├─ read/mutate in-memory Router runtime services
│    └─ DECISION: required runtime service/channel available?
│         ├─ NO  → route-specific 5xx/timeout/error response
│         └─ YES → perform operation
│
├─ Route-specific state
│    ├─ health: scheduler/circuit/channel/rate-budget snapshot
│    ├─ price sync: force_sync_tx + oneshot result
│    ├─ trip-all: circuit_breaker.trip_all()
│    └─ metrics: Router runtime counters
│
├─ Serialize result
│    └─ DECISION: operation/result successful?
│         ├─ NO  → error HTTP response
│         └─ YES → JSON success response
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ metrics_handler()
│    │    │    └─ CALL → export() @ crates/router/src/metrics.rs
│    └─ FILE: crates/router/src/metrics.rs
│    │    ├─ export()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "requests_total": 3482211,
  "requests_inflight": 37,
  "upstream_failures_total": 14203,
  "rate_limited_total": 8231,
  "channels_healthy": 19
}
```

### E2E FLOW #66 — GET /console/api/&#123;*path&#125; → protected 404 catch-all

**树路径：** `HTTP / API → Admin / Internal → GET /console/api/{*path} → protected 404 catch-all`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/api/{*path} → protected 404 catch-all
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → Management API protected router
│         └─ NO → other router
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ JWT middleware executes before protected catch-all
│    ├─ DECISION: Bearer JWT valid?
│    │    ├─ NO → HTTP 401 → END
│    │    └─ YES → Claims inserted
│    └─ continue route matching
│
├─ Concrete /console/api/* routes checked
│    └─ DECISION: any concrete protected route matched?
│         ├─ YES → that handler executes
│         └─ NO → api_not_found()
│
├─ Catch-all purpose
│    ├─ prevents unknown API path from being served as LiveView HTML
│    └─ returns explicit API 404
│
│
├─ 源码函数展开（静态扫描确认）
│    └─ FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│    │    ├─ api_not_found()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ HTTP 404 "API endpoint not found"
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 404 Not Found
Content-Type: application/json

{
  "error": "API endpoint not found"
}
```

### E2E FLOW #67 — GET /api-docs/openapi.json

**树路径：** `HTTP / API → OpenAPI / Swagger → GET /api-docs/openapi.json`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /api-docs/openapi.json
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → api::routes()
│         └─ NO → other route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ OpenAPI routes are currently inside protected_routes
│    ├─ auth_middleware()
│    └─ DECISION: JWT valid?
│         ├─ NO → HTTP 401 → END
│         └─ YES → route handler
│
▼
FILE: crates/server/src/api/openapi.rs
│
├─ Handler
│    └─ openapi_json()
│
├─ Content construction
│    └─ DECISION: endpoint type?
│         ├─ /api-docs/openapi.json
│         │    ├─ construct OpenAPI 3.0.3 object
│         │    ├─ include documented paths/schemas
│         │    └─ serialize JSON
│         └─ /swagger-ui[/]
│              ├─ construct embedded Swagger HTML shell
│              └─ browser later loads Swagger UI assets
│
├─ Response
│    ├─ JSON endpoint → application/json
│    └─ UI endpoint → text/html
│
│
├─ 源码函数展开（静态扫描确认）
│    └─ FILE: crates/server/src/api/openapi.rs
│    │    ├─ openapi_json()
│    │    │    └─ CALL → generate_openapi_spec() @ crates/server/src/api/openapi.rs
│    │    ├─ generate_openapi_spec()
│    │    │    └─ CALL → get_version() @ crates/server/src/api/openapi.rs
│    │    │    └─ CALL → generate_paths() @ crates/server/src/api/openapi.rs
│    │    │    └─ CALL → generate_schemas() @ crates/server/src/api/openapi.rs
│    │    ├─ get_version()
│    │    ├─ generate_paths()
│    │    ├─ generate_schemas()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: application/json

{
  "openapi": "3.0.3",
  "info": {
    "title": "BurnCloud API",
    "version": "1.0.0"
  },
  "paths": {
    "/api/auth/login": {
      "post": {
        "summary": "Login"
      }
    },
    "/console/api/channel": {
      "get": {
        "summary": "List channels"
      }
    }
  }
}
```

### E2E FLOW #68 — GET /swagger-ui

**树路径：** `HTTP / API → OpenAPI / Swagger → GET /swagger-ui`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /swagger-ui
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → api::routes()
│         └─ NO → other route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ OpenAPI routes are currently inside protected_routes
│    ├─ auth_middleware()
│    └─ DECISION: JWT valid?
│         ├─ NO → HTTP 401 → END
│         └─ YES → route handler
│
▼
FILE: crates/server/src/api/openapi.rs
│
├─ Handler
│    └─ swagger_ui()
│
├─ Content construction
│    └─ DECISION: endpoint type?
│         ├─ /api-docs/openapi.json
│         │    ├─ construct OpenAPI 3.0.3 object
│         │    ├─ include documented paths/schemas
│         │    └─ serialize JSON
│         └─ /swagger-ui[/]
│              ├─ construct embedded Swagger HTML shell
│              └─ browser later loads Swagger UI assets
│
├─ Response
│    ├─ JSON endpoint → application/json
│    └─ UI endpoint → text/html
│
│
├─ 源码函数展开（静态扫描确认）
│    └─ FILE: crates/server/src/api/openapi.rs
│    │    ├─ swagger_ui()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html>
  <head><title>BurnCloud Swagger UI</title></head>
  <body><div id="swagger-ui"></div></body>
</html>
```

### E2E FLOW #69 — GET /swagger-ui/

**树路径：** `HTTP / API → OpenAPI / Swagger → GET /swagger-ui/`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /swagger-ui/
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → api::routes()
│         └─ NO → other route
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ OpenAPI routes are currently inside protected_routes
│    ├─ auth_middleware()
│    └─ DECISION: JWT valid?
│         ├─ NO → HTTP 401 → END
│         └─ YES → route handler
│
▼
FILE: crates/server/src/api/openapi.rs
│
├─ Handler
│    └─ swagger_ui()
│
├─ Content construction
│    └─ DECISION: endpoint type?
│         ├─ /api-docs/openapi.json
│         │    ├─ construct OpenAPI 3.0.3 object
│         │    ├─ include documented paths/schemas
│         │    └─ serialize JSON
│         └─ /swagger-ui[/]
│              ├─ construct embedded Swagger HTML shell
│              └─ browser later loads Swagger UI assets
│
├─ Response
│    ├─ JSON endpoint → application/json
│    └─ UI endpoint → text/html
│
│
├─ 源码函数展开（静态扫描确认）
│    └─ FILE: crates/server/src/api/openapi.rs
│    │    ├─ swagger_ui()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html>
  <head><title>BurnCloud Swagger UI</title></head>
  <body><div id="swagger-ui"></div></body>
</html>
```

### E2E FLOW #70 — GET /

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #71 — GET /home

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /home`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /home
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #72 — GET /login

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /login`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /login
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #73 — GET /register

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /register`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /register
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #74 — GET /forgot-password

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /forgot-password`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /forgot-password
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #75 — GET /reset-password

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /reset-password`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /reset-password
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #76 — GET /console

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /console`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #77 — GET /console/

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /console/`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #78 — GET /console/&#123;*path&#125;

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /console/{*path}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /console/{*path}
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #79 — GET /favicon.ico

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /favicon.ico`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /favicon.ico
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: image/x-icon
Content-Length: 5430

<binary favicon bytes>
```

### E2E FLOW #80 — GET /preview/home

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /preview/home`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /preview/home
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #81 — GET /preview/login

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /preview/login`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /preview/login
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #82 — GET /preview/console

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /preview/console`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /preview/console
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #83 — GET /preview/console/

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /preview/console/`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /preview/console/
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #84 — GET /preview/console/&#123;*path&#125;

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /preview/console/{*path}`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /preview/console/{*path}
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView/static route candidate
│         └─ NO → other route/fallback
│
├─ LiveView feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → LiveView route unavailable; routing continues/falls back
│         └─ YES → match LiveView Router
│
▼
FILE: crates/client/src/lib.rs
│
├─ HTTP shell/static handler
│    ├─ route shell / preview / favicon according to path
│    └─ DECISION: requested LiveView/static route recognized?
│         ├─ NO → route miss
│         └─ YES → construct HTML/static response
│
▼
FILE: crates/client/src/app.rs
│
├─ Client route model
│    ├─ Dioxus Route enum represents browser-side view
│    ├─ App contexts: auth/theme/i18n/toast
│    └─ page-specific component selected after client session establishes
│
├─ Follow-up interactive transport
│    ├─ initial HTTP response delivers shell
│    └─ interactive events move to /ws LiveView session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ Browser receives HTML/static shell; UI lifecycle continues separately
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 200 OK
Content-Type: text/html; charset=utf-8

<!doctype html>
<html lang="zh-CN">
  <head><meta charset="utf-8"><title>BurnCloud</title></head>
  <body><div id="main">Dioxus LiveView shell</div></body>
</html>
```

### E2E FLOW #85 — GET /ws

**树路径：** `HTTP / API → Web UI / LiveView / WebSocket → GET /ws`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ 调用方与输入边界
│    ├─ Actor: User / SDK / Browser / Operator
│    ├─ Entry: GET /ws
│    ├─ Input sources
│    │    ├─ Method + URI path
│    │    ├─ Query string（如有）
│    │    ├─ HTTP headers
│    │    └─ Request body（如有）
│    └─ DECISION: TCP/HTTP 请求能否到达 BurnCloud listener?
│         ├─ NO  → 网络层失败；应用代码未执行 → END
│         └─ YES → 进入 Axum
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ 统一 HTTP Server
│    ├─ start_server() 已在进程启动时完成
│    │    ├─ database 初始化
│    │    ├─ RouterDatabase::init()
│    │    ├─ UserDatabase::init()
│    │    ├─ create_app(...)
│    │    ├─ TcpListener::bind(...)
│    │    └─ axum::serve(listener, app)
│    ├─ 当前请求进入 Unified Axum App
│    └─ 全局 middleware
│         ├─ CORS
│         ├─ TraceLayer
│         ├─ SetRequestIdLayer
│         └─ PropagateRequestIdLayer
│
├─ 顶层 Route 决策
│    └─ DECISION: Unified App 是否已有显式/合并路由命中当前 Method + Path?
│         ├─ YES → merged LiveView router
│         └─ NO → fallback routing
│
├─ Feature gate
│    └─ DECISION: enable_liveview == true?
│         ├─ NO → /ws not served by LiveView path
│         └─ YES → websocket route available
│
▼
FILE: crates/client/src/lib.rs
│
├─ WebSocket upgrade
│    ├─ validate Upgrade/Connection/Sec-WebSocket-* headers
│    └─ DECISION: WebSocket handshake valid?
│         ├─ NO → HTTP handshake error → END
│         └─ YES → 101 Switching Protocols
│
├─ LiveView session
│    ├─ create/attach Dioxus LiveView session
│    ├─ receive browser UI events
│    ├─ update Virtual DOM/server-side UI state
│    └─ send render patches/messages to browser
│
├─ Connection loop
│    └─ DECISION: socket still connected?
│         ├─ YES → await next UI message → repeat
│         └─ NO → cleanup session
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/client/src/app.rs
│    │    ├─ App()
│    │    │    └─ CALL → use_init_i18n() @ crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ launch_gui_with_tray()
│    └─ FILE: crates/client/crates/client-shared/src/i18n.rs
│    │    ├─ use_init_i18n()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```http
HTTP/1.1 101 Switching Protocols
Upgrade: websocket
Connection: Upgrade
Sec-WebSocket-Accept: <computed-value>

# 随后进入双向 WebSocket / LiveView 消息通道
```

### E2E FLOW #86 — burncloud

**树路径：** `CLI / Executables → burncloud → burncloud`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`

```text
START
│
├─ Shell / OS: burncloud（无参数）
│
▼
FILE: src/main.rs
│
├─ main()
│    ├─ dotenvy::dotenv()
│    ├─ ensure_master_key()
│    ├─ init_logging()
│    └─ match args == [binary]
│
├─ DECISION: target_os == windows?
│    ├─ YES
│    │    ├─ std::thread::spawn(background server thread)
│    │    │    ├─ tokio::runtime::Runtime::new()
│    │    │    ├─ HOST / PORT
│    │    │    └─ burncloud_server::start_server(host, port, false)
│    │    │         └─ enable_liveview=false：后台 Server 不挂 LiveView Router
│    │    └─ main thread → burncloud_client::launch_gui_with_tray()
│    │
│    └─ NO（Linux/macOS 等）
│         ├─ print Headless Mode startup line
│         └─ run_async_server()
│              └─ burncloud_server::start_server(host, port, true)
│
├─ Windows server thread 后续主链与 `burncloud server` 的 start_server/create_app/create_router_app 相同
├─ Non-Windows 后续主链与 `burncloud server` 完全相同
│
▼
FILE: crates/client/src/app.rs
│
├─ Windows GUI branch: launch_gui_with_tray()
│    ├─ WindowBuilder
│    ├─ Config::new().with_window(...).with_data_directory(...)
│    └─ LaunchBuilder::desktop().launch(AppWithTray)
│
├─ AppWithTray()
│    ├─ DesktopMode context
│    ├─ maximize window
│    ├─ std::thread::spawn(start_tray)
│    ├─ spawn async show-window poll loop
│    └─ render App()
│
└─ App()
     ├─ use_init_i18n()
     ├─ use_init_toast()
     ├─ use_init_auth()
     ├─ use_init_theme()
     └─ Router<Route>
│
▼
END / LONG-RUNNING SERVER + UI LOOPS
```

#### PR Before / After（patch 证据）

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
# 非 Windows
Starting BurnCloud Server with LiveView (Headless Mode)...
Unified Gateway listening on 127.0.0.1:3000
- Dashboard: http://127.0.0.1:3000/
- LLM API:   http://127.0.0.1:3000/v1/...

# Windows
background_server_thread=running (enable_liveview=false)
desktop_gui_event_loop=running
system_tray_thread=running
```

### E2E FLOW #87 — server

**树路径：** `CLI / Executables → burncloud → server`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ Shell / OS 入口
│    └─ burncloud server
│    └─ 显式 Server 模式；当前实现同时 enable_liveview=true
│
▼
FILE: src/main.rs
│
├─ main()
│    ├─ dotenvy::dotenv().ok()
│    ├─ ensure_master_key()
│    │    ├─ is_valid_master_key()
│    │    │    ├─ env::var("MASTER_KEY")
│    │    │    ├─ hex::decode(...)
│    │    │    └─ DECISION: decoded key length == 32 bytes?
│    │    │         ├─ YES → keep existing key
│    │    │         └─ NO  → generate replacement
│    │    ├─ rand::random::<[u8;32]>()
│    │    ├─ hex::encode()
│    │    ├─ resolve .env path from CWD / current_exe()
│    │    ├─ read_to_string() when file exists
│    │    ├─ fs::write(.env)
│    │    └─ env::set_var("MASTER_KEY", ...)
│    ├─ env::args().collect()
│    ├─ is_server = args[1] in [server, router, client]
│    └─ burncloud_server::logging::init_logging()
│
▼
FILE: crates/server/src/logging.rs
│
├─ init_logging()
│    ├─ LOG_DIR / LOG_MAX_FILES / RUST_LOG
│    ├─ fs::create_dir_all(log_dir)
│    ├─ tracing_log::LogTracer::init()
│    ├─ file_appender(server/service/database/router)
│    │    └─ RollingFileAppender::builder() → non_blocking(...)
│    ├─ module_filter(...)
│    └─ tracing::subscriber::set_global_default(...)
│
▼
FILE: src/main.rs
│
├─ match args.as_slice()
│    └─ DECISION: subcommand == "server" OR "router"?
│         ├─ NO  → other direct/CLI branch
│         └─ YES → run_async_server()
│
├─ run_async_server() #[tokio::main]
│    ├─ HOST env or "127.0.0.1"
│    ├─ PORT env or DEFAULT_PORT
│    └─ burncloud_server::start_server(host, port, true)
│
├─ IMPORTANT
│    ├─ 当前源码中 `server` 与 `router` 都走同一个 run_async_server()
│    └─ 此 direct branch 不进入 Clap CLI dispatch
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server(host, port, enable_liveview=true)
│    ├─ create_default_database().await
│    ├─ RouterDatabase::init(&db).await
│    ├─ UserDatabase::init(&db).await
│    ├─ create_app(db, true).await
│    ├─ SocketAddr::parse()
│    ├─ TcpListener::bind(addr).await
│    └─ axum::serve(listener, app).await
│
▼
FILE: crates/database/src/database.rs
│
├─ create_default_database() → Database::new()
├─ Database::new()
│    ├─ DECISION: BURNCLOUD_DATABASE_URL exists?
│    │    ├─ YES → use configured URL
│    │    └─ NO
│    │         ├─ get_default_database_path()
│    │         ├─ create_directory_if_not_exists()
│    │         ├─ DECISION: BURNCLOUD_FRESH_DB == "1" and DB exists?
│    │         │    ├─ YES → fs::remove_file(default DB)
│    │         │    └─ NO  → preserve DB
│    │         └─ build sqlite://...?...mode=rwc
│    └─ Database::initialize().await
│
├─ Database::initialize()
│    ├─ sqlx::any::install_default_drivers()
│    ├─ DatabaseConnection::new(database_url)
│    │    ├─ AnyConnectOptions::from_str()
│    │    └─ AnyPoolOptions::new().max_connections(10).connect_with(...)
│    ├─ DECISION: db.kind() == sqlite?
│    │    └─ YES → PRAGMA journal_mode=WAL
│    ├─ MigrationRunner::run(self)
│    └─ Schema::init(self)
│
▼
FILE: crates/database/src/migration/mod.rs
│
├─ MigrationRunner::run()
│    ├─ ensure _schema_migrations table
│    ├─ choose SQLite / PostgreSQL migration catalogue
│    ├─ inspect already-applied versions
│    └─ execute pending versioned DDL migrations
│
▼
FILE: crates/database/src/schema/mod.rs
│
├─ Schema::init()
│    ├─ rename::migrate_table_renames()
│    ├─ router::migrate_router_logs()
│    ├─ price::migrate_prices()
│    └─ user::migrate_users_and_seed()
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
└─ start_server() continues → RouterDatabase::init(&db)
│
▼
FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│
├─ RouterDatabase::init()
│    ├─ db.get_connection()
│    ├─ db.kind()
│    ├─ CREATE TABLE IF NOT EXISTS router_tokens
│    └─ SQLite-only compatibility ALTER TABLE statements
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
└─ start_server() continues → UserDatabase::init(&db)
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase::init()
│    ├─ CREATE / verify user_roles
│    ├─ CREATE / verify user_role_bindings
│    ├─ CREATE / verify user_recharges
│    ├─ SQLite/PostgreSQL compatibility migrations
│    ├─ SELECT COUNT(*) FROM user_roles
│    ├─ DECISION: no roles exist?
│    │    └─ YES → seed admin/user roles
│    ├─ query users without role bindings
│    └─ assign_role(...) for orphan users when required
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ create_app(db, enable_liveview=true)
│    ├─ SystemMonitorService::new()
│    ├─ monitor.start_auto_update().await
│    ├─ CacheService::new().await
│    ├─ cache.is_available().await
│    ├─ create_router_app(db.clone()).await
│    ├─ UserService::new()
│    ├─ api::routes(state.clone())
│    ├─ build /health + merge Management/Internal routers
│    ├─ DECISION: enable_liveview?
│    │    └─ YES → burncloud_client::liveview_router(db.clone()) → merge
│    ├─ fallback_service(router_app)
│    └─ SetRequestId / PropagateRequestId / Trace / CORS layers
│
▼
FILE: crates/service/crates/monitor/src/service.rs
│
├─ SystemMonitorService::new()
├─ start_auto_update()
│    └─ tokio::spawn
│         └─ LOOP interval(1s)
│              └─ collect_metrics_internal()
│                   ├─ CpuCollector::collect()
│                   ├─ MemoryCollector::collect()
│                   └─ DiskCollector::collect_all()
│
▼
FILE: crates/service/crates/cache/src/service.rs
│
├─ CacheService::new() → with_config(CacheConfig::default())
├─ DECISION: CACHE_ENABLED?
│    ├─ NO  → return disabled cache
│    └─ YES
│         ├─ require REDIS_URL
│         ├─ redis::Client::open()
│         ├─ get_connection_manager().await
│         └─ PING
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ create_router_app(db)
│    ├─ reqwest::Client::builder() + timeouts/pool settings
│    ├─ RoundRobinBalancer::new()
│    ├─ RateLimiter::new(100, 10)
│    ├─ CircuitBreaker::new(...)
│    ├─ ModelRouter::new(db.clone())
│    ├─ ChannelStateTracker::new()
│    ├─ DynamicAdaptorFactory::new(db.clone())
│    ├─ ApiVersionDetector::new(db.clone())
│    ├─ PriceCache::load(&db).await
│    │    └─ DECISION: load fails? → PriceCache::empty()
│    ├─ CostCalculator::new(price_cache.clone())
│    ├─ ExchangeRateService::new(db.clone())
│    ├─ load_rates_from_db().await
│    ├─ tokio::spawn → ExchangeRateService::start_sync_task()
│    ├─ scheduler::load_scheduler_config()
│    ├─ AffinityCache::default()
│    ├─ InMemoryBudget::new()
│    ├─ configure_rate_budget_from_db(...).await
│    ├─ read BILLING_STRICT_MODE
│    ├─ read REQUEST_LOG_STORAGE_POLICY
│    ├─ tokio::spawn → AIMD budget-update consumer
│    ├─ price_sync::start_price_sync_task(...)
│    ├─ tokio::spawn → RouterLog writer → RouterDatabase::insert_log()
│    ├─ tokio::spawn → RequestLog writer → RouterDatabase::insert_request_log()
│    ├─ ChannelHealthManager::new()
│    ├─ build internal_app routes
│    │    ├─ /console/internal/health
│    │    ├─ /console/internal/prices/sync
│    │    ├─ /console/internal/circuit-breaker/trip-all
│    │    └─ /console/internal/metrics
│    └─ build data-plane app
│         ├─ GET /v1/models → models_handler [REGISTER ONLY]
│         ├─ GET /api/v1/usage → usage_handler [REGISTER ONLY]
│         ├─ GET /api/v1/usage/models → usage_models_handler [REGISTER ONLY]
│         └─ fallback(proxy_handler) [REGISTER ONLY]
│
├─ IMPORTANT: 上述 Handler 在启动时只被注册；收到对应 HTTP 请求后才执行
│
▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache::load(db)
│    ├─ PriceCache::empty()
│    └─ refresh(db)
│         └─ BillingPriceModel::list(...)
│
▼
FILE: crates/router/src/exchange_rate.rs
│
├─ ExchangeRateService::new()
├─ load_rates_from_db()
└─ start_sync_task()
     └─ tokio::spawn hourly loop
│
▼
FILE: crates/router/src/price_sync.rs
│
├─ start_price_sync_task(...)
│    └─ tokio::spawn
│         ├─ PriceSyncService::new()/with_config()
│         ├─ startup sync_all(false) unless SKIP_INITIAL_PRICE_SYNC
│         └─ LOOP select(periodic tick, force-sync receiver)
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ api::routes(state)
│    ├─ auth::public_routes() [REGISTER]
│    ├─ auth::protected_routes() [REGISTER]
│    ├─ billing::routes() [REGISTER]
│    ├─ channel::routes() [REGISTER]
│    ├─ token::routes() [REGISTER]
│    ├─ log::routes() [REGISTER]
│    ├─ monitor::routes() [REGISTER]
│    ├─ user::routes() [REGISTER]
│    ├─ security::security_routes() [REGISTER]
│    ├─ openapi::routes() [REGISTER]
│    ├─ cache::routes() [REGISTER]
│    └─ auth_middleware layered over protected router [REGISTER]
│
├─ IMPORTANT: routes() 函数在启动期执行；具体 HTTP Handler 此刻不执行
│
▼
FILE: crates/client/src/lib.rs
│
├─ liveview_router(db)
│    ├─ LiveViewPool::new()
│    ├─ register HTML shell routes /, /home, /login, /register, ...
│    ├─ register /console and /console/*
│    ├─ register /favicon.ico
│    └─ register WS_PATH
│         └─ app::App is passed as future WebSocket-session callback [REGISTER ONLY]
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ create_app() returns Unified Router
├─ SocketAddr::parse(host:port)
├─ TcpListener::bind(addr).await
├─ axum::serve(listener, app).await
│
└─ LONG-RUNNING STATE
     ├─ HTTP listener waits for Management / Internal / Data Plane / LiveView traffic
     ├─ monitor updater keeps running
     ├─ exchange-rate task keeps running
     ├─ price-sync task keeps running
     ├─ AIMD feedback consumer keeps running
     ├─ RouterLog writer keeps running
     └─ RequestLog writer keeps running
│
▼
END = server shutdown or fatal serve error
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
Unified Gateway listening on 127.0.0.1:3000
- Dashboard: http://127.0.0.1:3000/
- LLM API:   http://127.0.0.1:3000/v1/...

# 随后 axum::serve 持续运行，命令不会立即返回到 Shell。
```

### E2E FLOW #88 — router

**树路径：** `CLI / Executables → burncloud → router`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ Shell / OS 入口
│    └─ burncloud router
│    └─ 源码命令名为 router，但当前实现与 server 一样直接进入 run_async_server()
│
▼
FILE: src/main.rs
│
├─ main()
│    ├─ dotenvy::dotenv().ok()
│    ├─ ensure_master_key()
│    │    ├─ is_valid_master_key()
│    │    │    ├─ env::var("MASTER_KEY")
│    │    │    ├─ hex::decode(...)
│    │    │    └─ DECISION: decoded key length == 32 bytes?
│    │    │         ├─ YES → keep existing key
│    │    │         └─ NO  → generate replacement
│    │    ├─ rand::random::<[u8;32]>()
│    │    ├─ hex::encode()
│    │    ├─ resolve .env path from CWD / current_exe()
│    │    ├─ read_to_string() when file exists
│    │    ├─ fs::write(.env)
│    │    └─ env::set_var("MASTER_KEY", ...)
│    ├─ env::args().collect()
│    ├─ is_server = args[1] in [server, router, client]
│    └─ burncloud_server::logging::init_logging()
│
▼
FILE: crates/server/src/logging.rs
│
├─ init_logging()
│    ├─ LOG_DIR / LOG_MAX_FILES / RUST_LOG
│    ├─ fs::create_dir_all(log_dir)
│    ├─ tracing_log::LogTracer::init()
│    ├─ file_appender(server/service/database/router)
│    │    └─ RollingFileAppender::builder() → non_blocking(...)
│    ├─ module_filter(...)
│    └─ tracing::subscriber::set_global_default(...)
│
▼
FILE: src/main.rs
│
├─ match args.as_slice()
│    └─ DECISION: subcommand == "server" OR "router"?
│         ├─ NO  → other direct/CLI branch
│         └─ YES → run_async_server()
│
├─ run_async_server() #[tokio::main]
│    ├─ HOST env or "127.0.0.1"
│    ├─ PORT env or DEFAULT_PORT
│    └─ burncloud_server::start_server(host, port, true)
│
├─ IMPORTANT
│    ├─ 当前源码中 `server` 与 `router` 都走同一个 run_async_server()
│    └─ 此 direct branch 不进入 Clap CLI dispatch
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server(host, port, enable_liveview=true)
│    ├─ create_default_database().await
│    ├─ RouterDatabase::init(&db).await
│    ├─ UserDatabase::init(&db).await
│    ├─ create_app(db, true).await
│    ├─ SocketAddr::parse()
│    ├─ TcpListener::bind(addr).await
│    └─ axum::serve(listener, app).await
│
▼
FILE: crates/database/src/database.rs
│
├─ create_default_database() → Database::new()
├─ Database::new()
│    ├─ DECISION: BURNCLOUD_DATABASE_URL exists?
│    │    ├─ YES → use configured URL
│    │    └─ NO
│    │         ├─ get_default_database_path()
│    │         ├─ create_directory_if_not_exists()
│    │         ├─ DECISION: BURNCLOUD_FRESH_DB == "1" and DB exists?
│    │         │    ├─ YES → fs::remove_file(default DB)
│    │         │    └─ NO  → preserve DB
│    │         └─ build sqlite://...?...mode=rwc
│    └─ Database::initialize().await
│
├─ Database::initialize()
│    ├─ sqlx::any::install_default_drivers()
│    ├─ DatabaseConnection::new(database_url)
│    │    ├─ AnyConnectOptions::from_str()
│    │    └─ AnyPoolOptions::new().max_connections(10).connect_with(...)
│    ├─ DECISION: db.kind() == sqlite?
│    │    └─ YES → PRAGMA journal_mode=WAL
│    ├─ MigrationRunner::run(self)
│    └─ Schema::init(self)
│
▼
FILE: crates/database/src/migration/mod.rs
│
├─ MigrationRunner::run()
│    ├─ ensure _schema_migrations table
│    ├─ choose SQLite / PostgreSQL migration catalogue
│    ├─ inspect already-applied versions
│    └─ execute pending versioned DDL migrations
│
▼
FILE: crates/database/src/schema/mod.rs
│
├─ Schema::init()
│    ├─ rename::migrate_table_renames()
│    ├─ router::migrate_router_logs()
│    ├─ price::migrate_prices()
│    └─ user::migrate_users_and_seed()
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
└─ start_server() continues → RouterDatabase::init(&db)
│
▼
FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│
├─ RouterDatabase::init()
│    ├─ db.get_connection()
│    ├─ db.kind()
│    ├─ CREATE TABLE IF NOT EXISTS router_tokens
│    └─ SQLite-only compatibility ALTER TABLE statements
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
└─ start_server() continues → UserDatabase::init(&db)
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
├─ UserDatabase::init()
│    ├─ CREATE / verify user_roles
│    ├─ CREATE / verify user_role_bindings
│    ├─ CREATE / verify user_recharges
│    ├─ SQLite/PostgreSQL compatibility migrations
│    ├─ SELECT COUNT(*) FROM user_roles
│    ├─ DECISION: no roles exist?
│    │    └─ YES → seed admin/user roles
│    ├─ query users without role bindings
│    └─ assign_role(...) for orphan users when required
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ create_app(db, enable_liveview=true)
│    ├─ SystemMonitorService::new()
│    ├─ monitor.start_auto_update().await
│    ├─ CacheService::new().await
│    ├─ cache.is_available().await
│    ├─ create_router_app(db.clone()).await
│    ├─ UserService::new()
│    ├─ api::routes(state.clone())
│    ├─ build /health + merge Management/Internal routers
│    ├─ DECISION: enable_liveview?
│    │    └─ YES → burncloud_client::liveview_router(db.clone()) → merge
│    ├─ fallback_service(router_app)
│    └─ SetRequestId / PropagateRequestId / Trace / CORS layers
│
▼
FILE: crates/service/crates/monitor/src/service.rs
│
├─ SystemMonitorService::new()
├─ start_auto_update()
│    └─ tokio::spawn
│         └─ LOOP interval(1s)
│              └─ collect_metrics_internal()
│                   ├─ CpuCollector::collect()
│                   ├─ MemoryCollector::collect()
│                   └─ DiskCollector::collect_all()
│
▼
FILE: crates/service/crates/cache/src/service.rs
│
├─ CacheService::new() → with_config(CacheConfig::default())
├─ DECISION: CACHE_ENABLED?
│    ├─ NO  → return disabled cache
│    └─ YES
│         ├─ require REDIS_URL
│         ├─ redis::Client::open()
│         ├─ get_connection_manager().await
│         └─ PING
│
▼
FILE: crates/router/src/lib.rs  [PR CHANGED]
│
├─ create_router_app(db)
│    ├─ reqwest::Client::builder() + timeouts/pool settings
│    ├─ RoundRobinBalancer::new()
│    ├─ RateLimiter::new(100, 10)
│    ├─ CircuitBreaker::new(...)
│    ├─ ModelRouter::new(db.clone())
│    ├─ ChannelStateTracker::new()
│    ├─ DynamicAdaptorFactory::new(db.clone())
│    ├─ ApiVersionDetector::new(db.clone())
│    ├─ PriceCache::load(&db).await
│    │    └─ DECISION: load fails? → PriceCache::empty()
│    ├─ CostCalculator::new(price_cache.clone())
│    ├─ ExchangeRateService::new(db.clone())
│    ├─ load_rates_from_db().await
│    ├─ tokio::spawn → ExchangeRateService::start_sync_task()
│    ├─ scheduler::load_scheduler_config()
│    ├─ AffinityCache::default()
│    ├─ InMemoryBudget::new()
│    ├─ configure_rate_budget_from_db(...).await
│    ├─ read BILLING_STRICT_MODE
│    ├─ read REQUEST_LOG_STORAGE_POLICY
│    ├─ tokio::spawn → AIMD budget-update consumer
│    ├─ price_sync::start_price_sync_task(...)
│    ├─ tokio::spawn → RouterLog writer → RouterDatabase::insert_log()
│    ├─ tokio::spawn → RequestLog writer → RouterDatabase::insert_request_log()
│    ├─ ChannelHealthManager::new()
│    ├─ build internal_app routes
│    │    ├─ /console/internal/health
│    │    ├─ /console/internal/prices/sync
│    │    ├─ /console/internal/circuit-breaker/trip-all
│    │    └─ /console/internal/metrics
│    └─ build data-plane app
│         ├─ GET /v1/models → models_handler [REGISTER ONLY]
│         ├─ GET /api/v1/usage → usage_handler [REGISTER ONLY]
│         ├─ GET /api/v1/usage/models → usage_models_handler [REGISTER ONLY]
│         └─ fallback(proxy_handler) [REGISTER ONLY]
│
├─ IMPORTANT: 上述 Handler 在启动时只被注册；收到对应 HTTP 请求后才执行
│
▼
FILE: crates/service/crates/billing/src/cache.rs
│
├─ PriceCache::load(db)
│    ├─ PriceCache::empty()
│    └─ refresh(db)
│         └─ BillingPriceModel::list(...)
│
▼
FILE: crates/router/src/exchange_rate.rs
│
├─ ExchangeRateService::new()
├─ load_rates_from_db()
└─ start_sync_task()
     └─ tokio::spawn hourly loop
│
▼
FILE: crates/router/src/price_sync.rs
│
├─ start_price_sync_task(...)
│    └─ tokio::spawn
│         ├─ PriceSyncService::new()/with_config()
│         ├─ startup sync_all(false) unless SKIP_INITIAL_PRICE_SYNC
│         └─ LOOP select(periodic tick, force-sync receiver)
│
▼
FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│
├─ api::routes(state)
│    ├─ auth::public_routes() [REGISTER]
│    ├─ auth::protected_routes() [REGISTER]
│    ├─ billing::routes() [REGISTER]
│    ├─ channel::routes() [REGISTER]
│    ├─ token::routes() [REGISTER]
│    ├─ log::routes() [REGISTER]
│    ├─ monitor::routes() [REGISTER]
│    ├─ user::routes() [REGISTER]
│    ├─ security::security_routes() [REGISTER]
│    ├─ openapi::routes() [REGISTER]
│    ├─ cache::routes() [REGISTER]
│    └─ auth_middleware layered over protected router [REGISTER]
│
├─ IMPORTANT: routes() 函数在启动期执行；具体 HTTP Handler 此刻不执行
│
▼
FILE: crates/client/src/lib.rs
│
├─ liveview_router(db)
│    ├─ LiveViewPool::new()
│    ├─ register HTML shell routes /, /home, /login, /register, ...
│    ├─ register /console and /console/*
│    ├─ register /favicon.ico
│    └─ register WS_PATH
│         └─ app::App is passed as future WebSocket-session callback [REGISTER ONLY]
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ create_app() returns Unified Router
├─ SocketAddr::parse(host:port)
├─ TcpListener::bind(addr).await
├─ axum::serve(listener, app).await
│
└─ LONG-RUNNING STATE
     ├─ HTTP listener waits for Management / Internal / Data Plane / LiveView traffic
     ├─ monitor updater keeps running
     ├─ exchange-rate task keeps running
     ├─ price-sync task keeps running
     ├─ AIMD feedback consumer keeps running
     ├─ RouterLog writer keeps running
     └─ RequestLog writer keeps running
│
▼
END = server shutdown or fatal serve error
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
Unified Gateway listening on 127.0.0.1:3000
- Dashboard: http://127.0.0.1:3000/
- LLM API:   http://127.0.0.1:3000/v1/...

# 随后 axum::serve 持续运行，命令不会立即返回到 Shell。
```

### E2E FLOW #89 — burncloud user register

**树路径：** `CLI / Executables → burncloud → burncloud user register`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ Shell input
│    ├─ command: burncloud user register
│    ├─ argv tokenization done by shell
│    └─ process environment / cwd available
│
▼
FILE: src/main.rs
│
├─ Process bootstrap
│    ├─ dotenv load
│    ├─ ensure/generate MASTER_KEY
│    ├─ initialize logging
│    └─ inspect argv
│
├─ Top-level dispatch
│    └─ DECISION: default/server/router/client direct runtime mode?
│         ├─ YES → launch corresponding runtime
│         └─ NO → CLI parser path
│
▼
FILE: src/cli/commands.rs
│
├─ Clap parse
│    ├─ parse command/subcommand/options/positionals
│    └─ DECISION: syntax + required args valid?
│         ├─ NO → Clap help/error + exit code → END
│         └─ YES → typed command enum
│
├─ Command dispatch
│    ├─ match typed command variant
│    └─ call implementation module
│
▼
FILE: src/cli/user.rs
│
├─ Command-specific input validation
│    ├─ validate IDs/files/model names/ranges/options as required
│    └─ DECISION: semantic input valid?
│         ├─ NO → error output → END
│         └─ YES → perform operation
│
├─ External/state I/O
│    ├─ DB / filesystem / HTTP / service operation depending on command
│    └─ DECISION: operation succeeds?
│         ├─ NO → print/return error → END
│         └─ YES → domain result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ UserService operation
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
└─ UserDatabase operation → domain result
│
├─ Output formatting
│    ├─ map result to table/text/status output
│    └─ write stdout/stderr
│
├─ Process exit
│    └─ success returns to shell; long-running command may remain active
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: src/cli/user.rs
│    │    ├─ handle_user_command()
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → cmd_user_register() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_login() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_list() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_topup() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_recharges() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_check_username() @ src/cli/user.rs
│    │    ├─ cmd_user_register()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_user() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_login()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → BundleVerifier::verify() @ crates/installer/src/bundle.rs
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_list()
│    │    │    └─ CALL → UserDatabase::list_users() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_topup()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::get_user_by_id() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_recharge() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_recharges()
│    │    │    └─ CALL → UserDatabase::list_recharges() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_check_username()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    │    ├─ UserDatabase::get_user_by_username()
│    │    ├─ UserDatabase::create_user()
│    │    ├─ UserDatabase::get_user_roles()
│    │    ├─ UserDatabase::list_users()
│    │    ├─ UserDatabase::get_user_by_id()
│    │    ├─ UserDatabase::create_recharge()
│    │    ├─ UserDatabase::update_balance()
│    │    ├─ UserDatabase::list_recharges()
│    └─ FILE: crates/installer/src/bundle.rs
│    │    ├─ BundleVerifier::verify()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
$ burncloud user register
User created
id=10001
username=demo_user
```

### E2E FLOW #90 — burncloud user login

**树路径：** `CLI / Executables → burncloud → burncloud user login`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ Shell input
│    ├─ command: burncloud user login
│    ├─ argv tokenization done by shell
│    └─ process environment / cwd available
│
▼
FILE: src/main.rs
│
├─ Process bootstrap
│    ├─ dotenv load
│    ├─ ensure/generate MASTER_KEY
│    ├─ initialize logging
│    └─ inspect argv
│
├─ Top-level dispatch
│    └─ DECISION: default/server/router/client direct runtime mode?
│         ├─ YES → launch corresponding runtime
│         └─ NO → CLI parser path
│
▼
FILE: src/cli/commands.rs
│
├─ Clap parse
│    ├─ parse command/subcommand/options/positionals
│    └─ DECISION: syntax + required args valid?
│         ├─ NO → Clap help/error + exit code → END
│         └─ YES → typed command enum
│
├─ Command dispatch
│    ├─ match typed command variant
│    └─ call implementation module
│
▼
FILE: src/cli/user.rs
│
├─ Command-specific input validation
│    ├─ validate IDs/files/model names/ranges/options as required
│    └─ DECISION: semantic input valid?
│         ├─ NO → error output → END
│         └─ YES → perform operation
│
├─ External/state I/O
│    ├─ DB / filesystem / HTTP / service operation depending on command
│    └─ DECISION: operation succeeds?
│         ├─ NO → print/return error → END
│         └─ YES → domain result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ UserService operation
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
└─ UserDatabase operation → domain result
│
├─ Output formatting
│    ├─ map result to table/text/status output
│    └─ write stdout/stderr
│
├─ Process exit
│    └─ success returns to shell; long-running command may remain active
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: src/cli/user.rs
│    │    ├─ handle_user_command()
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → cmd_user_register() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_login() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_list() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_topup() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_recharges() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_check_username() @ src/cli/user.rs
│    │    ├─ cmd_user_register()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_user() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_login()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → BundleVerifier::verify() @ crates/installer/src/bundle.rs
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_list()
│    │    │    └─ CALL → UserDatabase::list_users() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_topup()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::get_user_by_id() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_recharge() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_recharges()
│    │    │    └─ CALL → UserDatabase::list_recharges() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_check_username()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    │    ├─ UserDatabase::get_user_by_username()
│    │    ├─ UserDatabase::create_user()
│    │    ├─ UserDatabase::get_user_roles()
│    │    ├─ UserDatabase::list_users()
│    │    ├─ UserDatabase::get_user_by_id()
│    │    ├─ UserDatabase::create_recharge()
│    │    ├─ UserDatabase::update_balance()
│    │    ├─ UserDatabase::list_recharges()
│    └─ FILE: crates/installer/src/bundle.rs
│    │    ├─ BundleVerifier::verify()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
$ burncloud user login
Login successful
username=demo_user
client state saved
```

### E2E FLOW #91 — burncloud user list

**树路径：** `CLI / Executables → burncloud → burncloud user list`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ Shell input
│    ├─ command: burncloud user list
│    ├─ argv tokenization done by shell
│    └─ process environment / cwd available
│
▼
FILE: src/main.rs
│
├─ Process bootstrap
│    ├─ dotenv load
│    ├─ ensure/generate MASTER_KEY
│    ├─ initialize logging
│    └─ inspect argv
│
├─ Top-level dispatch
│    └─ DECISION: default/server/router/client direct runtime mode?
│         ├─ YES → launch corresponding runtime
│         └─ NO → CLI parser path
│
▼
FILE: src/cli/commands.rs
│
├─ Clap parse
│    ├─ parse command/subcommand/options/positionals
│    └─ DECISION: syntax + required args valid?
│         ├─ NO → Clap help/error + exit code → END
│         └─ YES → typed command enum
│
├─ Command dispatch
│    ├─ match typed command variant
│    └─ call implementation module
│
▼
FILE: src/cli/user.rs
│
├─ Command-specific input validation
│    ├─ validate IDs/files/model names/ranges/options as required
│    └─ DECISION: semantic input valid?
│         ├─ NO → error output → END
│         └─ YES → perform operation
│
├─ External/state I/O
│    ├─ DB / filesystem / HTTP / service operation depending on command
│    └─ DECISION: operation succeeds?
│         ├─ NO → print/return error → END
│         └─ YES → domain result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ UserService operation
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
└─ UserDatabase operation → domain result
│
├─ Output formatting
│    ├─ map result to table/text/status output
│    └─ write stdout/stderr
│
├─ Process exit
│    └─ success returns to shell; long-running command may remain active
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: src/cli/user.rs
│    │    ├─ handle_user_command()
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → cmd_user_register() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_login() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_list() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_topup() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_recharges() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_check_username() @ src/cli/user.rs
│    │    ├─ cmd_user_register()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_user() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_login()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → BundleVerifier::verify() @ crates/installer/src/bundle.rs
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_list()
│    │    │    └─ CALL → UserDatabase::list_users() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_topup()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::get_user_by_id() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_recharge() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_recharges()
│    │    │    └─ CALL → UserDatabase::list_recharges() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_check_username()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    │    ├─ UserDatabase::get_user_by_username()
│    │    ├─ UserDatabase::create_user()
│    │    ├─ UserDatabase::get_user_roles()
│    │    ├─ UserDatabase::list_users()
│    │    ├─ UserDatabase::get_user_by_id()
│    │    ├─ UserDatabase::create_recharge()
│    │    ├─ UserDatabase::update_balance()
│    │    ├─ UserDatabase::list_recharges()
│    └─ FILE: crates/installer/src/bundle.rs
│    │    ├─ BundleVerifier::verify()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
$ burncloud user list
ID     USERNAME    ROLES   BALANCE
10001  demo_user   user    286.42 USD
```

### E2E FLOW #92 — burncloud user topup

**树路径：** `CLI / Executables → burncloud → burncloud user topup`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ Shell input
│    ├─ command: burncloud user topup
│    ├─ argv tokenization done by shell
│    └─ process environment / cwd available
│
▼
FILE: src/main.rs
│
├─ Process bootstrap
│    ├─ dotenv load
│    ├─ ensure/generate MASTER_KEY
│    ├─ initialize logging
│    └─ inspect argv
│
├─ Top-level dispatch
│    └─ DECISION: default/server/router/client direct runtime mode?
│         ├─ YES → launch corresponding runtime
│         └─ NO → CLI parser path
│
▼
FILE: src/cli/commands.rs
│
├─ Clap parse
│    ├─ parse command/subcommand/options/positionals
│    └─ DECISION: syntax + required args valid?
│         ├─ NO → Clap help/error + exit code → END
│         └─ YES → typed command enum
│
├─ Command dispatch
│    ├─ match typed command variant
│    └─ call implementation module
│
▼
FILE: src/cli/user.rs
│
├─ Command-specific input validation
│    ├─ validate IDs/files/model names/ranges/options as required
│    └─ DECISION: semantic input valid?
│         ├─ NO → error output → END
│         └─ YES → perform operation
│
├─ External/state I/O
│    ├─ DB / filesystem / HTTP / service operation depending on command
│    └─ DECISION: operation succeeds?
│         ├─ NO → print/return error → END
│         └─ YES → domain result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ UserService operation
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
└─ UserDatabase operation → domain result
│
├─ Output formatting
│    ├─ map result to table/text/status output
│    └─ write stdout/stderr
│
├─ Process exit
│    └─ success returns to shell; long-running command may remain active
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: src/cli/user.rs
│    │    ├─ handle_user_command()
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → cmd_user_register() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_login() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_list() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_topup() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_recharges() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_check_username() @ src/cli/user.rs
│    │    ├─ cmd_user_register()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_user() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_login()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → BundleVerifier::verify() @ crates/installer/src/bundle.rs
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_list()
│    │    │    └─ CALL → UserDatabase::list_users() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_topup()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::get_user_by_id() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_recharge() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_recharges()
│    │    │    └─ CALL → UserDatabase::list_recharges() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_check_username()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    │    ├─ UserDatabase::get_user_by_username()
│    │    ├─ UserDatabase::create_user()
│    │    ├─ UserDatabase::get_user_roles()
│    │    ├─ UserDatabase::list_users()
│    │    ├─ UserDatabase::get_user_by_id()
│    │    ├─ UserDatabase::create_recharge()
│    │    ├─ UserDatabase::update_balance()
│    │    ├─ UserDatabase::list_recharges()
│    └─ FILE: crates/installer/src/bundle.rs
│    │    ├─ BundleVerifier::verify()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
$ burncloud user topup
Top-up successful
new_balance=286.42 USD
```

### E2E FLOW #93 — burncloud user recharges

**树路径：** `CLI / Executables → burncloud → burncloud user recharges`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ Shell input
│    ├─ command: burncloud user recharges
│    ├─ argv tokenization done by shell
│    └─ process environment / cwd available
│
▼
FILE: src/main.rs
│
├─ Process bootstrap
│    ├─ dotenv load
│    ├─ ensure/generate MASTER_KEY
│    ├─ initialize logging
│    └─ inspect argv
│
├─ Top-level dispatch
│    └─ DECISION: default/server/router/client direct runtime mode?
│         ├─ YES → launch corresponding runtime
│         └─ NO → CLI parser path
│
▼
FILE: src/cli/commands.rs
│
├─ Clap parse
│    ├─ parse command/subcommand/options/positionals
│    └─ DECISION: syntax + required args valid?
│         ├─ NO → Clap help/error + exit code → END
│         └─ YES → typed command enum
│
├─ Command dispatch
│    ├─ match typed command variant
│    └─ call implementation module
│
▼
FILE: src/cli/user.rs
│
├─ Command-specific input validation
│    ├─ validate IDs/files/model names/ranges/options as required
│    └─ DECISION: semantic input valid?
│         ├─ NO → error output → END
│         └─ YES → perform operation
│
├─ External/state I/O
│    ├─ DB / filesystem / HTTP / service operation depending on command
│    └─ DECISION: operation succeeds?
│         ├─ NO → print/return error → END
│         └─ YES → domain result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ UserService operation
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
└─ UserDatabase operation → domain result
│
├─ Output formatting
│    ├─ map result to table/text/status output
│    └─ write stdout/stderr
│
├─ Process exit
│    └─ success returns to shell; long-running command may remain active
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: src/cli/user.rs
│    │    ├─ handle_user_command()
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → cmd_user_register() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_login() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_list() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_topup() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_recharges() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_check_username() @ src/cli/user.rs
│    │    ├─ cmd_user_register()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_user() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_login()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → BundleVerifier::verify() @ crates/installer/src/bundle.rs
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_list()
│    │    │    └─ CALL → UserDatabase::list_users() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_topup()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::get_user_by_id() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_recharge() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_recharges()
│    │    │    └─ CALL → UserDatabase::list_recharges() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_check_username()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    │    ├─ UserDatabase::get_user_by_username()
│    │    ├─ UserDatabase::create_user()
│    │    ├─ UserDatabase::get_user_roles()
│    │    ├─ UserDatabase::list_users()
│    │    ├─ UserDatabase::get_user_by_id()
│    │    ├─ UserDatabase::create_recharge()
│    │    ├─ UserDatabase::update_balance()
│    │    ├─ UserDatabase::list_recharges()
│    └─ FILE: crates/installer/src/bundle.rs
│    │    ├─ BundleVerifier::verify()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
$ burncloud user recharges
ID    AMOUNT      CREATED_AT
9001  100.00 USD  2026-08-11 14:30:00
```

### E2E FLOW #94 — burncloud user check-username

**树路径：** `CLI / Executables → burncloud → burncloud user check-username`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/service/crates/user/src/lib.rs`

```text
START
│
├─ Shell input
│    ├─ command: burncloud user check-username
│    ├─ argv tokenization done by shell
│    └─ process environment / cwd available
│
▼
FILE: src/main.rs
│
├─ Process bootstrap
│    ├─ dotenv load
│    ├─ ensure/generate MASTER_KEY
│    ├─ initialize logging
│    └─ inspect argv
│
├─ Top-level dispatch
│    └─ DECISION: default/server/router/client direct runtime mode?
│         ├─ YES → launch corresponding runtime
│         └─ NO → CLI parser path
│
▼
FILE: src/cli/commands.rs
│
├─ Clap parse
│    ├─ parse command/subcommand/options/positionals
│    └─ DECISION: syntax + required args valid?
│         ├─ NO → Clap help/error + exit code → END
│         └─ YES → typed command enum
│
├─ Command dispatch
│    ├─ match typed command variant
│    └─ call implementation module
│
▼
FILE: src/cli/user.rs
│
├─ Command-specific input validation
│    ├─ validate IDs/files/model names/ranges/options as required
│    └─ DECISION: semantic input valid?
│         ├─ NO → error output → END
│         └─ YES → perform operation
│
├─ External/state I/O
│    ├─ DB / filesystem / HTTP / service operation depending on command
│    └─ DECISION: operation succeeds?
│         ├─ NO → print/return error → END
│         └─ YES → domain result
│
▼
FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│
└─ UserService operation
│
▼
FILE: crates/database/crates/user/src/lib.rs
│
└─ UserDatabase operation → domain result
│
├─ Output formatting
│    ├─ map result to table/text/status output
│    └─ write stdout/stderr
│
├─ Process exit
│    └─ success returns to shell; long-running command may remain active
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: src/cli/user.rs
│    │    ├─ handle_user_command()
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → cmd_user_register() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_login() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_list() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_topup() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_recharges() @ src/cli/user.rs
│    │    │    └─ CALL → cmd_user_check_username() @ src/cli/user.rs
│    │    ├─ cmd_user_register()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_user() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_login()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → BundleVerifier::verify() @ crates/installer/src/bundle.rs
│    │    │    └─ CALL → UserDatabase::get_user_roles() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_list()
│    │    │    └─ CALL → UserDatabase::list_users() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_topup()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::get_user_by_id() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::create_recharge() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → UserDatabase::update_balance() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_recharges()
│    │    │    └─ CALL → UserDatabase::list_recharges() @ crates/database/crates/user/src/lib.rs
│    │    ├─ cmd_user_check_username()
│    │    │    └─ CALL → UserDatabase::get_user_by_username() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    │    ├─ UserDatabase::get_user_by_username()
│    │    ├─ UserDatabase::create_user()
│    │    ├─ UserDatabase::get_user_roles()
│    │    ├─ UserDatabase::list_users()
│    │    ├─ UserDatabase::get_user_by_id()
│    │    ├─ UserDatabase::create_recharge()
│    │    ├─ UserDatabase::update_balance()
│    │    ├─ UserDatabase::list_recharges()
│    └─ FILE: crates/installer/src/bundle.rs
│    │    ├─ BundleVerifier::verify()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
```

#### PR Before / After（patch 证据）

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
$ burncloud user check-username
username=demo_user
available=false
```

### E2E FLOW #95 — System Monitor Auto Update

**树路径：** `Background Jobs / Async Side Effects → Long-running Jobs → System Monitor Auto Update`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`, `crates/server/src/api/mod.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: System Monitor Auto Update
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute System Monitor Auto Update
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/lib.rs  [PR CHANGED]
│    │    ├─ start_server()
│    │    │    └─ CALL → create_default_database() @ crates/database/src/database.rs
│    │    │    └─ CALL → RouterDatabase::init() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → create_app() @ crates/server/src/lib.rs  [PR CHANGED]
│    │    ├─ create_app()
│    │    │    └─ CALL → SystemMonitorService::new() @ crates/service/crates/monitor/src/service.rs
│    │    │    └─ CALL → CacheService::new() @ crates/service/crates/cache/src/service.rs
│    │    │    └─ CALL → create_router_app() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → UserService::new() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → routes() @ crates/server/src/api/mod.rs  [PR CHANGED]
│    ├─ FILE: crates/database/src/database.rs
│    │    ├─ create_default_database()
│    │    │    └─ CALL → Database::new() @ crates/database/src/database.rs
│    │    ├─ Database::new()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::init()
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    ├─ FILE: crates/service/crates/monitor/src/service.rs
│    │    ├─ SystemMonitorService::new()
│    ├─ FILE: crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::new()
│    │    │    └─ CALL → CacheService::with_config() @ crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::with_config()
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ configure_rate_budget_from_db()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::new()
│    ├─ FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│    │    ├─ routes()
│    │    │    └─ CALL → public_routes() @ crates/server/src/api/auth.rs
│    │    │    └─ CALL → protected_routes() @ crates/server/src/api/auth.rs
│    │    │    └─ CALL → security_routes() @ crates/server/src/api/security.rs
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    └─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
2026-08-11T14:45:10+08:00 monitor_update cpu=31.7% memory=62.4% disk=48.9% status=ok
```

### E2E FLOW #96 — Price Sync

**树路径：** `Background Jobs / Async Side Effects → Long-running Jobs → Price Sync`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: Price Sync
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute Price Sync
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
2026-08-11T14:45:11+08:00 price_sync updated_models=46 duration_ms=842 status=success
```

### E2E FLOW #97 — Exchange Rate Sync

**树路径：** `Background Jobs / Async Side Effects → Long-running Jobs → Exchange Rate Sync`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: Exchange Rate Sync
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute Exchange Rate Sync
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
2026-08-11T14:45:12+08:00 exchange_rate_sync currencies=8 stale=false status=success
```

### E2E FLOW #98 — AIMD Budget Feedback

**树路径：** `Background Jobs / Async Side Effects → Long-running Jobs → AIMD Budget Feedback`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: AIMD Budget Feedback
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute AIMD Budget Feedback
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
2026-08-11T14:45:13+08:00 aimd channel_id=12 result=success old_budget=96 new_budget=100
```

### E2E FLOW #99 — Async Router Log Writer

**树路径：** `Background Jobs / Async Side Effects → Long-running Jobs → Async Router Log Writer`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: Async Router Log Writer
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute Async Router Log Writer
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
2026-08-11T14:45:14+08:00 router_log_writer persisted=64 queue_remaining=0 status=ok
```

### E2E FLOW #100 — Async Request Log Writer

**树路径：** `Background Jobs / Async Side Effects → Long-running Jobs → Async Request Log Writer`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: Async Request Log Writer
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute Async Request Log Writer
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
2026-08-11T14:45:15+08:00 request_log_writer persisted=64 queue_remaining=0 status=ok
```

### E2E FLOW #101 — Token accessed_time update

**树路径：** `Background Jobs / Async Side Effects → Request-time Async Side Effects → Token accessed_time update`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: Token accessed_time update
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute Token accessed_time update
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
token=bc_live_7d4e...example accessed_time=2026-08-11T14:45:16+08:00 update=success
```

### E2E FLOW #102 — Quota deduction

**树路径：** `Background Jobs / Async Side Effects → Request-time Async Side Effects → Quota deduction`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: Quota deduction
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute Quota deduction
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
user_id=10001 cost=0.00042 quota_before=100.00000 quota_after=99.99958 status=success
```

### E2E FLOW #103 — Video task mapping save

**树路径：** `Background Jobs / Async Side Effects → Request-time Async Side Effects → Video task mapping save`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: Video task mapping save
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute Video task mapping save
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
task_id=video_task_bc_01JXYZ channel_id=12 user_id=10001 mapping_saved=true
```

### E2E FLOW #104 — API version detect / update

**树路径：** `Background Jobs / Async Side Effects → Request-time Async Side Effects → API version detect / update`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START / TRIGGER
│
├─ Trigger source
│    ├─ startup spawn / request-time tokio::spawn / manager restoration / channel message
│    └─ task: API version detect / update
│
├─ Spawn / registration
│    ├─ parent runtime creates async task/thread/loop
│    └─ DECISION: spawn/runtime handle available?
│         ├─ NO → task never starts; parent may log error
│         └─ YES → task owns/borrows required shared state
│
├─ Wait boundary
│    ├─ timer sleep / mpsc receive / polling interval / restored work item
│    └─ DECISION: trigger/event available?
│         ├─ NO → continue waiting
│         └─ YES → one iteration begins
│
├─ Input snapshot
│    ├─ read latest runtime/DB/request-derived state needed by job
│    └─ freeze iteration context
│
├─ Core job operation
│    ├─ execute API version detect / update
│    └─ may call DB / HTTP / filesystem / runtime service depending on task
│
├─ Operation result
│    └─ DECISION: iteration succeeds?
│         ├─ NO
│         │    ├─ log/record failure
│         │    ├─ preserve parent request availability when task is fail-open
│         │    └─ decide retry on next event/interval
│         └─ YES
│              ├─ update in-memory state and/or persistent state
│              └─ emit success telemetry/log
│
├─ Cancellation / lifetime
│    └─ DECISION: parent runtime still alive AND task should continue?
│         ├─ YES → 返回等待边界
│         └─ NO → release task resources
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / NEXT ITERATION
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
channel_id=12 detected_api_version=v1 version_cache_updated=true
```

### E2E FLOW #105 — start_server

**树路径：** `Startup → Startup Chain → start_server`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`, `crates/server/src/api/mod.rs`

```text
START
│
├─ Process/environment input
│    ├─ startup target: start_server
│    ├─ environment variables / dotenv
│    ├─ CLI/platform mode
│    └─ filesystem/database availability
│
├─ Enter startup function
│    └─ execute start_server
│
├─ Dependency initialization
│    ├─ construct required DB/services/runtime state
│    ├─ register routes/tasks as applicable
│    └─ DECISION: dependency initialization succeeds?
│         ├─ NO → propagate startup error → process/runtime not ready → END
│         └─ YES → next dependency
│
├─ Runtime composition
│    ├─ wire shared Arc/State/services
│    ├─ compose routers/middleware/background jobs
│    └─ make dependencies reachable from runtime entrypoints
│
├─ Readiness boundary
│    └─ DECISION: all required startup stages complete?
│         ├─ NO → startup fails/returns Err
│         └─ YES → expose listener/client/event loop/runtime
│
├─ Steady-state handoff
│    ├─ long-running loops take ownership of runtime
│    └─ requests/events can now enter documented entrypoints
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/lib.rs  [PR CHANGED]
│    │    ├─ start_server()
│    │    │    └─ CALL → create_default_database() @ crates/database/src/database.rs
│    │    │    └─ CALL → RouterDatabase::init() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → create_app() @ crates/server/src/lib.rs  [PR CHANGED]
│    │    ├─ create_app()
│    │    │    └─ CALL → SystemMonitorService::new() @ crates/service/crates/monitor/src/service.rs
│    │    │    └─ CALL → CacheService::new() @ crates/service/crates/cache/src/service.rs
│    │    │    └─ CALL → create_router_app() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → UserService::new() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → routes() @ crates/server/src/api/mod.rs  [PR CHANGED]
│    ├─ FILE: crates/database/src/database.rs
│    │    ├─ create_default_database()
│    │    │    └─ CALL → Database::new() @ crates/database/src/database.rs
│    │    ├─ Database::new()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::init()
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    ├─ FILE: crates/service/crates/monitor/src/service.rs
│    │    ├─ SystemMonitorService::new()
│    ├─ FILE: crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::new()
│    │    │    └─ CALL → CacheService::with_config() @ crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::with_config()
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ configure_rate_budget_from_db()
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::new()
│    ├─ FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│    │    ├─ routes()
│    │    │    └─ CALL → public_routes() @ crates/server/src/api/auth.rs
│    │    │    └─ CALL → protected_routes() @ crates/server/src/api/auth.rs
│    │    │    └─ CALL → security_routes() @ crates/server/src/api/security.rs
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    └─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ component is READY / RUNNING
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
database=connected
router_db=initialized
user_db=initialized
listener=0.0.0.0:3000
server=running
```

### E2E FLOW #106 — create_app

**树路径：** `Startup → Startup Chain → create_app`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/server/src/api/mod.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`, `crates/server/src/api/auth.rs`

```text
START
│
├─ Process/environment input
│    ├─ startup target: create_app
│    ├─ environment variables / dotenv
│    ├─ CLI/platform mode
│    └─ filesystem/database availability
│
├─ Enter startup function
│    └─ execute create_app
│
├─ Dependency initialization
│    ├─ construct required DB/services/runtime state
│    ├─ register routes/tasks as applicable
│    └─ DECISION: dependency initialization succeeds?
│         ├─ NO → propagate startup error → process/runtime not ready → END
│         └─ YES → next dependency
│
├─ Runtime composition
│    ├─ wire shared Arc/State/services
│    ├─ compose routers/middleware/background jobs
│    └─ make dependencies reachable from runtime entrypoints
│
├─ Readiness boundary
│    └─ DECISION: all required startup stages complete?
│         ├─ NO → startup fails/returns Err
│         └─ YES → expose listener/client/event loop/runtime
│
├─ Steady-state handoff
│    ├─ long-running loops take ownership of runtime
│    └─ requests/events can now enter documented entrypoints
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/server/src/lib.rs  [PR CHANGED]
│    │    ├─ start_server()
│    │    │    └─ CALL → create_default_database() @ crates/database/src/database.rs
│    │    │    └─ CALL → RouterDatabase::init() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → create_app() @ crates/server/src/lib.rs  [PR CHANGED]
│    │    ├─ create_app()
│    │    │    └─ CALL → SystemMonitorService::new() @ crates/service/crates/monitor/src/service.rs
│    │    │    └─ CALL → CacheService::new() @ crates/service/crates/cache/src/service.rs
│    │    │    └─ CALL → create_router_app() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → UserService::new() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → routes() @ crates/server/src/api/mod.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│    │    ├─ routes()
│    │    │    └─ CALL → public_routes() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    │    └─ CALL → protected_routes() @ crates/server/src/api/auth.rs  [PR CHANGED]
│    │    │    └─ CALL → security_routes() @ crates/server/src/api/security.rs
│    ├─ FILE: crates/client/src/lib.rs
│    │    ├─ liveview_router()
│    │    │    └─ CALL → liveview_style_tags() @ crates/client/crates/client-shared/src/app_styles.rs
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    ├─ FILE: crates/database/src/database.rs
│    │    ├─ create_default_database()
│    │    │    └─ CALL → Database::new() @ crates/database/src/database.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::init()
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    ├─ FILE: crates/service/crates/monitor/src/service.rs
│    │    ├─ SystemMonitorService::new()
│    ├─ FILE: crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::new()
│    │    │    └─ CALL → CacheService::with_config() @ crates/service/crates/cache/src/service.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::new()
│    ├─ FILE: crates/server/src/api/auth.rs  [PR CHANGED]
│    │    ├─ public_routes()
│    │    ├─ protected_routes()
│    ├─ FILE: crates/server/src/api/security.rs
│    │    ├─ security_routes()
│    ├─ FILE: crates/client/crates/client-shared/src/app_styles.rs
│    │    ├─ liveview_style_tags()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    └─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ component is READY / RUNNING
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/api/auth.rs`**

HUNKS:
```text
@@ -9,8 +9,7 @@ use axum::{
@@ -54,18 +53,11 @@ struct AuthData {
@@ -104,10 +96,7 @@ pub async fn admin_middleware(
@@ -127,6 +116,7 @@ fn is_sensitive_internal_mutation(method: &Method, path: &str) -> bool {
@@ -171,7 +161,7 @@ pub async fn security_boundary_middleware(
@@ -351,7 +341,11 @@ async fn oauth_github(State(_state): State<AppState>) -> impl IntoResponse {
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserServiceError;
-use jsonwebtoken::{decode, DecodingKey, Validation};
-fn get_jwt_secret() -> String {
-    burncloud_common::constants::jwt_secret()
-}
-
-pub fn verify_jwt(token: &str) -> Result<Claims, jsonwebtoken::errors::Error> {
-    let secret = get_jwt_secret();
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserServiceError};
+pub fn verify_jwt(
+    token: &str,
+    jwt_secret: &JwtSecret,
+) -> Result<Claims, jsonwebtoken::errors::Error> {
+    jwt_secret.verify(token)
+    path == "/v1" || path.starts_with("/v1/") || path == "/api/v1" || path.starts_with("/api/v1/")
+    State(state): State<AppState>,
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
monitor=started
cache=ready
management_routes=mounted
internal_routes=mounted
liveview=enabled
data_plane_fallback=mounted
```

### E2E FLOW #107 — create_router_app

**树路径：** `Startup → Startup Chain → create_router_app`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/router/src/lib.rs`

```text
START
│
├─ Process/environment input
│    ├─ startup target: create_router_app
│    ├─ environment variables / dotenv
│    ├─ CLI/platform mode
│    └─ filesystem/database availability
│
├─ Enter startup function
│    └─ execute create_router_app
│
├─ Dependency initialization
│    ├─ construct required DB/services/runtime state
│    ├─ register routes/tasks as applicable
│    └─ DECISION: dependency initialization succeeds?
│         ├─ NO → propagate startup error → process/runtime not ready → END
│         └─ YES → next dependency
│
├─ Runtime composition
│    ├─ wire shared Arc/State/services
│    ├─ compose routers/middleware/background jobs
│    └─ make dependencies reachable from runtime entrypoints
│
├─ Readiness boundary
│    └─ DECISION: all required startup stages complete?
│         ├─ NO → startup fails/returns Err
│         └─ YES → expose listener/client/event loop/runtime
│
├─ Steady-state handoff
│    ├─ long-running loops take ownership of runtime
│    └─ requests/events can now enter documented entrypoints
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    │    ├─ proxy_handler()
│    │    │    └─ CALL → normalize_doubled_path() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_and_get_info() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::update_token_accessed_time() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → RouterDatabase::validate_token_detailed() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → RouterVideoTaskModel::get_by_task_id() @ crates/database/crates/router/src/router_video_task.rs
│    │    │    └─ CALL → ChannelProviderModel::get_by_id() @ crates/database/crates/channel/src/channel_provider.rs
│    │    │    └─ CALL → UnifiedTokenCounter::new() @ crates/service/crates/billing/src/counter.rs
│    │    │    └─ CALL → proxy_logic() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ proxy_logic()
│    │    │    └─ CALL → sanitize_request_body() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → sanitize_request_headers() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → extract_model_from_gemini_path() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → UserService::resolve_traffic_class() @ crates/service/crates/user/src/lib.rs
│    │    │    └─ CALL → OrderType::from_db_row() @ crates/router/src/order_type.rs
│    │    │    └─ CALL → build_response_with_header() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → record_failover_attempt() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → BudgetGuard::new() @ crates/router/src/rate_budget.rs
│    │    │    └─ CALL → should_passthrough() @ crates/router/src/passthrough.rs
│    │    │    └─ CALL → build_gemini_passthrough_url() @ crates/router/src/passthrough.rs
│    │    ├─ configure_rate_budget_from_db()
│    │    ├─ EmptyResponseCounter::new()
│    │    ├─ normalize_doubled_path()
│    │    ├─ build_response_with_header()
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    ├─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│    ├─ FILE: crates/router/src/adaptor/factory.rs
│    │    ├─ DynamicAdaptorFactory::new()
│    ├─ FILE: crates/router/src/adaptor/detector.rs
│    │    ├─ ApiVersionDetector::new()
│    ├─ FILE: crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::load()
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    ├─ PriceCache::empty()
│    ├─ FILE: crates/service/crates/billing/src/calculator.rs
│    │    ├─ CostCalculator::new()
│    ├─ FILE: crates/router/src/exchange_rate.rs
│    │    ├─ ExchangeRateService::new()
│    ├─ FILE: crates/router/src/scheduler/mod.rs
│    │    ├─ load_scheduler_config()
│    ├─ FILE: crates/router/src/rate_budget.rs
│    │    ├─ InMemoryBudget::new()
│    ├─ FILE: crates/router/src/price_sync.rs
│    │    ├─ start_price_sync_task()
│    │    │    └─ CALL → PriceSyncService::with_config() @ crates/router/src/price_sync.rs
│    │    │    └─ CALL → PriceSyncService::new() @ crates/router/src/price_sync.rs
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::insert_log()
│    │    │    └─ CALL → RouterLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::insert_request_log()
│    │    │    └─ CALL → RouterRequestLogModel::insert() @ crates/database/crates/router/src/log.rs
│    │    ├─ RouterDatabase::validate_token_and_get_info()
│    │    ├─ RouterDatabase::update_token_accessed_time()
│    │    │    └─ CALL → RouterTokenModel::update_accessed_time() @ crates/database/crates/router/src/token.rs
│    │    ├─ RouterDatabase::validate_token_detailed()
│    │    │    └─ CALL → RouterTokenModel::validate_detailed() @ crates/database/crates/router/src/token.rs
│    ├─ FILE: crates/router/src/channel_health_manager.rs
│    │    ├─ ChannelHealthManager::new()
│    └─ FILE: crates/router/src/passthrough.rs
│    │    ├─ extract_model_from_gemini_path()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END
     └─ component is READY / RUNNING
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
http_client=ready
model_router=ready
circuit_breaker=ready
price_cache=ready
exchange_rates=ready
background_writers=running
router=ready
```

### E2E FLOW #108 — Windows Background Server Thread

**树路径：** `Background Jobs / Async Side Effects → Desktop Background Work → Windows Background Server Thread`  
**PR Impact：** `STATIC SOURCE-PATH MATCH`  
**本 Flow 命中的 changed files：** `crates/server/src/lib.rs`, `crates/router/src/lib.rs`, `crates/service/crates/user/src/lib.rs`, `crates/server/src/api/mod.rs`

```text
START
│
├─ User launches burncloud.exe with no CLI arguments on Windows
│
▼
FILE: src/main.rs
│
├─ main()
│    ├─ dotenvy::dotenv()
│    ├─ ensure_master_key()
│    ├─ init_logging()
│    └─ DECISION: args == [binary] AND target_os == windows?
│         ├─ NO → other CLI/server/client branch
│         └─ YES → std::thread::spawn server thread
│
├─ Background OS thread
│    ├─ tokio::runtime::Runtime::new()
│    ├─ read HOST or default 127.0.0.1
│    ├─ read PORT or DEFAULT_PORT
│    └─ rt.block_on(burncloud_server::start_server(host, port, false))
│
▼
FILE: crates/server/src/lib.rs  [PR CHANGED]
│
├─ start_server(host, port, enable_liveview=false)
│    ├─ create_default_database()
│    ├─ RouterDatabase::init()
│    ├─ UserDatabase::init()
│    ├─ create_app(db, false)
│    ├─ TcpListener::bind()
│    └─ axum::serve()
│
├─ DECISION: server startup/runtime returns Err?
│    ├─ YES → src/main.rs thread prints "Server failed to start"
│    └─ NO → server thread remains serving requests
│
▼
FILE: src/main.rs
│
└─ Main Windows thread independently continues
     └─ burncloud_client::launch_gui_with_tray()
│
│
├─ 源码函数展开（静态扫描确认）
│    ├─ FILE: src/main.rs
│    │    ├─ main()
│    │    │    └─ CALL → ensure_master_key() @ src/main.rs
│    │    │    └─ CALL → init_logging() @ crates/server/src/logging.rs
│    │    │    └─ CALL → run_async_server() @ src/main.rs
│    │    │    └─ CALL → run_async_cli() @ src/main.rs
│    │    │    └─ CALL → show_help() @ src/cli/commands.rs
│    │    ├─ ensure_master_key()
│    │    │    └─ CALL → is_valid_master_key() @ src/main.rs
│    │    ├─ run_async_server()
│    │    ├─ run_async_cli()
│    │    │    └─ CALL → handle_command() @ src/cli/commands.rs
│    │    ├─ is_valid_master_key()
│    ├─ FILE: crates/server/src/lib.rs  [PR CHANGED]
│    │    ├─ start_server()
│    │    │    └─ CALL → create_default_database() @ crates/database/src/database.rs
│    │    │    └─ CALL → RouterDatabase::init() @ crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → UserDatabase::init() @ crates/database/crates/user/src/lib.rs
│    │    │    └─ CALL → create_app() @ crates/server/src/lib.rs  [PR CHANGED]
│    │    ├─ create_app()
│    │    │    └─ CALL → SystemMonitorService::new() @ crates/service/crates/monitor/src/service.rs
│    │    │    └─ CALL → CacheService::new() @ crates/service/crates/cache/src/service.rs
│    │    │    └─ CALL → create_router_app() @ crates/router/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → UserService::new() @ crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    │    └─ CALL → routes() @ crates/server/src/api/mod.rs  [PR CHANGED]
│    ├─ FILE: crates/server/src/logging.rs
│    │    ├─ init_logging()
│    │    │    └─ CALL → file_appender() @ crates/server/src/logging.rs
│    │    │    └─ CALL → module_filter() @ crates/server/src/logging.rs
│    │    ├─ file_appender()
│    │    ├─ module_filter()
│    ├─ FILE: src/cli/commands.rs
│    │    ├─ show_help()
│    │    ├─ handle_command()
│    ├─ FILE: crates/database/src/database.rs
│    │    ├─ create_default_database()
│    │    │    └─ CALL → Database::new() @ crates/database/src/database.rs
│    │    ├─ Database::new()
│    ├─ FILE: crates/database/crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ RouterDatabase::init()
│    ├─ FILE: crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::init()
│    │    │    └─ CALL → UserDatabase::assign_role() @ crates/database/crates/user/src/lib.rs
│    │    ├─ UserDatabase::assign_role()
│    ├─ FILE: crates/service/crates/monitor/src/service.rs
│    │    ├─ SystemMonitorService::new()
│    ├─ FILE: crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::new()
│    │    │    └─ CALL → CacheService::with_config() @ crates/service/crates/cache/src/service.rs
│    │    ├─ CacheService::with_config()
│    ├─ FILE: crates/router/src/lib.rs  [PR CHANGED]
│    │    ├─ create_router_app()
│    │    │    └─ CALL → RoundRobinBalancer::new() @ crates/router/src/balancer/mod.rs
│    │    │    └─ CALL → RateLimiter::new() @ crates/router/src/limiter.rs
│    │    │    └─ CALL → CircuitBreaker::new() @ crates/router/src/circuit_breaker.rs
│    │    │    └─ CALL → ModelRouter::new() @ crates/router/src/model_router.rs
│    │    │    └─ CALL → ChannelStateTracker::new() @ crates/router/src/channel_state.rs
│    │    │    └─ CALL → DynamicAdaptorFactory::new() @ crates/router/src/adaptor/factory.rs
│    │    │    └─ CALL → ApiVersionDetector::new() @ crates/router/src/adaptor/detector.rs
│    │    │    └─ CALL → PriceCache::load() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → PriceCache::empty() @ crates/service/crates/billing/src/cache.rs
│    │    │    └─ CALL → CostCalculator::new() @ crates/service/crates/billing/src/calculator.rs
│    ├─ FILE: crates/service/crates/user/src/lib.rs  [PR CHANGED]
│    │    ├─ UserService::new()
│    ├─ FILE: crates/server/src/api/mod.rs  [PR CHANGED]
│    │    ├─ routes()
│    │    │    └─ CALL → public_routes() @ crates/server/src/api/auth.rs
│    │    │    └─ CALL → protected_routes() @ crates/server/src/api/auth.rs
│    │    │    └─ CALL → security_routes() @ crates/server/src/api/security.rs
│    ├─ FILE: crates/router/src/balancer/mod.rs
│    │    ├─ RoundRobinBalancer::new()
│    ├─ FILE: crates/router/src/limiter.rs
│    │    ├─ RateLimiter::new()
│    ├─ FILE: crates/router/src/circuit_breaker.rs
│    │    ├─ CircuitBreaker::new()
│    ├─ FILE: crates/router/src/model_router.rs
│    │    ├─ ModelRouter::new()
│    └─ FILE: crates/router/src/channel_state.rs
│    │    ├─ ChannelStateTracker::new()
│
├─ 规则：只展开能够解析到 BurnCloud 仓库内部真实函数定义的调用；第三方库调用保留在主 E2E 中，不伪造源码目标文件
│

▼
END / SERVER THREAD CONTINUES
```

#### PR Before / After（patch 证据）

**FILE: `crates/router/src/lib.rs`**

HUNKS:
```text
@@ -19,9 +19,9 @@ pub mod rate_budget;
@@ -31,8 +31,8 @@ const PEEK_FIRST_CHUNK_TIMEOUT_SECS: u64 = 36000;
@@ -58,7 +58,7 @@ impl EmptyResponseCounter {
@@ -472,13 +472,13 @@ fn safe_cut(s: &str, max_bytes: usize) -> &str {
@@ -502,19 +502,23 @@ fn sanitize_request_body(body_bytes: &[u8]) -> (Option<String>, bool) {
@@ -528,7 +532,10 @@ fn redact_sensitive_fields(value: &mut serde_json::Value) {
```

BEFORE / REMOVED:
```diff
-mod stream_peek;
-use std::sync::RwLock as StdRwLock;
-        
-    
-    
-            format!("{}... [TRUNCATED: {} bytes total]",
-                body_bytes.len())
-            format!("{}... [TRUNCATED: {} bytes total]",
```

AFTER / ADDED:
```diff
+mod stream_peek;
+use std::sync::RwLock as StdRwLock;
+
+
+
+            format!(
+                "{}... [TRUNCATED: {} bytes total]",
+                body_bytes.len()
```

**FILE: `crates/server/src/api/mod.rs`**

HUNKS:
```text
@@ -1,13 +1,7 @@
@@ -56,7 +50,10 @@ pub fn routes(state: AppState) -> Router {
```

BEFORE / REMOVED:
```diff
-use axum::{
-    http::StatusCode,
-    middleware,
-    response::IntoResponse,
-    routing::get,
-    Router,
-};
-        .layer(middleware::from_fn(crate::auth_middleware))
```

AFTER / ADDED:
```diff
+use axum::{http::StatusCode, middleware, response::IntoResponse, routing::get, Router};
+        .layer(middleware::from_fn_with_state(
+            state.clone(),
+            crate::auth_middleware,
+        ))
```

**FILE: `crates/server/src/lib.rs`**

HUNKS:
```text
@@ -10,7 +10,7 @@ use burncloud_database_user::UserDatabase;
@@ -23,6 +23,7 @@ pub struct AppState {
@@ -31,18 +32,24 @@ pub struct AppState {
@@ -83,19 +90,26 @@ pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Res
```

BEFORE / REMOVED:
```diff
-use burncloud_service_user::UserService;
-pub async fn create_app(db: Arc<Database>, enable_liveview: bool) -> anyhow::Result<Router> {
-    let (router_app, internal_app, force_sync_tx) = create_router_app(db.clone()).await?;
-        user_service: Arc::new(UserService::new()),
-        .layer(middleware::from_fn(api::auth::security_boundary_middleware));
-    let app = create_app(db, enable_liveview).await?;
```

AFTER / ADDED:
```diff
+use burncloud_service_user::{JwtSecret, UserService};
+    pub jwt_secret: JwtSecret,
+pub async fn create_app(
+    db: Arc<Database>,
+    enable_liveview: bool,
+    jwt_secret: JwtSecret,
+) -> anyhow::Result<Router> {
+    let (router_app, internal_app, force_sync_tx) =
```

**FILE: `crates/service/crates/user/src/lib.rs`**

HUNKS:
```text
@@ -13,7 +13,10 @@ use dashmap::DashMap;
@@ -56,6 +59,48 @@ pub enum UserServiceError {
@@ -76,27 +121,13 @@ struct Claims {
@@ -105,14 +136,15 @@ impl UserService {
@@ -453,24 +485,44 @@ impl UserService {
@@ -501,7 +553,7 @@ mod tests {
```

BEFORE / REMOVED:
```diff
-    jwt_secret: String,
-    /// Create a new UserService instance
-    ///
-    /// # Panics
-    /// Panics if JWT_SECRET environment variable is not set in production builds
-    pub fn new() -> Self {
-        let jwt_secret = std::env::var("JWT_SECRET").unwrap_or_else(|_| {
-            #[cfg(not(debug_assertions))]
```

AFTER / ADDED:
```diff
+use serde::de::DeserializeOwned;
+use std::fmt;
+use std::sync::Arc;
+/// Validated JWT signing and verification secret owned by Identity.
+///
+/// The inner value is intentionally private and its `Debug` implementation is
+/// redacted so application state and errors cannot accidentally expose it.
+#[derive(Clone)]
```

#### 当前 Atlas 返回结果基线

> 这是当前审计基线的结果示例，不代表该历史 PR 的 response 一定完全相同；PR-specific response 变化以 patch 证据为准。

```text
job=Windows Background Server Thread
status=success
```

## 未映射 Changed Files

- `.env.example`
- `crates/router/src/state.rs`
- `crates/router/tests/common.rs`
- `crates/server/tests/api_tests.rs`
- `crates/server/tests/log_api_tests.rs`
- `crates/server/tests/security_invariants.rs`
- `crates/server/tests/test_utils.rs`
- `crates/service/crates/user/examples/usage.rs`

## Execution Classification

- **PR metadata / changed filenames / patch hunks：STATIC CONFIRMED**
- **Affected Flow 判定：STATIC CONFIRMED（当前 Atlas source-path 精确交集）**
- **完整 Flow 拓扑：CURRENT AUDITED BASELINE**
- **历史 PR head 的逐行精确 Runtime 拓扑：NOT CLAIMED，除非 patch 本身直接证明**
