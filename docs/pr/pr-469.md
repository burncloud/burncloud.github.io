---
title: "PR #469 — security(cache): redact Redis credentials from logs and errors"
slug: /pr/pr-469
---

# PR #469 — security(cache): redact Redis credentials from logs and errors

> **状态：** MERGED  
> **作者：** `rustburn`  
> **创建：** `2026-09-13T17:50:59Z`  
> **Base：** `main@c314bff9646f`  
> **Head：** `security/cache-secret-redaction-468@d77fa0063576`  
...
- **Changed Files：** `2`
- **Additions / Deletions：** `+54 / -24`
- **受影响的完整 E2E Flow：** `8`
- **未映射 changed files：** `1`

**PR 说明摘要：** ## Summary Closes #468. This PR performs one security change only: - replaces the Redis URL startup log with a fixed structured log; - prevents raw Redis and serialization backend errors from entering public `CacheError::Display`; - keeps the existing Cache HTTP paths, authentication, envelopes, keys, TTLs, and behavior unchanged; - adds a regression test using a fake credential-bearing Redis URL and verifies that its username, password, internal host, and full URL are absent from the returned public error. ## Scope Changed only: - `crates/service/crates/cache/src/error.rs` - `crates/service/crates/cache/src/service.rs` No directory migration, API cleanup, dependency changes, Server changes, Client changes, Redis key changes, or real credentials. ## Contract The existing Server code may continue formatting `CacheError`, because its public `Display` is now stable and generic: - `cache connection failed` - `cache backend operation failed` - `cache data is invalid` - `cache is disabled` - `cache key was not found` ## Verification Completed: - `git diff --check main...HEAD` - manual scope and diff review - regression test added: `test_connection_error_redacts_redis_url` - existing disabled-cache test remains unchanged Still required before merge: - `cargo fmt --all -- --check` - `cargo test -p burncloud-service-cache` - `cargo clippy -p burncloud-service-cache --all-targets -- -D w…

## Changed Files

| 文件 | 状态 | + | - | Atlas 命中 |
|---|---:|---:|---:|---:|
| `crates/service/crates/cache/src/error.rs` | modified | 9 | 9 | 0 |
| `crates/service/crates/cache/src/service.rs` | modified | 45 | 15 | 8 |
