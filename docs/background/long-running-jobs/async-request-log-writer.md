---
title: "Async Request Log Writer"
slug: /background/long-running-jobs/async-request-log-writer
hide_table_of_contents: true
---

# Async Request Log Writer

**树路径：** `BurnCloud → Background Jobs / Async Side Effects → Long-running Jobs → Async Request Log Writer`

> **中文解释：** 后台消费详细 RequestLog 队列并写入持久层。
>
...
| 2 | `crates/database/crates/router/src/log.rs` | `RouterLogModel::* / usage & billing queries` | Request accounting / usage / billing persistence | READ/WRITE router_logs |
...
| 5 | `crates/router/src/circuit_breaker.rs` | `CircuitBreaker::new(), CircuitBreaker::record_failure_with_type()` | 由 create_router_app() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
| 6 | `crates/router/src/channel_state.rs` | `ChannelStateTracker::new(), ChannelStateTracker::record_error()` | 由 create_router_app() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
