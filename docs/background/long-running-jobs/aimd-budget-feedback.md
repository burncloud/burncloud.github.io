...
| 2 | `crates/router/src/balancer/mod.rs` | `RoundRobinBalancer::new()` | 由 create_router_app() 直接调用 | CALL / runtime-specific |
| 3 | `crates/router/src/limiter.rs` | `RateLimiter::new()` | 由 create_router_app() 直接调用 | CALL / runtime-specific |
| 4 | `crates/router/src/circuit_breaker.rs` | `CircuitBreaker::new(), CircuitBreaker::record_failure_with_type()` | 由 create_router_app() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
| 5 | `crates/router/src/channel_state.rs` | `ChannelStateTracker::new(), ChannelStateTracker::record_error()` | 由 create_router_app() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
| 6 | `crates/router/src/adaptor/detector.rs` | `ApiVersionDetector::new(), detect_and_update(), is_deprecation_error()` | 由 create_router_app() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
| 7 | `crates/service/crates/billing/src/cache.rs` | `PriceCache::empty(), PriceCache::load(), PriceCache::refresh()` | 由 PriceCache::load() 直接调用；由 build_response_with_header() 直接调用；由 create_router_app() 直接调用 | CALL / runtime-specific |
| 8 | `crates/service/crates/billing/src/calculator.rs` | `CostCalculator::calculate(), CostCalculator::new(), CostCalculator::preflight()` | 由 create_router_app() 直接调用；由 proxy_handler() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
| 9 | `crates/router/src/exchange_rate.rs` | `ExchangeRateService::load_rates_from_db(), ExchangeRateService::new(), ExchangeRateService::start_sync_task()` | 由 ExchangeRateService::start_sync_task() 直接调用；由 create_router_app() 直接调用 | CALL / runtime-specific |
| 10 | `crates/router/src/scheduler/mod.rs` | `load_scheduler_config()` | 由 create_router_app() 直接调用 | CALL / runtime-specific |
| 11 | `crates/router/src/rate_budget.rs` | `BudgetGuard::commit(), BudgetGuard::configure(), ChannelBuckets::new(), ConsumeOutcome::is_valid()` | 由 BudgetGuard::configure() 直接调用；由 configure_rate_budget_from_db() 直接调用；由 create_router_app() 直接调用 | CALL / runtime-specific |
| 12 | `crates/server/src/api/response.rs` | `ok()` | 由 create_router_app() 直接调用；由 proxy_handler() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
| 13 | `crates/router/src/price_sync.rs` | `PriceSyncService::new(), PriceSyncService::sync_all(), PriceSyncService::with_config(), start_price_sync_task()` | 由 create_router_app() 直接调用；由 start_price_sync_task() 直接调用 | CALL / runtime-specific |
| 14 | `crates/database/crates/router/src/lib.rs` | `RouterDatabase::deduct_quota(), RouterDatabase::insert_log(), RouterDatabase::insert_request_log(), RouterDatabase::update_token_accessed_time(), RouterDatabase::validate_token_and_get_info(), RouterDatabase::validate_token_detailed()` | 由 create_router_app() 直接调用；由 proxy_handler() 直接调用 | CALL / runtime-specific |
| 15 | `crates/router/src/channel_health_manager.rs` | `ChannelHealthManager::new()` | 由 create_router_app() 直接调用 | CALL / runtime-specific |
