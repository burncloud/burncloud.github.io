...
| 10 | `crates/router/src/adaptor/detector.rs` | `ApiVersionDetector::is_deprecation_error(), ApiVersionDetector::new()` | 由 create_router_app() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
...
| 15 | `crates/router/src/rate_budget.rs` | `BudgetGuard::new(), InMemoryBudget::new()` | 由 create_router_app() 直接调用；由 proxy_logic() 直接调用 | CALL / runtime-specific |
